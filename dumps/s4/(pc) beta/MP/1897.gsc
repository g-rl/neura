// S4 GSC SOURCE
// Generated by https://github.com/xensik/gsc-tool

init( var_0 )
{
    var_1 = scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "getGameType" ) && [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "game", "getGameType" ) ]]() == "br";

    if ( var_1 )
        var_2 = getdvarint( "scr_br_gasMask_health", 108 );
    else
        var_2 = 180;

    if ( !isdefined( level._id_61B4 ) )
    {
        if ( var_1 )
            level._id_61B4 = getdvarfloat( "scr_br_gasMask_resist", 0 );
        else
            level._id_61B4 = 0.2;
    }

    self._id_61B6 = var_2;

    if ( isdefined( var_0 ) )
        self._id_61B6 = var_0;

    if ( var_1 )
        level._id_61B7 = self._id_61B6 / 6;

    level._id_61B8 = var_2;
    self setclientomnvar( "ui_head_equip_class", 2 );
    self setclientomnvar( "ui_gasmask_damage", self._id_61B6 / var_2 );
}

_id_BD57( var_0 )
{
    var_1 = scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "getGameType" ) && [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "game", "getGameType" ) ]]() == "br";
    var_2 = self._id_61B6 / level._id_61B8;
    self._id_61B6 = self._id_61B6 - var_0;
    self._id_61B6 = max( 0, self._id_61B6 );

    if ( var_1 )
        var_3 = ( self._id_61B6 + 1 ) / level._id_61B8;
    else
        var_3 = self._id_61B6 / level._id_61B8;

    self setclientomnvar( "ui_gasmask_damage", var_3 );

    if ( self._id_61B6 <= 0 )
    {
        if ( var_1 && scripts\cp_mp\utility\script_utility::issharedfuncdefined( "gasmask", "breakGasMaskBR" ) )
            self [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "gasmask", "breakGasMaskBR" ) ]]();
        else
            _id_2B70();
    }
    else if ( var_1 )
    {
        var_4 = floor( var_2 * 6 + 0.5 );
        var_5 = floor( var_3 * 6 + 0.5 );

        if ( var_4 > var_5 )
            self playsoundtoplayer( "br_gas_mask_crack_plr", self );
    }

    if ( !isdefined( self._id_61AD ) )
        self._id_61AD = 0;

    self._id_61AD = self._id_61AD + var_0 * level._id_61B4;
    var_6 = floor( self._id_61AD );

    if ( var_6 >= 1 )
    {
        self dodamage( var_6, self.origin, self, undefined, "MOD_TRIGGER_HURT" );
        self._id_61AD = self._id_61AD - var_6;
    }
}

_id_494F( var_0, var_1 )
{
    var_2 = scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "getGameType" ) && [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "game", "getGameType" ) ]]() == "br";

    if ( !var_2 )
        return 0;

    if ( !isdefined( level._id_61B8 ) )
        level._id_61B8 = getdvarint( "scr_br_gasMask_health", 108 );

    var_3 = var_0 / level._id_61B8;
    var_4 = var_1 / level._id_61B8;
    var_5 = floor( var_3 * 6 + 0.5 );
    var_6 = floor( var_4 * 6 + 0.5 );
    return var_5 != var_6;
}

_id_552F()
{
    self endon( "death_or_disconnect" );
    self playsoundtoplayer( "br_gas_mask_on_plr", self );
    var_0 = makeweapon( "none" );
    var_1 = self getcurrentweapon();

    if ( !issameweapon( var_1, var_0 ) )
        self _meth_84AE( "ges_visor_down" );

    self._id_61BA = 1;
    wait 0.338;
    self._id_61BA = 0;
    self._id_61B5 = 1;
    _id_0995::_id_523B();
    self attach( "hat_gasmask" );
    _id_4007();

    if ( isdefined( self._id_AC2A._id_6266 ) && self._id_AC2A._id_6266 == "female" )
        self _meth_84A7( "gender", "gasmask_female", 0.1 );
    else
        self _meth_84A7( "gender", "gasmask_male", 0.1 );
}

_id_C4B8()
{
    self endon( "death_or_disconnect" );

    if ( !istrue( self._id_61B5 ) )
        return;

    self playsoundtoplayer( "br_gas_mask_off_plr", self );
    var_0 = makeweapon( "none" );
    var_1 = self getcurrentweapon();

    if ( !issameweapon( var_1, var_0 ) )
        self _meth_84AE( "ges_visor_up" );

    self._id_61BA = 1;
    wait 0.521;
    self._id_61BA = 0;
    self._id_61B5 = 0;
    _id_0995::_id_4AAA();
    self detach( "hat_gasmask" );
    _id_483E();

    if ( isdefined( self._id_AC2A._id_6266 ) && self._id_AC2A._id_6266 == "female" )
        self _meth_84A7( "gender", "female", 0.1 );
    else
        self _meth_84A7( "gender", "", 0.1 );
}

_id_2B70()
{
    if ( !istrue( self._id_61B5 ) )
        return;

    self._id_61B5 = 0;
    self playsoundtoplayer( "br_gas_mask_crack_plr", self );
    self detach( "hat_gasmask" );
    _id_483E();

    if ( isdefined( self._id_AC2A._id_6266 ) && self._id_AC2A._id_6266 == "female" )
        self _meth_84A7( "gender", "female", 0.1 );
    else
        self _meth_84A7( "gender", "", 0.1 );

    self playsoundtoplayer( "br_gas_mask_depleted_plr", self );
    self setclientomnvar( "ui_head_equip_class", 0 );
    self setclientomnvar( "ui_gasmask_damage", 0 );

    if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "getGameType" ) )
    {
        if ( [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "game", "getGameType" ) ]]() == "br" )
            self._id_61B6 = undefined;
    }
}

_id_4007( var_0, var_1 )
{
    self._id_61B9 = newclienthudelem( self );
    self._id_61B9.x = 0;
    self._id_61B9.y = 0;
    self._id_61B9.alignx = "left";
    self._id_61B9.aligny = "top";
    self._id_61B9.horzalign = "fullscreen";
    self._id_61B9.vertalign = "fullscreen";
    self._id_61B9 setshader( "gasmask_overlay_delta2", 640, 480 );
    self._id_61B9.sort = -10;
    self._id_61B9.archived = 1;

    if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "game", "getGameType" ) )
    {
        if ( [[ scripts\cp_mp\utility\script_utility::getsharedfunc( "game", "getGameType" ) ]]() == "br" || self._id_88C5 )
        {
            self._id_61B9.alpha = 0.0;

            if ( isdefined( var_0 ) )
            {
                if ( isdefined( var_1 ) )
                    wait( var_1 );

                if ( isdefined( self._id_61B9 ) )
                    self._id_61B9 fadeovertime( var_0 );
            }
        }
    }

    if ( isdefined( self._id_61B9 ) )
    {
        self._id_61B9.alpha = 1.0;
        self._id_61B9._id_026A = 1;
    }
}

_id_D49A()
{
    if ( isdefined( self._id_61B9 ) )
        self._id_61B9 setshader( "gasmask_overlay_delta2_broken", 640, 480 );
}

_id_589E( var_0 )
{
    self._id_61B9.alpha = 1.0;
    self._id_61B9 fadeovertime( var_0 );
    self._id_61B9.alpha = 0.0;
}

_id_483E( var_0, var_1 )
{
    level endon( "game_ended" );
    self endon( "death_or_disconnect" );

    if ( !isdefined( self._id_61B9 ) )
        return;

    if ( isdefined( var_0 ) )
    {
        if ( isdefined( var_1 ) )
            wait( var_1 );

        if ( isdefined( self._id_61B9 ) )
        {
            _id_589E( var_0 );
            wait( var_0 );
        }
    }

    if ( isdefined( self._id_61B9 ) )
    {
        self._id_61B9 destroy();
        self._id_61B9 = undefined;
    }
}

_id_DCAF( var_0 )
{
    if ( !var_0 )
        self._id_61B9.alpha = 0.0;
    else
        self._id_61B9.alpha = 1.0;
}

_id_754B( var_0 )
{
    return isdefined( var_0._id_61B6 );
}
