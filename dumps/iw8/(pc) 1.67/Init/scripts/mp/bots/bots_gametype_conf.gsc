...
setup_callbacks()
{
    level.bot_funcs["_encstr_A7610FD916DACA47CB832BF547432DDC6D"] = ::bot_conf_think;
}
...
bot_conf_think()
{
    self notify( "_encstr_A9900FD57F13F1B6BF1A3906A37B7BB06C" );
    self endon( "_encstr_A9900FD57F13F1B6BF1A3906A37B7BB06C" );
    ...
    for (;;)
    {
        ...
        if ( var_0 && self bothasscriptgoal() )
        {
            ...
            if ( scripts\mp\bots\bots_util::bot_vectors_are_equal( self.tag_getting.ground_pos, var_2 ) )
            {
                if ( self botpursuingscriptgoal() )
                    var_1 = 1;
            }
            else if ( scripts\mp\bots\bots_strategy::bot_has_tactical_goal( "_encstr_99D509CBFC59B01903E199" ) && self.tag_getting custom_death_func( self.team ) )
            {
        ...
        if ( isdefined( self.tag_getting ) )
        {
            ...
            if ( !self.conf_camping_tag )
            {
                if ( !scripts\mp\bots\bots_strategy::bot_has_tactical_goal( "_encstr_99D509CBFC59B01903E199" ) )
                {
                    ...
                    var_5._id_132B7 = 1;
                    scripts\mp\bots\bots_strategy::bot_new_tactical_goal( "_encstr_99D509CBFC59B01903E199", self.tag_getting.ground_pos, 25, var_5 );
                }
            }
        }
...
bot_watch_new_tags()
{
    for (;;)
    {
        ...
        level thread debug_display_veh_hit( self, var_0 );
    }
}

debug_display_veh_hit( var_0, var_1 )
{
    if ( level.gametype == "_encstr_A7290676E4D2378C" )
        wait 1.5;
        ...
        if ( isdefined( var_1.victim ) && var_1.victim == var_0 || isdefined( var_1.attacker ) && var_1.attacker == var_0 )
        {
            if ( !isdefined( var_1.on_path_grid ) && !isdefined( var_1.calculations_in_progress ) )
            {
                ...
                if ( var_1.on_path_grid )
                {
                    ...
                    var_2.tag = var_1;
                    var_3[0] = var_2;
                    var_0.tags_seen = bot_combine_tag_seen_arrays( var_3, var_0.tags_seen );
                }
            }
        }
    }
...
bot_find_visible_tags( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( var_3 ) && isdefined( level.dogtags ) )
    {
        ...
        {
            if ( var_7 custom_death_func( self.team ) )
            {
                var_8 = 0;
...
bot_remove_invalid_tags( var_0 )
{
    ...
    {
        if ( var_3.tag custom_death_func( self.team ) && scripts\mp\bots\bots_util::bot_vectors_are_equal( var_3.tag.curorigin, var_3.origin ) )
        {
