...
playanimwithdooropen( var_0, var_1, var_2 )
{
    thread waitfordooropen( var_0, var_1, 1 );
...
opendooratreasonabletime()
{
    ...
    var_5 = calcdooropenspeed();
    self notify( "opening_door" );
    self thread [[ var_1 ]]( var_0, var_5 );
    return var_5;
}
        ...
        if ( isdefined( self._blackboard.doortoopen ) )
        {
            ...
            if ( !self.facemotion )
            {
                ...
                if ( vectordot( var_5, var_6 ) < 0.966 )
                {
                    ...
                    var_7 = opendooratreasonabletime();
            ...
            var_8 = self._blackboard.doortoopen;
            self._blackboard.door_opened = undefined;
            var_9 = 1;
            var_10 = 160;
            var_11 = 2;
            var_12 = length2d( self.velocity );

            if ( !istrue( var_2 ) )
            {
                var_13 = var_1;
                var_14 = scripts\asm\asm::asm_lookupanimfromaliasifexists( var_1, "2" );

                if ( !isdefined( var_14 ) )
                {
                    var_12 = self aigettargetspeed();
                    var_15 = getnextlowestspeedthresholdstring( self.basearchetype, var_12 );

                    if ( isdefined( var_15 ) )
                    {
                        var_16 = var_15 + "2";
                        var_14 = scripts\asm\asm::asm_lookupanimfromaliasifexists( var_1, var_16 );

                        if ( !isdefined( var_14 ) )
                        {
                            var_13 = "door_open";
                            var_14 = scripts\asm\asm::asm_lookupanimfromaliasifexists( "door_open", var_16 );
                        }
                    }
                }

                if ( !isdefined( var_14 ) )
                    var_14 = scripts\asm\asm::asm_lookupanimfromaliasifexists( "door_open", "2" );

                if ( isdefined( var_14 ) )
                {
                    var_17 = scripts\asm\asm::asm_getxanim( var_13, var_14 );
                    var_18 = getnotetracktimes( var_17, "door_touch" );
                    var_19 = ( var_18[0] * getanimlength( var_17 ) + 2 * level.framedurationseconds ) * var_12;
                    var_10 = var_19 + 24;
                }
            }

            var_20 = var_9 + var_10 + var_11;
            var_21 = self [[ self.fngetdoorcenter ]]( var_8 );
            var_22 = distance2d( var_21, self.origin );

            if ( var_22 < var_20 )
            {
                self.isopeningdoor = 1;
                thread closedoorifnecessary( var_8 );

                if ( istrue( var_2 ) || var_22 < var_20 - var_12 * 2 * level.framedurationseconds )
                {
                    var_7 = opendooratreasonabletime();
                ...
                {
                    self setupdooropen( var_8, var_20, getdooropenspeedlookup() );
...
getarrivaltype()
{
    ...
    {
        var_1 = scripts\engine\utility::getbestcovermultinodetype( var_0 );

        if ( isdefined( var_1 ) && var_1 != var_0.type )
            var_0 setcovermultinodetype( var_1 );
    }

    var_2 = var_0.type;
    var_3 = allowlmgarrival();
    ...
    if ( isdefined( self._blackboard.croucharrivaltype ) && !var_3 )
        return self._blackboard.croucharrivaltype;

    return self choosearrivaltype( var_0, var_2 );
}
