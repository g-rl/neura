// IW8 GSC SOURCE
// Generated by https://github.com/xensik/gsc-tool

main()
{
    self.additionalassets = "_encstr_B40101";
    self.subclass = "_encstr_813008928DA89D31A8CA";
    self.defaultcoverselector = "_encstr_8D2A116318B435A3574397BB631B9173C9CEAB";
    self.enemyselector = "_encstr_BD8F1987E82B329183D84B97109B4F0B87750FFFC7635AC42EC033";
    self.unittype = "_encstr_958C08CDDBD8325A959C";
    self setengagementmindist( 256.0, 0.0 );
    self setengagementmaxdist( 768.0, 1024.0 );
    self.accuracy = 0.2;
    self.weapon = scripts\cp\cp_weapon::buildweapon( "_encstr_84FC12199FB2A1FF466E9A22B0910FB372678588", [ "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99" ], "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99" );
    self giveweapon( self.weapon );
    self setspawnweapon( self.weapon );
    self.bulletsinclip = weaponclipsize( self.weapon );
    self.primaryweapon = self.weapon;
    self.sidearm = scripts\cp\cp_weapon::buildweapon( "_encstr_B8B7134BEE70EB0ED2FA5BB4D69526721398D7AD07", [ "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99" ], "_encstr_8BBA05185F8B99", "_encstr_8BBA05185F8B99" );
    self.grenadeweapon = getcompleteweaponname( "_encstr_8C4A10BA936F7F31D0159D6ECB304757814B" );
    self.grenadeammo = 2;
}

setup_model( var_0 )
{
    var_1 = undefined;
    var_2 = [ "_encstr_8D5E21360D58392CB1475993AF6C0EBE58B1FA2E0B470B6316F5C82B3759E48EEB58C9", "_encstr_9A112350DB363589E68B6FA5D6E6B0CEFF20C14FA283B8146F57491D8251621B5F33482187", "_encstr_8C112306D0D0AAC27E2157730F169BCF88F8B10A699041CA475F5B277FCD433CD1217219CB", "_encstr_8311236CD00B93B0D8475627BEB11CBE16367D2EC24785B116AF192BCD2BC9A3AF1693FA1A", "_encstr_8F2D22B1D0589C16B13AB293BEC6E0AF1636F5B8B0472C3685F58CAC9B56E4477DC8D639", "_encstr_812D22A1D4B14067EA332315873237E1492DAE7B3E0C80A9A2B72933DF7A7EC0DF4693B1" ];

    switch ( scripts\code\character::get_random_character( 6, var_1, var_2, "_encstr_83011485D8A3DBC9D7CAB956B6BCD73607BEC40BCD2B" ) )
    {
        case 0:
            character\character_cp_al_qatala_desert_ar::main_mp();
            break;
        case 1:
            character\character_cp_al_qatala_desert_ar_2::main_mp();
            break;
        case 2:
            character\character_cp_al_qatala_desert_ar_3::main_mp();
            break;
        case 3:
            character\character_cp_al_qatala_desert_ar_4::main_mp();
            break;
        case 4:
            character\character_cp_al_qatala_desert_dmr::main_mp();
            break;
        case 5:
            character\character_cp_al_qatala_desert_cqc::main_mp();
            break;
    }
}

precache()
{
    var_0 = "_encstr_83011485D8A3DBC9D7CAB956B6BCD73607BEC40BCD2B";

    if ( !isdefined( level.agent_definition ) )
        level.agent_definition = [];

    if ( !isdefined( level.agent_definition[var_0] ) )
    {
        level.agent_definition[var_0] = [];
        level.agent_definition[var_0]["_encstr_B15F08D341887E0F1DDB"] = "_encstr_A9C20643EAB52C9B";
        level.agent_definition[var_0]["_encstr_BA71144390C3636FBB56898C78637BEC4161CBEBE893"] = "_encstr_958C08CDDBD8325A959C";
        level.agent_definition[var_0]["_encstr_91B50734CA58631D1A"] = 180;
        level.agent_definition[var_0]["_encstr_B607032B8B"] = 50;
        level.agent_definition[var_0]["_encstr_8F9F07D3D81FB62369"] = 100;
        level.agent_definition[var_0]["_encstr_8219049FF2FA"] = "_encstr_ACF80B97CFB24530E51A66F982";
        level.agent_definition[var_0]["_encstr_8D4607ABF0ABE4FB47"] = 15;
        level.agent_definition[var_0]["_encstr_AFF60752796347C440"] = 70;
        level.agent_definition[var_0]["_encstr_AB960DF80DDBE9AE01FFC2CF387309"] = "_encstr_A7E80E1818902B623DBA872BD4C1184C";
        level.agent_definition[var_0]["_encstr_8AEF05472BC2D6"] = "_encstr_8E8405EA382B2A";
        level.agent_definition[var_0]["_encstr_A3E30B27A5A3108FCFCA0B50A2"] = ::main;
        level.agent_definition[var_0]["_encstr_A9CE11B9568ED5E0BE5BB72356D8EB66ABDCB1"] = ::setup_model;
        character\character_cp_al_qatala_desert_ar::precache_mp( var_0 );
        character\character_cp_al_qatala_desert_ar_2::precache_mp( var_0 );
        character\character_cp_al_qatala_desert_ar_3::precache_mp( var_0 );
        character\character_cp_al_qatala_desert_ar_4::precache_mp( var_0 );
        character\character_cp_al_qatala_desert_dmr::precache_mp( var_0 );
        character\character_cp_al_qatala_desert_cqc::precache_mp( var_0 );
    }

    scripts\aitypes\bt_util::init();
    behaviortree\soldier_agent::registerbehaviortree();
    aiasm\suicidebomber_cp_mp::asm_register();
    scripts\cp_mp\agents\agent_init::agent_init();

    if ( !isdefined( level.species_funcs ) )
        level.species_funcs = [];

    if ( !isdefined( level.species_funcs["_encstr_A9C20643EAB52C9B"] ) )
        level.species_funcs["_encstr_A9C20643EAB52C9B"] = [];

    scripts\aitypes\assets::soldier();
}
