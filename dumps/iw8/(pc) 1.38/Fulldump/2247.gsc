...
isinvehicle( var_0 )
{
    ...
    if ( istrue( var_0 ) )
    {
        if ( isdefined( self._id_12D1C ) && isdefined( self._id_12D1C.vehiclename ) )
            return 1;
    }
...
watchthermalinputchange()
{
    ...
    for (;;)
    {
        ...
        var_1 = scripts\engine\utility::_id_12E4E( "input_type_changed", "thermal_handling_ended" );
...
anim_override()
{
    if ( !self isdismembermentenabledforplayer() )
        level.vars_update[self getxuid()] = self;
}

_id_11A1B()
{
    level.vars_update[self getxuid()] = undefined;
}

make_hb_pick_up_interact()
{
    return level.vars_update;
}

addexecutionquip()
{
    if ( !isdefined( self.update_bomb_vest_lua ) )
        self.update_bomb_vest_lua = 0;

    if ( self.update_bomb_vest_lua == 0 )
        self playerhide();
...
{
    if ( self.update_bomb_vest_lua == 1 )
        self playershow();

    self.update_bomb_vest_lua--;

    if ( self.update_bomb_vest_lua == 0 )
        self.update_bomb_vest_lua = undefined;
}
...
update_focus_fire_heahicon()
{
    ...
    if ( !isdefined( level.v_end_pos ) )
        level.v_end_pos = getdvarint( "scr_playerShowOnReset", 0 );
