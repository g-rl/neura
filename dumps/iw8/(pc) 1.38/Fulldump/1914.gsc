...
init()
{
    ...
    scripts\mp\utility\spawn_event_aggregator::registeronplayerspawncallback( ::questtimerinit );
}
...
getspawnpoint()
{
    ...
    if ( isdefined( self.selectedspawnarea ) )
    {
        if ( issubstr( self.selectedspawnarea, "squad" ) )
        {
            ...
            if ( isdefined( var_3.vehicle ) )
            {
                ...
                self._id_12D22 = var_3.vehicle.vehiclename;
            }
    ...
    if ( istrue( level.usesquadspawn ) && istrue( self.squadspawnconfirmed ) )
    {
        ...
        if ( isdefined( var_14 ) && isdefined( self.squadindex ) && self.team == var_14.team && self.squadindex == var_14.squadindex )
        {
            ...
            var_14 thread scripts\mp\utility\points::monitordriverexitbutton( "squad_spawn" );
            thread scripts\mp\utility\points::monitordriverexitbutton( "squad_spawn_self" );
            self._id_12ECD = 1;
        }
    }
    ...
    for (;;)
    {
        ...
        foreach ( var_3 in level.availablespawnlocations[self.team] )
        {
            ...
            if ( isdefined( var_6.objectivekey ) )
                var_8 = istrue( var_6.play_players_take_hvt_firsttime );
            else if ( issubstr( var_3, "vehicle" ) )
            {
                ...
                {
                    var_8 = istrue( var_11.play_players_take_hvt_firsttime );
                    var_9 = obj_room_fire_09( var_11, self );
...
{
    return level.objectives[var_0].ownerteam != var_1 || level.objectives[var_0].stalemate || level.objectives[var_0].captureblocked || istrue( level.objectives[var_0]._id_122E7 ) || level.objectives[var_0].claimteam != "none" && level.objectives[var_0].claimteam != var_1;
}

rear_spotlight_tag( var_0 )
{
    var_1 = gettime();
...
{
    var_0 endon( "death" );
    var_0.play_players_take_hvt_firsttime = 0;
    var_1 = getdvarfloat( "scr_gw_apc_ignore_damage_health_pct", 0.15 );

    for (;;)
    {
        var_2 = gettime();
        var_3 = istrue( var_0.currentcarrier );
        var_4 = scripts\cp_mp\utility\weapon_utility::islockedonto( var_0 );
        var_5 = scripts\cp_mp\vehicles\vehicle_occupancy::_id_12CA0( var_0 );
        ...
        var_14 = var_0.play_players_take_hvt_firsttime;

        if ( var_6 || var_8 || var_9 || var_3 || var_4 || !var_5 )
            var_0.play_players_take_hvt_firsttime = 1;
        else
            var_0.play_players_take_hvt_firsttime = 0;
...
trackriotshield_watchcancelswitchaway( var_0 )
{
    if ( !isdefined( var_0 ) )
        var_0 = "vehicle";
...
spawnselectionthink( var_0 )
{
    ...
    self endon( "disconnect" );
    thread disableuseotherplayerspents();
    ...
    if ( istrue( level._id_12061 ) )
        thread scripts\mp\gametypes\arm::_id_12CE0();

    if ( istrue( level._id_12060 ) )
        thread scripts\mp\gametypes\arm::_id_12CE1();
...
watchforafk()
{
    ...
    if ( isdefined( level._id_122FD ) )
        wait( level._id_122FD );
    else
        wait 60.0;
    ...
    for (;;)
    {
        ...
        if ( var_0 == "tac_ops_spawn_area_selected" )
        {
            ...
            foreach ( var_4 in level.spawnselectionlocations )
            {
                ...
                {
                    _id_12B2B( var_4[self.team].typeref, self, self.team );

                    if ( isdefined( var_4[self.team].objectivekey ) && isobjectiveindanger( var_4[self.team].objectivekey, self.team ) )
                        break;

                    if ( isdefined( var_4[self.team].typeref ) && issubstr( var_4[self.team].typeref, "vehicle" ) && istrue( var_4[self.team].dynamicent.play_players_take_hvt_firsttime ) )
                        break;
            ...
            if ( !isdefined( var_2 ) )
            {
                var_6 = minsteps( self.team, self.squadindex );
                ...
                foreach ( var_4 in var_6 )
                {
                    if ( isdefined( var_4.dynamicent ) && var_4.dynamicent getentitynumber() == var_1 )
                    {
                        ...
                        var_9 = spawnstruct();
                        var_9 = scripts\mp\spawnscoring::delaystreamtomovingplane( var_9, var_4.dynamicent );

                        if ( !isdefined( var_9._id_122EB ) )
                        {
                            var_4.dynamicent.spawnproplist = gettime();
                            break;
                        }
...
_id_12B1B( var_0 )
{
    if ( !isdefined( self.neurotoxin_damage_loop ) )
        self.neurotoxin_damage_loop = [];

    var_1 = 65536;
    var_2 = 131072;
    var_3 = 262144;
    var_4 = 524288;
    self.neurotoxin_damage_loop["rightmouseup"] = istrue( self.neurotoxin_damage_loop["rightmouseup"] ) || ( var_0 & var_1 ) > 0;
    self.neurotoxin_damage_loop["leftmouseup"] = istrue( self.neurotoxin_damage_loop["leftmouseup"] ) || ( var_0 & var_2 ) > 0;
    self.neurotoxin_damage_loop["activate"] = istrue( self.neurotoxin_damage_loop["activate"] ) || ( var_0 & var_3 ) > 0;
    self.neurotoxin_damage_loop["gostand"] = istrue( self.neurotoxin_damage_loop["gostand"] ) || ( var_0 & var_4 ) > 0;
...
watchendconditions()
{
    ...
    var_0 = scripts\engine\utility::_id_12E52( "tac_ops_map_selection_valid", "tac_ops_map_changed", "tac_ops_map_cleared", "spawned_player", "tac_ops_map_game_ended", "tac_ops_map_nuke" );
...
endcondition_focuschanged( var_0 )
{
    ...
    foreach ( var_3 in level.spawnselectionlocations )
    {
        ...
        {
            thread regroup_think( var_3[self.team].anchorentity.origin );
    ...
    if ( !isdefined( var_1 ) )
    {
        var_5 = minsteps( self.team, self.squadindex );

        foreach ( var_3 in var_5 )
        {
            if ( isdefined( var_3.dynamicent ) && var_3.dynamicent getentitynumber() == var_0 )
            {
                thread regroup_think( var_3.dynamicent.origin );
                var_1 = var_3.typeref;
                break;
            }
        }
...
regroup_think( var_0 )
{
    ...
    if ( !isdefined( level._id_1302D ) )
        level._id_1302D = max( getdvarint( "NOLPSOTORP", 20000 ) * 0.5 / 1000, 1.0 );

    for (;;)
    {
        self predictstreampos( var_0, 1 );
        wait( level._id_1302D );
    }
}
...
evaluatesquadspawn()
{
    ...
    for ( var_4 = 0; var_4 < 4; var_4++ )
    {
        ...
        var_5.play_authentication_error_window = undefined;
        var_3[var_4] = var_5;
    }
    ...
    for (;;)
    {
        ...
        var_11 = minsteps( self.team, self.squadindex );
        ...
        for ( var_4 = 0; var_4 < 4; var_4++ )
        {
            ...
            {
                var_15 = istrue( var_9.play_players_take_hvt_firsttime );
                var_16 = var_1[var_4];
                ...
                if ( !isdefined( var_3[var_4].play_authentication_error_window ) || var_3[var_4].play_authentication_error_window != var_16 )
                {
                    var_3[var_4].play_authentication_error_window = var_16;
            ...
            if ( isdefined( var_3[var_4].play_authentication_error_window ) && var_3[var_4].play_authentication_error_window != -1 )
            {
                var_3[var_4].play_authentication_error_window = -1;
...
handlesquadspawncycle()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::_id_12E47( "cyclePos", "cycleNeg" );
...
questtimerinit()
{
    thread plunder_repositoryendcountdown();

    if ( getdvarint( "scr_squad_spawn_debug", 0 ) == 1 )
        thread scripts\mp\spawnscoring::_id_12389();
}

plunder_repositoryendcountdown()
{
    ...
    for (;;)
    {
        if ( issquadspawnable( self ) )
            self.play_players_take_hvt_firsttime = issquadmateindanger( self );
...
issquadmateindanger( var_0 )
{
    ...
    if ( isdefined( var_0.revivent_watchfordeath_safety ) && var_0.revivent_watchfordeath_safety + 3000 > var_1 )
        return 1;
    ...
    var_2 = var_0 getclosestenemy( 384, 100, 1 );
    ...
    {
        if ( istrue( var_0.vehicle.currentcarrier ) )
            return 1;
    ...
    {
        var_3 = var_0 getspawnbucketforplayer( 1, var_0.origin );
    ...
    if ( isdefined( var_0 ) && isdefined( var_0.spawnproplist ) && var_0.spawnproplist + 1000 > var_1 )
        return 1;
...
timeuntilspawnmessaging( var_0, var_1 )
{
    ...
    if ( var_0 > 0 )
    {
        ...
        scripts\engine\utility::_id_12E59( var_0, "force_spawn" );
    }
}
...
managesquadcameraposition( var_0 )
{
    ...
    while ( isdefined( self.spawncameraent ) && isdefined( var_2 ) )
    {
        ...
        else
        {
            ...
            var_4 = var_4 + scripts\mp\gametypes\arm::calculatecameraoffset( self.team, var_2.origin );
        }
...
managevehiclecameraposition( var_0 )
{
    ...
    while ( isdefined( self.spawncameraent ) && isdefined( var_2 ) && !istrue( var_2.isdestroyed ) )
    {
        ...
        else
        {
            ...
            var_5 = var_5 + scripts\mp\gametypes\arm::calculatecameraoffset( self.team, var_9 );
        }
...
getclosestavailablespawnlocation()
{
    ...
    else
    {
        ...
        {
            var_7 = minsteps( self.team, self.squadindex );
        ...
        foreach ( var_1 in level.teamnamelist )
        {
            foreach ( var_3 in level.availablespawnlocations[var_1] )
            {
                ...
                if ( isdefined( var_4.objectivekey ) )
                    var_4.play_players_take_hvt_firsttime = isobjectiveindanger( var_4.objectivekey, var_1 );
            }
        }
                ...
                if ( !issubstr( var_4, "HQ" ) && issubstr( var_4, "fob" ) )
                {
                    ...
                    if ( isdefined( var_2 ) )
                    {
                        ...
                        if ( var_5 )
                        {
                            ...
                            var_15 = var_2 getspawnbucketforplayer();
                        ...
                        if ( var_6 )
                        {
                            ...
                            var_16 = var_2 getspawnbucketforplayer();
                    ...
                    level.objectives[var_17]._id_122E7 = var_14;
                    level.objectives[var_17]._id_122E8 = gettime();
                }
            }
        }
...
_id_12B2B( var_0, var_1, var_2 )
{
    var_3 = level.spawnselectionlocations[var_0][var_2].objectivekey;

    if ( !isdefined( var_3 ) || !isdefined( level.objectives[var_3] ) || !isdefined( level.objectives[var_3]._id_122E8 ) || level.objectives[var_3]._id_122E8 == gettime() )
        return;
    ...
    if ( isdefined( var_1 ) )
    {
        ...
        if ( var_4 )
        {
            ...
            var_14 = var_1 getspawnbucketforplayer();
        ...
        {
            scripts\mp\spawnlogic::activatespawnset( var_7, 1 );
            var_15 = var_1 getspawnbucketforplayer();
            var_15 = scripts\mp\spawnlogic::getspawnbucketfromstring( var_15 );
        }
    ...
    level.objectives[var_3]._id_122E7 = var_13;
    level.objectives[var_3]._id_122E8 = gettime();
}
