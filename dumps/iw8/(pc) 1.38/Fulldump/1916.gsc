...
init()
{
    ...
    scripts\mp\supers\super_stoppingpower::_id_1250A();
    scripts\mp\utility\spawn_event_aggregator::registeronplayerspawncallback( ::_id_12589 );
...
loadsupertable()
{
    ...
    for (;;)
    {
        ...
        var_2 = spawnstruct();
        level.superglobals.staticsuperdata[var_1] = var_2;
        var_2.id = var_0;
        var_2.ref = var_1;
        var_2.weapon = readsupertablecell( var_0, 1 );
        var_3 = undefined;

        if ( scripts\mp\utility\game::isanymlgmatch() )
            var_3 = readsupertablecell( var_0, 28, 1 );

        if ( !isdefined( var_3 ) )
            var_3 = readsupertablecell( var_0, 2, 1 );

        var_2.pointsneeded = var_3;
        var_4 = undefined;

        if ( scripts\mp\utility\game::isanymlgmatch() )
            var_4 = readsupertablecell( var_0, 26, 1 );

        if ( !isdefined( var_4 ) )
            var_4 = readsupertablecell( var_0, 4, 1 );

        var_2.usetime = var_4;
        var_2.maxactivations = readsupertablecell( var_0, 5, 1 );
        var_2.x1opstransition = readsupertablecell( var_0, 6, 1 );
        var_2.useweapon = readsupertablecell( var_0, 11 );
        var_2.useweaponclipammo = readsupertablecell( var_0, 12, 1 );
        var_2.useweaponstockammo = readsupertablecell( var_0, 13, 1 );
        var_2.useweapontrackstats = readsupertablecell( var_0, 31, 1 ) > 0;
        var_2.movespeed = readsupertablecell( var_0, 14, 1 );
        var_5 = undefined;

        if ( scripts\mp\utility\game::isanymlgmatch() )
            var_5 = readsupertablecell( var_0, 27, 1 );

        if ( !isdefined( var_5 ) )
            var_5 = readsupertablecell( var_0, 15, 1 );

        var_2.graceperiod = var_5;
        var_2.maxactivationsalt = readsupertablecell( var_0, 16, 1 );
        var_2.usedelay = readsupertablecell( var_0, 18, 1 );
        var_2.convoy_pos = readsupertablecell( var_0, 19, 1 );
        var_2.archetype = readsupertablecell( var_0, 17 );
        var_2.isweapon = readsupertablecell( var_0, 20, 1 );
        var_2.canuseinlaststand = readsupertablecell( var_0, 32, 1 ) > 0;
        var_2.initlocs_morsephones = [];

        if ( isdefined( var_2.useweapon ) )
            var_2.useweapon = asmdevgetallstates( var_2.useweapon );

        level.superglobals.supersbyid[var_0] = var_1;

        if ( !isdefined( var_2.weapon ) )
            level.superglobals.staticsuperdata[var_1] = undefined;
        else
            level.superglobals.supersbyoffhand[var_2.weapon] = var_2;

        if ( !isdefined( var_2.pointsneeded ) )
            level.superglobals.staticsuperdata[var_1] = undefined;

        if ( isdefined( var_2.maxactivations ) )
        {
            if ( var_2.maxactivations > 0 )
                var_2.activatepenalty = 1.0 / var_2.maxactivations;
            else
                var_2.activatepenalty = 1.0;
        }

        if ( isdefined( var_2.maxactivationsalt ) )
        {
            if ( var_2.maxactivationsalt > 0 )
                var_2.activatepenaltyalt = 1.0 / var_2.maxactivationsalt;
        }

        if ( isdefined( var_2.useweapon ) )
            level.superglobals.superweapons[var_2.useweapon.basename] = var_2;

        if ( var_2.weapon == "<default>" )
            var_2.weapon = "super_default_mp";

        if ( isdefined( var_2.graceperiod ) )
            var_2.graceperiod = var_2.graceperiod * 1000.0;
        else
            var_2.graceperiod = 0.0;
    ...
    if ( isdefined( var_6 ) && var_6 != "" )
        level.superglobals.superearnratemultiplier = float( var_6 );
}
...
registersupers()
{
    ...
    registersuper( "super_select", ::_id_12592, ::_id_12593, ::superselectbeginuse, undefined );
    registersuper( "super_fulton", undefined, undefined, ::_id_1258C, undefined );
    registersuper( "super_supply_drop", undefined, undefined, ::_id_12595, undefined );
    registersuper( "super_vehicle_drop", undefined, undefined, ::_id_12597, undefined );
    registersuper( "super_loot_drop", undefined, undefined, ::_id_12595, undefined );
    _id_1195B( "super_emp_drone", "emp_drone_player_mp" );
    _id_1195B( "super_emp_drone", "emp_drone_non_player_mp" );
    _id_1195B( "super_emp_drone", "emp_drone_non_player_direct_mp" );
    _id_1195B( "super_weapon_drop", "deploy_weapondrop_mp" );
}
...
_id_1195B( var_0, var_1 )
{
    ...
    var_2.initlocs_morsephones[var_2.initlocs_morsephones.size] = var_1;
    level.superglobals._id_12591[var_1] = var_2;
}
...
givesuper( var_0, var_1, var_2 )
{
    ...
    var_4.tank_durations = undefined;
    _id_11EFF( 0 );
    _id_1258F( var_0 );
...
clearsuper( var_0 )
{
    ...
    if ( isdefined( var_1 ) && isdefined( var_1.staticdata ) )
        _id_12590( var_1.staticdata.ref );
    ...
    thread scripts\mp\trials\trial_target_utility::_id_11E97( "ui_super_state", 0 );
...
_id_12B2E()
{
    ...
    {
        var_0.stealth_enabled = undefined;
        return;
    }
    ...
    if ( !isdefined( var_0.stealth_enabled ) || var_1 != var_0.stealth_enabled )
        self setclientomnvar( "ui_super_progress", var_1 );

    self setplayersupermeterprogress( var_1 );
    var_0.stealth_enabled = var_1;
}
...
updatesuperuistate()
{
    ...
    if ( !isdefined( var_1 ) || var_2 != var_1 )
        thread scripts\mp\trials\trial_target_utility::_id_11E97( "ui_super_state", var_2 );
...
superusefinished( var_0, var_1, var_2, var_3 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        else if ( var_4.staticdata.convoy_pos )
        {
            clearsuper();
            self setclientomnvar( "ui_perk_package_state", 0 );
            self setclientomnvar( "ui_super_progress", 0 );
    ...
    scripts\cp\helicopter\chopper_boss::superterrainlightbakelodoverride( var_4, var_5 );
}
...
handledeath()
{
    ...
    var_0 = getcurrentsuper();
    var_1 = var_0.staticdata.x1opstransition;
...
getsuperpointsneeded()
{
    ...
    if ( isdefined( var_0.tank_durations ) )
        var_1 = var_0.tank_durations;
...
missiontime( var_0 )
{
    ...
    if ( !isdefined( level.superglobals._id_12591[var_0] ) )
        return undefined;

    return level.superglobals._id_12591[var_0].ref;
}
...
updateactivesupers( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7 )
{
    ...
    if ( var_11 )
    {
        ...
        if ( var_8 && var_0 scripts\mp\utility\perk::_hasperk( "specialty_bulletdamage" ) )
            var_1 thread scripts\mp\supers\super_stoppingpower::_id_1250F( var_5 );
    }
}
...
scramblebeginuse()
{
    ...
    thread scripts\cp_mp\killstreaks\helper_drone::original_meleechargedistvsplayer( self );
    return 1;
}
...
_id_12592()
{
    ...
    self.super.tank_durations = var_5;
}
