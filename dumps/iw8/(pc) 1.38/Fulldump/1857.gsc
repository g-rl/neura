...
init()
{
    ...
    level.set_relic_dogtags = getdvarint( "scr_map_use10v10_objectives", 0 );
    level.teambased = 0;
...
zombieregenratescaleoutgas()
{
    if ( ( scripts\mp\utility\game::getgametype() == "arm" || scripts\mp\utility\game::propwaitminigamehudinit() || scripts\mp\utility\game::rear_spotlight_target_found_dist_sq() ) && getdvarint( "scr_game_forceDefaultFallHeight", 0 ) != 1 )
    {
...
{
    setdvar( "TPKMQLMPP", 1 );
    var_0 = scripts\mp\utility\game::getgametype() == "br" || scripts\mp\utility\game::getgametype() == "brtdm";
...
setupcallbacks()
{
    ...
    scripts\cp\vehicles\vehicle_damage_cp::register_ai_damage_callbacks();
}

setdefaultcallbacks()
{
    level.callbackstartgametype = scripts\mp\gamelogic::callback_startgametype;
    level.deactivategastrap = scripts\mp\playerlogic::deactivate_station;
