...
main()
{
    ...
    if ( scripts\mp\gametypes\br_gametypes::proggressionmismatchpopup( "allowedEntities" ) )
        var_0 = scripts\mp\gametypes\br_gametypes::lootleadermarksizedynamic( "allowedEntities" );
    ...
    if ( scripts\mp\gametypes\br_gametypes::propsetflashesleft( "prematchBlueprints" ) )
    ...
    {
        level.shouldgetnewspawnpoint = scripts\mp\gametypes\br_weapons::code;
    ...
    level.shouldrespawn = scripts\mp\gametypes\br_public::shouldrespawn;
    ...
    level._id_12D16 = scripts\mp\gametypes\br_public::choose_and_drop_tank_near_hostage;
    level.allowclasschoicefunc = scripts\mp\gametypes\br_weapons::chopperexfil_sh070_start;
    ...
    level.br_infils_disabled = getdvarint( "scr_br_infils_disable", 0 ) || scripts\mp\gametypes\br_gametypes::propsetclonesleft( "infil" );
    ...
    scripts\mp\gametypes\br_analytics::chopperoccupied();
    ...
    scripts\mp\gametypes\br_ending::checkpoint_next_fx_set();
...
monitor_traversal_watcher( var_0, var_1, var_2 )
{
    ...
    if ( !istrue( var_2 ) )
        scripts\mp\gametypes\br_weapons::checkpoint_overwatch();
...
nakeddrop()
{
    var_0 = scripts\mp\gametypes\br_gulag::mp_vacant_patch_thread();

    if ( !var_0 && istrue( level.br_prematchstarted ) && !istrue( self.gulag ) )
    {
        if ( scripts\mp\gametypes\br_gametypes::progress_obj( "playerNakedDropLoadout" ) )
            scripts\mp\gametypes\br_gametypes::_id_11BE4( "playerNakedDropLoadout" );
        else
            spawn_remote_tank_thermite();
    }
...
handle_nav_bounds_buildings( var_0, var_1, var_2, var_3 )
{
    ...
    var_13 = var_13 * var_6;
    var_14 = scripts\mp\gametypes\br_public::weapon_xp_iw8_ar_akilo47( var_12, var_13[0], var_13[1], var_2, var_3, var_5, var_0 );
    ...
    var_14 = scripts\mp\gametypes\br_public::weapon_xp_iw8_ar_akilo47( var_12, 0, 0, var_2, var_3, var_5 );
...
clear_bomb_vest_controller_holder()
{
    if ( scripts\mp\gametypes\br_gametypes::propsetclonesleft( "giveStartFieldUpgrade" ) )
        return;
    ...
    if ( istrue( level.allowsupers ) )
    {
        ...
        scripts\mp\gametypes\br_pickups::forcegivesuper( self.score_event_nuked );
}
...
tracking_munitions_purchase()
{
    ...
    else
    {
        ...
        if ( scripts\mp\gametypes\br_gametypes::progress_obj( "getDefaultLoadout" ) )
            var_1 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "getDefaultLoadout" );
...
onstartgametype()
{
    ...
    scripts\mp\gametypes\br_pickups::chopper_boss_fight_spawn_watcher();
    ...
    scripts\mp\gametypes\br_events::cleanuppropcontrolshud();
    ...
    scripts\cp_mp\utility\script_utility::registersharedfunc( "br", "superSlotCleanUp", scripts\mp\gametypes\br_pickups::_id_12594 );
    scripts\cp_mp\utility\script_utility::registersharedfunc( "br", "challengeEvaluator", scripts\mp\gametypes\br_challenges::dialog_wait_ready );
    ...
    scripts\cp_mp\utility\script_utility::registersharedfunc( "gasmask", "breakGasMaskBR", scripts\mp\gametypes\br_pickups::cleanupafterweaponswitch );
    scripts\cp_mp\utility\script_utility::registersharedfunc( "game", "skipPlayerVO", ::convoy_turret );
    scripts\cp_mp\utility\script_utility::registersharedfunc( "game", "playSoundToSquad", scripts\mp\gametypes\br_public::vehicle_isfriendlytoteam );
    scripts\cp_mp\utility\script_utility::registersharedfunc( "game", "getSquadPlayers", scripts\mp\gametypes\br_public::minshotstostage3acc );
    ...
    thread scripts\mp\gametypes\br_gametype_dmz::_id_12E02();
    ...
    scripts\mp\gametypes\br_gametypes::_id_11BE4( "onStartGameType" );
}
...
successfulteams()
{
    ...
    if ( !scripts\mp\gametypes\br_public::isusinginfilselection() && clean_up_strafe() )
...
_id_119D6()
{
    ...
    {
        if ( !scripts\mp\gametypes\br_gametypes::propsetclonesleft( "gulag" ) )
            level scripts\mp\gametypes\br_gametypes::give_fists( "gulag" );
    ...
    if ( !istrue( level.br_circle_disabled ) )
    {
        level scripts\mp\gametypes\br_circle::bombs_setup();
        level thread scripts\mp\gametypes\br_circle::additionalrecondronetargets();
    }
}
...
onprematchstarted()
{
    ...
    {
        if ( scripts\mp\gametypes\br_gametypes::progress_obj( "createC130PathStruct" ) )
            var_0 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "createC130PathStruct" );
        else
    ...
    if ( !istrue( level.br_infils_disabled ) )
    {
        ...
        if ( istrue( level.infilcanusemap ) )
        {
            if ( clean_up_vandalize() && !clampstepbulletdamage() )
                scripts\mp\gametypes\br_rewards::vo_one_remain( 1 );
            ...
            foreach ( var_9 in level.players )
            {
                ...
                {
                    var_9 thread scripts\mp\gametypes\br_infils::_id_126BB();
                    continue;
                }
        ...
        if ( scripts\mp\gametypes\br_gametypes::progress_obj( "infilSequence" ) )
            scripts\mp\gametypes\br_gametypes::_id_11BE4( "infilSequence" );
        else if ( istrue( level.infilcanusec130 ) && !istrue( level.infilcanusemap ) )
        {
            var_11 = undefined;

            if ( scripts\mp\gametypes\br_gametypes::progress_obj( "getInfilPlayers" ) )
                var_11 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "getInfilPlayers" );

            if ( !getdvarint( "scr_br_use_script_model_infil", 0 ) )
                scripts\mp\gametypes\br_infils::bufferedammoboxdata( var_0, "player", var_11 );
            else
            {
                scripts\mp\gametypes\br_infils::bufferedammoboxdata( var_0, "script_model" );
    ...
    {
        if ( scripts\mp\gametypes\br_gametypes::progress_obj( "skipInfilSequence" ) )
            scripts\mp\gametypes\br_gametypes::_id_11BE4( "skipInfilSequence" );
        ...
        if ( !isdefined( var_11 ) || var_11.size > 0 )
            scripts\mp\gametypes\br_infils::_id_12DFE( var_11 );
...
notstand()
{
    ...
    if ( clean_up_vandalize() && clampstepbulletdamage() )
    {
        if ( !scripts\mp\gametypes\br_public::rear_minigun_model() && !scripts\mp\gametypes\br_public::processvoqueue() )
        {
            var_0 = clear_and_give_killstreak_loadout_assault();
            scripts\mp\gametypes\br_rewards::get_track_controller_struct( var_0 );

            if ( scripts\mp\gametypes\br_gametypes::propsetflashesleft( "dropBagLoop" ) )
            {
                for (;;)
                    scripts\mp\gametypes\br_rewards::get_track_controller_struct( var_0 );
            }
        ...
        foreach ( var_2 in level.players )
        {
            ...
            foreach ( var_5 in level.calloutglobals.calloutzones )
            {
                if ( var_2 istouching( var_5 ) )
                {
                    ...
                    var_2 thread scripts\mp\gametypes\br_gametypes::_id_11BE4( "onEnterCalloutArea", var_5.script_noteworthy );
                    break;
                }
            }
...
watchprematchdone()
{
    ...
    if ( !scripts\mp\gametypes\br_gametypes::propsetflashesleft( "allowLateJoiners" ) )
    {
    ...
    scripts\mp\gametypes\br_analytics::chosencodephones( var_0.size );
    level _id_11ED9();
}
...
getprematchlocationspawnorigins()
{
    ...
    var_5 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "prematchSpawnMaxLocations" );
...
maxelderrank( var_0, var_1 )
{
    var_2 = 5000;
    var_3 = -5000;
    var_4 = scripts\mp\gametypes\br_public::helihint_deposit( var_0, var_2, var_3 );
...
{
    var_0 = int( 150 / scripts\mp\gametypes\br_public::matchdata_logplayerdata() );
    return var_0;
}

getprematchspawnorigin()
{
    var_0 = scripts\mp\gametypes\br_gametypes::propsetflashesleft( "doFirstUnusedPrematchSpawnOrigin" );
    ...
    {
        if ( scripts\mp\gametypes\br_gametypes::propsetclonesleft( "randomizePrematchSpawnOriginNextIdx" ) )
    ...
    var_3 = scripts\mp\gametypes\br_public::minplunderdropondeath( self.sessionteam, self.squadindex, "prematchSpawnOrigin" );

    if ( !isdefined( var_3 ) )
    {
        var_3 = level.prematchspawnorigins[level.prematchspawnoriginnextidx];
        scripts\mp\gametypes\br_public::_id_11EFC( self.sessionteam, self.squadindex, "prematchSpawnOrigin", var_3 );
        level.vstartposition[level.prematchspawnoriginnextidx]++;
        var_4 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "prematchSpawnNumTeamsPerLocation" );
...
onplayerconnect( var_0 )
{
    ...
    if ( scripts\mp\gametypes\br_gametypes::progress_obj( "playerWelcomeSplashes" ) )
        var_0 thread scripts\mp\gametypes\br_gametypes::_id_11BE4( "playerWelcomeSplashes" );
    else
        var_0 thread vehicle_compass_deregisterinstance();

    var_0 _id_11A74();
    scripts\mp\gametypes\br_gametypes::_id_11BE4( "onPlayerConnect", var_0 );
    ...
    if ( isdefined( var_0 ) )
    {
        if ( !scripts\mp\gametypes\br_public::prophasclonesleft() && !scripts\mp\gametypes\br_public::rear_minigun_model() && !scripts\mp\gametypes\br_public::processvoqueue() && !scripts\mp\utility\game::race_set_checkpoint() )
        {
            if ( !istrue( level._id_120B6 ) )
            {
        ...
        if ( !scripts\mp\gametypes\br_public::rear_minigun_model() && !scripts\mp\gametypes\br_public::prophasclonesleft() && scripts\mp\utility\game::missions_clearinappropriaterewards() != "dmz" && !scripts\mp\utility\game::race_set_checkpoint() )
...
onspawnplayer()
{
    ...
    if ( !scripts\mp\gametypes\br_gametypes::propsetclonesleft( "waitLoadoutDone" ) )
    ...
    scripts\mp\gametypes\br_public::_id_11EDC( 0 );
    scripts\mp\gametypes\br_public::_id_11EDA( 0 );
...
monitor_traversal_timer( var_0 )
{
    ...
    if ( !var_0 scripts\mp\gametypes\br_public::obj_room_fire_08() )
        return;
...
waitloadoutdone()
{
    ...
    if ( scripts\mp\gametypes\br_public::rear_minigun_model() || scripts\mp\gametypes\br_public::prophasclonesleft() )
        return;
    ...
    if ( !istrue( level.br_prematchstarted ) )
    {
        ...
        {
            self.votesys_new = 1;
            scripts\engine\utility::delaythread( 1.0, scripts\mp\gametypes\br_public::cloud_cover, "prematch_enter", self );
    ...
    scripts\mp\gametypes\br_weapons::br_ammo_player_clear();
    var_3 = istrue( self.gulag ) || scripts\mp\gametypes\br_public::rear_minigun_model() || istrue( self._id_11AA7 );
    scripts\mp\gametypes\br_pickups::resetplayerinventory( var_3 );

    if ( !var_3 && !scripts\mp\gametypes\br_gametypes::propsetclonesleft( "armor" ) )
        scripts\mp\gametypes\br_armor::monitor_truck();
...
brmodifyplayerdamage( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10 )
{
    ...
    if ( var_3 > 0 )
    {
        ...
        if ( scripts\mp\gametypes\br_gametypes::progress_obj( "modifyPlayerDamage" ) )
        {
            ...
            var_3 = thread scripts\mp\gametypes\br_gametypes::_id_11BE4( "modifyPlayerDamage", var_16 );
        }
    }
...
{
    if ( scripts\mp\gametypes\br_gametypes::progress_obj( "modifyVehicleDamage" ) )
        return scripts\mp\gametypes\br_gametypes::_id_11BE4( "modifyVehicleDamage", var_0 );
...
{
    if ( scripts\mp\gametypes\br_gametypes::progress_obj( "allowMeleeVehicleDamage" ) )
        return scripts\mp\gametypes\br_gametypes::_id_11BE4( "allowMeleeVehicleDamage", var_0 );
...
{
    if ( scripts\mp\gametypes\br_gametypes::progress_obj( "ignoreVehicleExplosiveDamage" ) )
        return scripts\mp\gametypes\br_gametypes::_id_11BE4( "ignoreVehicleExplosiveDamage", var_0 );
...
{
    if ( scripts\mp\gametypes\br_gametypes::progress_obj( "shouldLastStandDamageScale" ) )
        return scripts\mp\gametypes\br_gametypes::_id_11BE4( "shouldLastStandDamageScale", var_0 );
...
show_headicon_to( var_0, var_1 )
{
    ...
    if ( isscriptabledefined() )
    {
        ...
        if ( !isdefined( var_2 ) )
        {
            var_5 = scripts\mp\gametypes\br_public::helihint_deposit( var_0.origin, 30 );
...
onplayerdamaged( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11, var_12, var_13 )
{
    ...
    if ( scripts\mp\gametypes\br_gametypes::progress_obj( "onPlayerDamaged" ) )
    {
        var_17 = scripts\cp_mp\utility\damage_utility::packdamagedata( var_1, var_2, var_3, var_6, var_5, var_0, undefined, var_9 );
        thread scripts\mp\gametypes\br_gametypes::_id_11BE4( "onPlayerDamaged", var_17 );
    }
}
...
doplayerkilledsplashes( var_0, var_1 )
{
    ...
    var_2 = scripts\mp\gametypes\br_public::minigunbackup( var_0.team, var_0.squadindex );
...
{
    if ( !scripts\mp\gametypes\br_gametypes::propsetclonesleft( "teamSpectate" ) )
        scripts\mp\gametypes\br_spectate::setup_teleport_rooms( var_0, undefined, 0 );
    ...
    var_6 = var_2.size + 1;
    scripts\mp\gametypes\br_analytics::chopperexfil_slamtoblack( var_0, int( scripts\mp\utility\player::isreallyalive( var_0 ) ) );
    ...
    if ( isdefined( var_0 ) )
    {
        if ( scripts\mp\flags::gameflag( "prematch_done" ) )
        {
            ...
            _id_12AED( var_0 );
            var_0 scripts\mp\gametypes\br_challenges::spawn_weapon_box_cache();
        }
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( scripts\mp\gametypes\br_gametypes::progress_obj( "onPlayerKilled" ) )
    {
        var_10 = scripts\cp_mp\utility\damage_utility::packdamagedata( var_1, self, var_2, var_4, var_3, var_0, undefined, var_5 );
        var_10.hitloc = var_6;
        thread scripts\mp\gametypes\br_gametypes::_id_11BE4( "onPlayerKilled", var_10 );
    }
    ...
    var_11 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "markPlayerAsEliminatedOnKilled" );
    ...
    if ( istrue( level.cleanupswaploadoutflags.binoculars_processlosqueue ) )
        self._id_12ECE = scripts\mp\gametypes\br_public::update_timer_for_bomb_vest_detonator_holder();
...
makeweaponfromcrate( var_0 )
{
    ...
    var_2 = scripts\mp\gametypes\br_pickups::manage_fakebody_hides( var_1 );
...
makeitemfromcrate( var_0 )
{
    ...
    var_3 = var_2;
    var_4 = scripts\mp\gametypes\br_pickups::maphint_computerscriptableused( var_1, ( 0, 0, 90 ) );
...
set_customizable_values( var_0 )
{
    ...
    var_2 = scripts\mp\gametypes\br_pickups::maphint_computerscriptableused( var_1, ( 0, 0, 90 ) );
...
subway_death_or_disconnect_monitor( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        for ( var_2 = 1; var_2 < scripts\mp\gametypes\br_public::matchdata_logplayerdata() + 1; var_2++ )
...
start_coop_escape_safehouse( var_0 )
{
    ...
    foreach ( var_4 in var_2 )
    {
        if ( scripts\mp\utility\teams::getteamdata( var_4, "aliveCount" ) )
        {
            ...
            {
                if ( isdefined( var_8 ) && !var_8 scripts\mp\gametypes\br_public::questtypes() )
                {
                    var_5 = 1;
                    break;
                }
...
supply_station_direction( var_0 )
{
    ...
    foreach ( var_4 in level.teamnamelist )
    {
        ...
        if ( var_5 )
        {
            ...
            if ( scripts\mp\gametypes\br_gametypes::progress_obj( "remainingPlayersAliveOnTeam" ) )
                var_6 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "remainingPlayersAliveOnTeam", var_4 );
...
supplydropprice( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    scripts\mp\gametypes\br_analytics::civ_death( var_0, var_1 );
    ...
    if ( !istrue( level.recently_spawned_vehicle ) )
    {
        if ( !istrue( var_3 ) && var_2 < 5 )
            scripts\mp\gametypes\br_public::brleaderdialog( "top_5_lose", 0, var_9, 1 );
        else if ( !istrue( var_3 ) && var_2 < 10 )
            scripts\mp\gametypes\br_public::brleaderdialog( "top_10_lose", 0, var_9, 1 );
        else if ( !istrue( var_3 ) && var_2 < 25 )
            scripts\mp\gametypes\br_public::brleaderdialog( "top_25_lose", 0, var_9, 1 );

        scripts\mp\gametypes\br_public::brleaderdialog( "team_loss", 0, var_9, 1 );
    }
...
nosplash( var_0 )
{
    ...
    if ( !istrue( var_1.br_spectatorinitialized ) && !var_1 scripts\mp\gametypes\br_public::update_timer_for_bomb_vest_detonator_holder() )
...
monitor_dropmenu( var_0, var_1, var_2 )
{
    ...
    if ( var_0 > 0 )
    {
        ...
        scripts\mp\gametypes\br_analytics::chopperexfil_sitting_wind( var_0, var_2 );
    }
...
ononeleftevent( var_0 )
{
    ...
    if ( level.teambased )
    {
        ...
        if ( isdefined( var_1 ) )
        {
            if ( istrue( scripts\mp\gametypes\br_gametypes::_id_11BE4( "disableLastManStandingDialog", var_1 ) ) )
                return;
...
spawn_smoke_bombers()
{
    if ( isdefined( level.forcedend ) )
        return scripts\mp\gametypes\br_public::processvoqueue() && level.forcedend;
...
brendgame( var_0, var_1 )
{
    ...
    scripts\mp\gametypes\br_vehicles::emptyallvehicles();
    thread scripts\mp\gametypes\br_gametypes::_id_11BE4( "endGame", var_0 );
    ...
    if ( isdefined( var_0 ) && var_0 != "tie" )
    {
        ...
        level scripts\engine\utility::delaythread( 1.0, scripts\mp\gametypes\br_challenges::set_strict_ff, var_0 );
        var_4 = 1;

        if ( scripts\mp\gametypes\br_public::prophasclonesleft() && scripts\engine\utility::is_equal( var_1, 25 ) )
            var_4 = 0;
        ...
        scripts\mp\gametypes\br_analytics::civ_death( var_0, 1 );
        ...
        foreach ( var_10 in var_3 )
        {
            ...
            var_10 scripts\mp\gametypes\br_pickups::empty_vo_func();
            ...
            scripts\mp\gametypes\br_analytics::choppersupport_issmallpatrolmap( var_10, "player_win" );
            var_10 _id_11ED8( 1 );
            ...
            if ( scripts\mp\gametypes\br_public::processvoqueue() && !isbot( var_10 ) )
    ...
    if ( isdefined( var_3 ) )
        var_34 = scripts\mp\gametypes\br_ending::get_center_of_array( var_3 );

    var_35 = scripts\mp\gametypes\br_quest_util::getvalidplayersinarray( level.players, level.questinfo.defaultfilter );
    scripts\mp\gametypes\br_analytics::choppergunner_waitintroanimtime( var_35 );
    level thread scripts\mp\gametypes\br_analytics::choppergunner_watchexplosioninfluencepoint();
    wait 1.5;
    ...
    if ( isdefined( var_3 ) && isdefined( var_34 ) )
        scripts\mp\gametypes\br_ending::trial_enemy_dont_drop_weapon( var_3, var_34 );
...
brdpadcallback( var_0, var_1 )
{
    ...
    if ( isdefined( var_0 ) )
    {
        ...
        {
            case "dpad_slot_down":
                if ( scripts\mp\gametypes\br_public::prophasclonesleft() && var_1 == 0 )
                {
                ...
                thread scripts\mp\gametypes\br_pickups::watch_players_entering_super( var_1 );
                break;
            case "dpad_slot_up":
            ...
            case "try_use_heal_slot":
                var_2 = var_1;
                scripts\mp\gametypes\br_pickups::vehicle_collision_ignorefuturemultievent( var_2 );
                break;
            case "br_drop_all":
                if ( scripts\mp\gametypes\br_public::prophasclonesleft() && var_1 == 0 )
                {
                ...
                scripts\mp\gametypes\br_pickups::watch_for_players_touching_ground( var_1 );
                break;
            default:
                break;
        }
...
parachutecomplete()
{
    ...
    {
        if ( scripts\mp\gametypes\br_public::questrewarduav() )
        {
    ...
    if ( scripts\mp\flags::gameflag( "prematch_done" ) )
        thread scripts\mp\gametypes\br_armory_kiosk::_id_1203E( 5 );
...
comparescore( var_0 )
{
    ...
    if ( !isdefined( self.playerstartjailsetcontrols ) )
    {
        ...
        var_2 = scripts\mp\gametypes\br_public::vandalize_attack_nodes( var_1 );
        var_3 = getdvarint( "scr_br_drop_prespawn_timeout_ms", 9000 );
        scripts\mp\gametypes\br_public::vandalize_internal( var_2, var_3, 1 );
    ...
    foreach ( var_7 in var_5 )
    {
        if ( isdefined( level.br_pickups.br_equipnametoscriptable[var_7] ) )
        {
            ...
            if ( isdefined( var_4.init_chopper_boss_battle ) && var_4.init_chopper_boss_battle.type == var_8 )
                scripts\mp\gametypes\br_pickups::lootused( var_4.init_chopper_boss_battle, undefined, "visible", var_0, 1 );
            else if ( isdefined( var_4.init_civs ) && var_4.init_civs.type == var_8 )
                scripts\mp\gametypes\br_pickups::lootused( var_4.init_civs, undefined, "visible", var_0, 1 );
        }
    }
...
checkpoint_edit_munitions_icon_init( var_0, var_1 )
{
    ...
    foreach ( var_4 in var_0.equippedweapons )
    {
        ...
        if ( issameweapon( var_4 ) && var_4.inventorytype == "primary" )
        {
            ...
            {
                var_6 = var_0 scripts\mp\gametypes\br_extract_quest::initbrmechanics( var_4 );

                if ( istrue( var_1 ) && !var_6 )
                {
                    var_7 = scripts\mp\gametypes\br_pickups::getitemdroporiginandangles( var_2, var_0.origin, var_0.angles, var_0 );
                    var_8 = scripts\mp\gametypes\br_weapons::weaponspawn( var_4, var_0, var_7, 0, 1 );
                    ...
                    if ( isdefined( var_8 ) )
                    {
                        ...
                        if ( var_4.hasalternate )
                        {
                            ...
                            if ( !scripts\mp\gametypes\br_weapons::checkformatchend( var_4, var_12 ) )
                                var_11 = var_0 getweaponammoclip( var_12 );
                        }

                        scripts\mp\gametypes\br_pickups::select_back_two_spawners( var_8, var_9, var_10, var_11 );
    ...
    if ( isdefined( var_0.equipment["primary"] ) )
    {
        if ( istrue( var_1 ) )
        {
            ...
            {
                var_7 = scripts\mp\gametypes\br_pickups::getitemdroporiginandangles( var_2, var_0.origin, var_0.angles, var_0 );
                var_8 = scripts\mp\gametypes\br_pickups::spawnpickup( var_15, var_7, var_14, 1 );
        ...
        if ( istrue( var_1 ) )
        {
            ...
            if ( isdefined( var_15 ) && var_14 > 0 )
            {
                ...
                if ( isdefined( var_8 ) )
                {
                    ...
                    var_8 scripts\mp\gametypes\br_pickups::helicopter_firendly_dmg_text_display( var_0, var_8.type );
                }
            }
        }
...
br_givedropbagloadout( var_0 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        var_0 endon( "disconnect" );
        wait 0.5;
        scripts\mp\gametypes\br_analytics::choppergunner_refillmissiles( var_0, var_3, self );
        scripts\mp\gametypes\br_analytics::choppersupport_issmallpatrolmap( var_0, "dropbag_used" );
...
combatrecordequipmentused( var_0 )
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
        scripts\mp\gametypes\br_analytics::choppergunner_handledangerzone( self.team, self.origin );
}
...
{
    var_0 = self;
    var_1 = var_0 scripts\mp\gametypes\br_public::unset_relic_martyrdom();
...
{
    scripts\mp\gametypes\br_gulag::unset_just_keep_moving( var_0 );
}
...
shoulddropbrprimary( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8 )
{
    ...
    if ( scripts\mp\gametypes\br_gametypes::progress_obj( "lastStandAllowed" ) && !scripts\mp\gametypes\br_gametypes::_id_11BE4( "lastStandAllowed", var_9 ) )
        return 0;
...
cp_dwn_twn_2_sound_load( var_0 )
{
    ...
    if ( !scripts\mp\gametypes\br_gametypes::propsetclonesleft( "teamSpectate" ) )
        scripts\mp\gametypes\br_spectate::setup_teleport_rooms( var_0.victim, var_0.attacker, 1 );
}
...
{
    var_2 = scripts\mp\gametypes\br_public::unset_relic_martyrdom();
    ...
    if ( propcamerazoom( var_1 ) && !isdefined( var_2 ) )
    {
        ...
        if ( !scripts\mp\gametypes\br_gametypes::propsetclonesleft( "armor" ) )
            var_1 scripts\mp\gametypes\br_armor::cleanup_target_stats_thermal();
    }
}
...
clean_up_vandalize()
{
    if ( scripts\mp\gametypes\br_gametypes::propsetclonesleft( "drogBagLoadout" ) )
        return 0;
...
clear_and_give_killstreak_loadout_assault()
{
    ...
    var_1 = scripts\mp\gametypes\br_gametypes::lootleadermarksizedynamic( "dropBagDelay" );
    ...
    var_2 = -15;
    var_3 = scripts\mp\gametypes\br_circle::low_roof_enemy_watcher( 0 );
    var_4 = max( 0, var_3 + var_2 );
    var_5 = getdvarfloat( "scr_br_dropbag_delay", var_4 );
    var_0[var_0.size] = var_5;

    if ( getdvarint( "scr_br_dropbag2_enabled", 0 ) && !scripts\mp\gametypes\br_gametypes::propsetflashesleft( "dropBagLoop" ) )
    {
        var_6 = 15;
        var_7 = scripts\mp\gametypes\br_gulag::modeloadoutupdateammo();
...
{
    var_0 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "maySpawn" );
...
spawnclientbr( var_0 )
{
    ...
    else if ( istrue( self.waitingtospawnamortize ) || scripts\mp\gametypes\br_public::radiussq( self ) || istrue( scripts\mp\gametypes\br_gametypes::_id_11BE4( "spawnHandled", self ) ) )
    {
        self notify( "attempted_spawn" );
        self.waitingtospawn = 0;
    }
...
_id_11EDB( var_0 )
{
    ...
    if ( var_0 == var_1 scripts\mp\gametypes\br_public::questrewarduav() )
        return;
...
ai_goal_distribution( var_0, var_1 )
{
    ...
    if ( isdefined( var_2 ) && !var_2 scripts\mp\gametypes\br_public::questrewarduav() )
        return 0;

    if ( isdefined( var_2 ) )
        var_2.obj_overwatch_tanks_ref = 1;

    var_3 = scripts\mp\gametypes\br_public::minshotstostage3acc( var_0, var_2.squadindex );
    var_4 = 0;
    var_5 = undefined;
    ...
    if ( isdefined( var_5 ) )
    {
        ...
        if ( !istrue( level.checkpoint_add_carepackage ) )
            level scripts\mp\gametypes\br_public::cloud_cover( "deploy_squad_leader", var_5, 1, 0 );
    }
...
_id_12683( var_0, var_1 )
{
    ...
    {
        if ( !isdefined( var_5 ) || var_5 scripts\mp\gametypes\br_public::questrewarduav() )
            continue;
...
ui_init( var_0 )
{
    if ( !isdefined( var_0 ) || var_0 scripts\mp\gametypes\br_public::questrewarduav() || var_0 ismlgspectator() || var_0 isspectatingplayer() || !isalive( var_0 ) )
        return 0;
...
supply_crate_vo_when_used( var_0 )
{
    ...
    foreach ( var_6 in var_1 )
    {
        ...
        var_0 scripts\mp\gametypes\br_weapons::checkpoint_player_spawns_func( var_6, var_10 );
        var_6._id_11ABA = 1;
    }
...
{
    scripts\mp\gametypes\br_gametypes::_id_11BE4( "initCrateData", var_0 );
}

suncascademult2( var_0 )
{
    var_1 = var_0 getcurrentprimaryweapon();
    var_0 scripts\mp\gametypes\br_weapons::checkpoint_player_spawns_func( var_1 );
}

stunboltdelete( var_0, var_1 )
{
    var_0 scripts\mp\gametypes\br_pickups::nuke_core_time_warning_nag( var_1, "primary" );
}
...
usingtacmap()
{
    if ( scripts\mp\gametypes\br_gametypes::progress_obj( "playerShouldRespawn" ) )
        return scripts\mp\gametypes\br_gametypes::_id_11BE4( "playerShouldRespawn" );
...
{
    var_2 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "playerKilledSpawn", var_0, var_1 );
...
codenumber( var_0 )
{
    var_1 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "mayConsiderPlayerDead", var_0 );
...
bomb_count_down_end_time_stamp_ms()
{
    ...
    if ( scripts\mp\gametypes\br_gametypes::propsetclonesleft( "playerCountLandmarks" ) )
        return;
...
_id_12B2D( var_0, var_1, var_2, var_3 )
{
    if ( isdefined( var_0.pers["squadMemberIndex"] ) )
    {
        ...
        if ( istrue( var_3 ) )
            var_0 scripts\mp\gametypes\br_public::_id_11EE3( 0 );
        else if ( var_5 )
            var_0 scripts\mp\gametypes\br_public::_id_11EE3( 1 );
        ...
        var_14 = scripts\mp\gametypes\br_public::minshotstostage3acc( var_0.team, var_0.squadindex );
...
subtract_from_spawn_count_from_group()
{
    ...
    if ( scripts\mp\gametypes\br_public::processvoqueue() && !isbot( self ) )
    {
    ...
    var_0 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "onConnectSpawnPoint" );

    if ( !isdefined( var_0 ) )
        var_0 = getspawnpoint();

    self.playerstartjailsetcontrols = var_0;
    var_1 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "initialPrespawnZOffset" );
    ...
    self setclientomnvar( "ui_br_extended_load_screen", 1 );
    var_2 = scripts\mp\gametypes\br_public::vandalize_attack_nodes( var_0.origin );
    spawnintermission( var_2, var_0.angles );
    var_3 = getdvarint( "scr_br_initial_stream_timeout_ms", 15000 );
    scripts\mp\gametypes\br_public::vandalize_internal( var_2, var_3, 1, 1 );
    return 0;
}
...
{
    var_1 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "regenHealthAdd", var_0 );
...
{
    var_1 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "regenDelaySpeed", var_0 );
...
{
    scripts\mp\gametypes\br_gametypes::_id_11BE4( "addToTeamLives", var_0, var_1 );
}

collecteditems( var_0, var_1 )
{
    scripts\mp\gametypes\br_gametypes::_id_11BE4( "removeFromTeamLives", var_0, var_1 );
}
...
_id_12B17()
{
    ...
    for ( var_5 = 0; var_5 < var_0; var_5++ )
    {
        ...
        {
            var_7 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "isTeamEliminated", var_6.team );
...
chopper_lights()
{
    ...
    if ( level.teambased )
    {
        ...
        foreach ( var_12 in level.teamnamelist )
        {
            ...
            var_17 = 1;
            var_18 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "isTeamEliminated", var_12 );
    ...
    scripts\mp\gametypes\br_gametypes::_id_11BE4( "postUpdateGameEvents" );
}
...
checkpoint_carepackage_munitions_role_think( var_0 )
{
    ...
    if ( istrue( scripts\mp\gametypes\br_public::processvoqueue() ) )
        return 0;

    var_1 = scripts\mp\gametypes\br_gametypes::propsetflashesleft( "ignoreZombiesLastStandWipe" );
    var_2 = isdefined( level.gulag ) && !istrue( level.gulag.shutdown );
    var_3 = [];
    var_4 = 0;
    var_5 = [];
    ...
    foreach ( var_7 in level.teamnamelist )
    {
        var_8 = 1;
        var_3 = [];
        var_9 = scripts\mp\gametypes\br_public::minplunderextractions( var_7 );

        for ( var_10 = 0; var_10 < var_9.size; var_10++ )
        {
            var_11 = var_9[var_10];
            var_12 = scripts\mp\gametypes\br_public::minigunbackup( var_7, var_11 );

            foreach ( var_14 in var_12 )
            {
                var_15 = istrue( var_2 ) && var_14 scripts\mp\gametypes\br_public::questtypes();

                if ( var_15 )
                    continue;

                if ( var_1 && var_14 scripts\mp\gametypes\br_public::update_timer_for_bomb_vest_detonator_holder() )
                    continue;

                var_16 = istrue( var_14.inlaststand ) || isdefined( var_0 ) && var_0 == var_14;

                if ( var_16 && !istrue( var_14.obj_room_fire_06 ) && !var_14 scripts\mp\gametypes\br_public::update_timer_for_bomb_vest_detonator_holder() )
                {
...
_id_126A4( var_0, var_1, var_2 )
{
    ...
    {
        if ( isdefined( var_0 ) )
            obituary( var_0, var_1, var_2, "MOD_TEAM_WIPED", scripts\mp\gametypes\br_public::minigunbackup( var_1.team, var_1.squadindex ) );
    }
}
...
clients( var_0 )
{
    ...
    if ( !istrue( var_0._id_120AA ) && !var_1 scripts\mp\gametypes\br_pickups::gun_create_fake( var_0 ) )
        return 0;

    var_2 = scripts\mp\gametypes\br_gametypes::_id_11BE4( "onKillstreakBeginUseFunc", var_0 );
...
convoy_turret( var_0 )
{
    if ( !isdefined( var_0.operatorcustomization ) || !isdefined( var_0.operatorcustomization.voice ) || var_0 scripts\mp\gametypes\br_public::update_timer_for_bomb_vest_detonator_holder() )
        return 1;
