...
init()
{
    level._id_11AFA = getdvarint( "scr_br_reveal_event_type", 1 );
    ...
    scripts\mp\gametypes\br_gametypes::zombienumhitshuman( "playerDropPlunderOnDeath", ::playerdropplunderondeath );
    scripts\mp\gametypes\br_gametypes::zombienumhitshuman( "restartCircleElimination", ::_id_11AB7 );
    scripts\mp\gametypes\br_gametypes::zombienumhitshuman( "lootUsedIgnore", ::semtex_stuckplayer );
    ...
    scripts\mp\gametypes\br_gametypes::zombienumhitshuman( "playerNakedDropLoadout", ::_id_11ADF );
    ...
    scripts\mp\gametypes\br_gametypes::zombienumhitshuman( "modifyPlayerDamage", ::modifyplayerdamage );
    ...
    scripts\mp\gametypes\br_gametypes::zombienumhitshuman( "playerGetZombieSpawnLocation", ::unset_relic_vampire );
    ...
    scripts\mp\gametypes\br_gametypes::zombienumhitshuman( "postUpdateGameEvents", scripts\mp\gametypes\br_alt_mode_zxp::vfx_time );
    ...
    scripts\mp\gametypes\br_gametypes::zombienumhitshuman( "shouldLastStandDamageScale", scripts\mp\gametypes\br_gametype_zxp::_id_12006 );
    ...
    level.cleanupswaploadoutflags._id_11AA0 = getdvarint( "scr_br_reveal_respawnDelay", 0 );
    level.cleanupswaploadoutflags.timelimit = getdvarint( "scr_br_timelimit", 900 );
    level.cleanupswaploadoutflags.isattachmentgrenadelauncher = makeextractionobjective();
    ...
    level._id_12721 = getdvarint( "scr_br_timedSection1Duration", 420 );
    level._id_12723 = getdvarint( "scr_br_timedSection2Duration", 70 );
    level._id_12725 = getdvarint( "scr_br_timedSection3Duration", 19 );
    level._id_11AEF = [ "shipwreck", "prison", "hospital", "stadium", "tvstation", "superstore", "dam", "trainstation", "storagetown", "quarry", "promwest", "promeast", "downtown", "farms", "lumber", "port", "park", "hills", "boneyard", "airport", "milbase" ];
    level._id_11AFD = undefined;
    level._id_11AEB = [];
    level._id_11AF7 = undefined;
    ...
    if ( level.script == "mp_br_mechanics" )
        level.dropoff = ( 475, -1956, 0 );
    else
        level.dropoff = ( -20830, 46010, -654 );
    ...
    level.keycardlocs = [ "apc_russian", "atv", "big_bird", "cargo_truck", "cargo_truck_mg", "cop_car", "hoopty", "hoopty_truck", "jeep", "large_transport", "light_tank", "little_bird", "little_bird_mg", "medium_transport", "pickup_truck", "tac_rover", "technical", "van" ];
...
playerzombiedelayturnonfx()
{
    ...
    level thread isspawningstopped();

    if ( getdvarint( "scr_br_reveal_give_xp_to_humans", 0 ) )
        level thread monitorcontrolscallback();

    level thread chopperexfil_music_sh025();
    level thread _id_11AF1();
}
...
_id_11AFE()
{
    ...
    _id_11AF0();
    thread _id_12720();
    _id_11AFF( level._id_12721 );
    thread _id_12722();
    _id_11AFF( level._id_12723 );
    thread _id_12724();
    level notify( "watchFinalRunToExfil" );
    _id_11AFF( level._id_12725 );
    _id_11AE8( 1 );
    thread _id_11AE5();
}
...
_id_12720()
{
    ...
    thread _id_11AE2( 660 );
    thread _id_11B04( 6, 4, 0, 1 );
    thread playerzombieapplyemp();

    foreach ( var_1 in level.players )
    {
        var_1 scripts\mp\hud_message::showsplash( "br_dov_survive" );

        if ( scripts\mp\utility\player::isreallyalive( var_1 ) && !var_1 scripts\mp\gametypes\br_public::update_timer_for_bomb_vest_detonator_holder() )
            var_1 setclientomnvar( "ui_br_reveal_state", 1 );
    }

    level thread _id_11AE7();

    if ( level._id_12721 <= 45 )
        var_3 = 5;
    else
        var_3 = 45;
    ...
    foreach ( var_1 in level.players )
        var_1 _id_11B02( "op2_dov1_infil_3" );

    scripts\mp\gametypes\br_containmentprotocol::colorise_warnings( 5 );
    wait 6;
    thread _id_11BE1();
    scripts\mp\gametypes\br_publicevent_juggernaut::all_gas_trap_structs();
    wait 10;
    ...
    wait( level._id_12721 - var_3 - 93 - 70 );
...
_id_12722()
{
    ...
    foreach ( var_1 in level.players )
        var_1 _id_11B02( "ebr_alert_phase4_80" );
    ...
    foreach ( var_1 in level.players )
        var_1 _id_11B02( "op2_dov1_infil_2_10" );

    scripts\mp\gametypes\br_containmentprotocol::colorise_warnings( 4 );
    wait 6;

    if ( getdvarint( "scr_br_reveal_bombardement_enabled", 1 ) )
    {
        level _calloutmarkerping_handleluinotify_acknowledged::br_pickupdenyweaponpickupap( level.dropoff, undefined, 8, 7000 );
        wait 4;

        foreach ( var_1 in level.players )
            var_1 _id_11B02( "ebr_alert_missile_20" );
    ...
    if ( getdvarint( "scr_br_reveal_exfil_helicopter", 1 ) )
    {
        ...
        foreach ( var_1 in level.players )
            var_1 _id_11B02( "dx_bra_cp1_plague_zone_chopper_inbound", 1 );
    }
    ...
    scripts\mp\gametypes\br_containmentprotocol::colorise_warnings( 3.5 );
}
...
_id_12724()
{
    ...
    level notify( "end_containment_fx" );
    level._id_11AFD = scripts\engine\utility::play_loopsound_in_space( "iw8_nuke_alarm_lp", level.dropoff + ( 0, 0, 500 ) );
    setmusicstate( "dovp1_nuke_countdown" );

    foreach ( var_1 in level.players )
        var_1 setsoundsubmix( "mp_br_event_dovp1_nuke", 6 );

    _id_12051( level._id_12725 );
    level.stack_patch_thread = 1;
    wait 1;

    foreach ( var_1 in level.players )
    {
        if ( !var_1 scripts\mp\gametypes\br_public::update_timer_for_bomb_vest_detonator_holder() )
            scripts\mp\gametypes\br_public::cloud_cover( "plague_no_exfil", var_1, 0 );
    }
    ...
    foreach ( var_1 in level.players )
        var_1 _id_11B02( "op1_dov1_infil_2" );

    scripts\mp\gametypes\br_containmentprotocol::colorise_warnings( 14 );
    wait 6;

    foreach ( var_1 in level.players )
        var_1 _id_11B02( "ebr_alert_phase4_70" );

    wait 1;
    fast_revive_time();

    foreach ( var_1 in level.players )
        var_1 _id_11B02( "op2_dov1_infil_10" );

    wait 4;

    foreach ( var_1 in level.players )
    {
        var_1 _id_11B02( "ebr_alert_phase4_90" );
    ...
    scripts\mp\gametypes\br_containmentprotocol::colorise_toggle_onto();
}
...
isspawningstopped()
{
    ...
    while ( var_0 )
    {
        ...
        {
            if ( scripts\mp\gametypes\br_alt_mode_zxp::_id_119D9( var_3 ) > 0 )
            {
                var_0 = 1;
                break;
            }
...
_id_11AE0()
{
    foreach ( var_1 in level.players )
    {
        var_1 scripts\cp\helicopter\chopper_boss::support_box_use_logic( "event_dov_completed" );
        waitframe();
        scripts\mp\gametypes\br::_id_12AED( var_1 );
        waitframe();
    }
}

_id_11B04( var_0, var_1, var_2, var_3 )
{
    level endon( "endRevealTimeHandler" );
    level endon( "forceFinalRunToExfil" );
    ...
    for ( var_4 = 1; var_4 <= 4; var_4++ )
    {
        ...
        _id_11AF5( "mp_run_" + var_4, 4 );
        _id_11AF3( "mp_run_" + var_4, var_3, "zombie_outbreak" );
        _id_11B03( var_0 );
    }
}
...
_id_11AE8( var_0 )
{
    ...
    if ( isdefined( level._id_11AFD ) )
        level._id_11AFD delete();

    thread scripts\mp\gametypes\br_vehicles::emptyallvehicles();

    if ( isdefined( level._id_11AEB ) )
    {
        foreach ( var_4 in level._id_11AEB )
            var_4 destroy();
    ...
    {
        if ( var_7 scripts\mp\gametypes\br_public::update_timer_for_bomb_vest_detonator_holder() )
    ...
    _id_11AF5( var_1, 9 );
    level notify( "end_containment_fx" );
    _id_11AF3( var_1, var_0, "final_nuke" );
    wait( var_2 - 2 );
}

_id_11AF5( var_0, var_1 )
{
...
_id_11ADF()
{
    ...
    level.chopper_gunner = _id_11ADE();
    var_0 scripts\mp\class::loadout_emptycacheofloadout( "gamemode" );
    var_0 scripts\mp\gametypes\br::monitor_traversal_watcher( 0, 1, 0 );
    scripts\mp\gametypes\br_pickups::br_forcegivecustompickupitem( var_0, "brloot_equip_gasmask", 1 );
    var_0 scripts\mp\gametypes\br_weapons::checkforsubgametypeoverrides();
    var_0 scripts\mp\gametypes\br_pickups::_id_11A84();
    var_0 scripts\mp\gametypes\br_pickups::forcegivesuper( "super_ammo_drop", 0 );
    var_0 scripts\mp\gametypes\br::monitor_game_end_on_front_truck_death( var_0 );
...
_id_11AF1()
{
    ...
    level.register_vfx[level.register_vfx.size] = var_0;
    var_1 = spawnstruct();
    var_1.id = 1;
    var_1.occupied = 0;
    var_1.origin = ( -28977, 23076, -390 );
    level.register_vfx[level.register_vfx.size] = var_1;
    var_2 = spawnstruct();
    var_2.id = 1;
    var_2.occupied = 0;
...
_id_11AE4( var_0 )
{
    level endon( "game_ended" );
    wait( var_0 );
    level.wait_for_player_gulag_respawn = scripts\mp\gametypes\br::comparescore;
    level.shouldmodelognotify = scripts\mp\gametypes\br::codenumber;
    level.modeonspawnplayer = scripts\mp\gametypes\br::onspawnplayer;
    level.cleanupswaploadoutflags.funcs["spawnHandled"] = undefined;
    level.cleanupswaploadoutflags.funcs["playerKilledSpawn"] = undefined;
}
...
_id_11AE5()
{
    ...
    scripts\mp\gametypes\br_alt_mode_zxp::phfrozen();
}
...
_id_11AF0()
{
    ...
    scripts\mp\gametypes\br_circle::add_rider_to_decho();
...
monitor_flag_carrier()
{
    ...
    var_0 scripts\mp\class::loadout_emptycacheofloadout( "gamemode" );
    var_0 scripts\mp\gametypes\br_pickups::_id_11A84();
    var_0.pers["gamemodeLoadout"] = level.cleanupswaploadoutflags.isattachmentgrenadelauncher;
...
_id_11AE6()
{
    var_0 = spawn( "script_model", level.dropoff - ( 0, 0, 200 ) );
    ...
    _id_11AF7();
    var_4 = spawnfx( level._effect["smoke_exfil"], level.dropoff );
    triggerfx( var_4 );
}
...
_id_11AF7()
{
    ...
    if ( var_0 != -1 )
    {
        scripts\mp\objidpoolmanager::objective_add_objective( var_0, "current", level.dropoff + ( 0, 0, 600 ), "ui_mp_br_mapmenu_icon_dov_1_objective" );
...
_id_11AF8()
{
    ...
    if ( !isdefined( level._id_11AF7 ) )
        return;

    scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level._id_11AF7, self );
}

_id_11B02( var_0, var_1 )
{
    var_2 = self;
...
setincomingcallback( var_0 )
{
    ...
    return scripts\mp\gametypes\br_alt_mode_zxp::setincomingcallback( var_0, var_1 );
}
...
comparescore( var_0 )
{
    ...
    if ( !isdefined( self.playerstartjailsetcontrols ) && !( isdefined( level._id_11AFA ) && level._id_11AFA == 2 ) )
    {
        var_2 = level.cleanupswaploadoutflags._id_11AA0 > 0;
        var_3 = undefined;

        if ( var_2 )
        {
            var_3 = level.cleanupswaploadoutflags._id_11AA0 * 1000;
            scripts\mp\gametypes\br::spawnintermission( self.origin + ( 0, 0, 100 ), self.angles );
            scripts\mp\spectating::setdisabled();
        }

        var_4 = scripts\mp\gametypes\br_public::vandalize_attack_nodes( self.wait_display_pavelow_boss_health_bar.origin, self.wait_display_pavelow_boss_health_bar.height );
        var_5 = getdvarint( "scr_br_drop_prespawn_timeout_ms", 9000 );
        scripts\mp\gametypes\br_public::vandalize_internal( var_4, var_5, 1, 0, var_3 );
        ...
        if ( var_2 )
        {
            ...
            var_10 = max( level.cleanupswaploadoutflags._id_11AA0 - var_6, 0 );
            wait( var_10 );
            thread scripts\mp\gametypes\br_gulag::fadeoutin( var_7 );
            wait( var_9 );
            scripts\mp\gametypes\br::spawnintermission( var_4, self.wait_display_pavelow_boss_health_bar.angles );
            scripts\mp\gametypes\br::convoy_left_gulag_monitor();
            self setclientomnvar( "ui_br_transition_type", 2 );
            wait( var_8 );
            var_11 = max( level.cleanupswaploadoutflags._id_11AA0 - var_10 - var_7, 0 );
            wait( var_11 );
...
unset_relic_vampire()
{
    [var_1, var_2] = unset_slow_healthregen();

    if ( !isdefined( var_1 ) )
        [var_1, var_2] = unset_stay_at_spawn_flag_on_entering_combat();

    if ( isdefined( var_1 ) )
        return [ var_1, var_2 ];
    else
        return scripts\mp\gametypes\br_alt_mode_zxp::unset_relic_vampire();
}
...
unset_slow_healthregen()
{
    ...
    [var_0, var_1] = scripts\mp\gametypes\br_alt_mode_zxp::vehicle_damage_applytabletovehicle();
    return [ var_0, var_1 ];
}

unset_stay_at_spawn_flag_on_entering_combat()
{
    var_0 = undefined;
    var_1 = undefined;
    var_2 = 50;
    ...
    for ( var_14 = 0; var_14 < var_13.size; var_14++ )
    {
        ...
        [var_0, var_1] = scripts\mp\gametypes\br_alt_mode_zxp::modetype( var_5, var_15, var_6 );

        if ( isdefined( var_0 ) )
        {
            var_0 = scripts\mp\gametypes\br_public::helihint_deposit( var_0, var_3 );
            break;
        }
    }
...
_id_11F94()
{
    ...
    if ( level.cleanupswaploadoutflags.timelimit <= 0 )
        return;

    setomnvar( "ui_hardpoint_timer", gettime() + int( level.cleanupswaploadoutflags.timelimit * 1000 ) );
    var_1 = max( level.cleanupswaploadoutflags.timelimit - var_0, 0 );
    wait( var_1 );
    setomnvar( "ui_br_circle_state", 6 );
}
...
ontimelimit()
{
    ...
    thread scripts\mp\gametypes\br::brendgame( level.cleanupswaploadoutflags.isassaultdrone, game["end_reason"]["objective_completed"] );
}
...
onplayerkilled( var_0 )
{
    ...
    if ( isdefined( var_0.victim ) && isplayer( var_0.victim ) && !istrue( level.stack_patch_thread ) )
    {
        var_0.victim setclientomnvar( "ui_br_reveal_state", 4 );
        var_0.victim scripts\mp\gametypes\br_containmentprotocol::colorise_warnings_clear();
    }
...
_id_11AB7( var_0, var_1 )
{
    ...
    level.shouldmodelognotify = scripts\mp\gametypes\br::codenumber;
    level.modeonspawnplayer = scripts\mp\gametypes\br::onspawnplayer;
    level.cleanupswaploadoutflags.dropped_weapon_cleanup = 1;
    level notify( "closeSandboxMenu" );

    foreach ( var_4 in level.players )
    {
        if ( isdefined( var_4.physicson ) )
            var_4.physicson destroy();
    }
...
playerzombieapplyemp()
{
    if ( level.script == "mp_br_mechanics" )
    {
        scripts\mp\gametypes\br_zones::player_respawn( "a", ( 0, 0, 0 ), 500, 20, 0, 2 );
        scripts\mp\gametypes\br_zones::player_respawn( "b", ( 1000, 1000, 0 ), 500, 20, 0, 2 );
        scripts\mp\gametypes\br_zones::player_respawn( "c", ( 1000, -1000, 0 ), 500, 20, 0, 2 );
        var_0 = "a,b,c";
    }
    else
        var_0 = "dam,stadium,hospital,super,tv,quarry,bank";

    scripts\mp\gametypes\br_zones::player_respawn_struct( "scr_br_reveal_plague_zone_locations", var_0 );
    thread triage_ai_campers();
}
...
triage_ai_campers()
{
    ...
    foreach ( var_2 in level.chopperexfil_sh050_start.zones )
    {
        if ( var_2.type == "plague" )
            var_2 thread scripts\mp\gametypes\br_zones::_id_1314F( undefined, 50.0, var_0 );
    }
    ...
    {
        if ( var_2.type == "plague" )
            var_2 scripts\mp\gametypes\br_zones::_id_13148();
    }
}
...
_id_11AE7()
{
    ...
    var_0 = [ level.dropoff + ( 500, 0, 0 ), level.dropoff - ( 500, 0, 0 ), level.dropoff + ( 350, 0, 500 ) - ( 0, 650, 0 ) ];

    for ( var_1 = 0; var_1 < var_0.size; var_1++ )
        level thread activeintelchallengekeys( var_0[var_1] );
}

activeintelchallengekeys( var_0 )
{
    while ( istrue( level.can_trex_apply_to_player ) )
        wait 1;

    var_1 = level.players[randomint( level.players.size )];
    var_2 = var_1 validautoassignquests( var_0 );
    ...
    if ( isdefined( var_2 ) )
    {
        ...
        if ( isdefined( var_5 ) )
        {
            var_2.onsoccerballreset = var_5;
            var_5 hide();
            var_2 helicreateextractvfx( var_0 );
            thread init_relic_squadlink( var_2 );
            return;
        }
    }
}

init_relic_squadlink( var_0 )
{
    var_0 endon( "death" );
    var_0 endon( "leaving" );
    var_0 setvehgoalpos( var_0.pathgoal, 1 );
    var_0 settargetyaw( var_0.mortars_set_player_targeted );
    ...
    var_0.isloadinggulag = damagestatedata( var_2 );
    var_0 waittill( "goal" );
    var_0 thread openscriptabledoors();
    var_0 thread onriskplayerkilled();
...
{
    var_1 = damagestateref( 30000, 150, 100 );
    var_2 = damagestateref( var_0, 37.5, 25.0 );
    var_3 = var_1 + var_2;
    return var_3;
}
...
modifyplayerdamage( var_0 )
{
    ...
    {
        var_1 = var_0.victim scripts\mp\gametypes\br_public::update_timer_for_bomb_vest_detonator_holder();
        var_2 = propclonepower( var_0 );
...
{
    if ( propclonepower( var_0 ) && isdefined( self.occupants ) && self.occupants.size )
        var_0.damage = 0;

    return scripts\mp\gametypes\br_alt_mode_zxp::showintelinstancetoplayer( var_0 );
}
...
_id_11BE1()
{
    ...
    var_0 = [ level.dropoff, ( -4903, 25796, -394 ), ( -28977, 23076, -390 ), ( 2693, 41717, 1607 ) ];
    var_1 = ( 0, 0, 0 );
    var_2 = 0;
    ...
    for (;;)
    {
        ...
        thread ammo_crate_use( var_4, var_5 );
        wait 0.1;
        var_2++;
    }
}
...
fast_revive_time()
{
    ...
    foreach ( var_1 in level.secondsbeforeplacementupdates )
        var_1 br_pe_chopper_crate_index( undefined );
}

ammo_crate_use( var_0, var_1 )
{
    ...
    var_2 = _id_12284( var_0 );
}

_id_12284( var_0 )
{
    var_1 = getdvarfloat( "scr_br_reveal_attack_heli_patrol_radius", 4000 );
    var_0 = scripts\mp\gametypes\br::maxelderrank( var_0, ( 0, 0, 10000 ) );
    ...
    var_5.temp_debug_wait_and_stop_music_loop = var_0;
    var_5.terminal_pusher_approach_array = var_1;
    var_5._id_123A8 = getdvarint( "scr_br_reveal_attack_heli_stage_1_acc", 60 );
    var_5.shot_to_miss_for_dialog = getdvarint( "scr_br_reveal_attack_heli_stage_2_shots", 10 );
    var_5._id_123A9 = getdvarint( "scr_br_reveal_attack_heli_stage_2_acc", 40 );
    var_5.should_ai_take_damage = getdvarint( "scr_br_reveal_attack_heli_stage_3_shots", 20 );
    var_5._id_123AA = getdvarint( "scr_br_reveal_attack_heli_stage_3_acc", 20 );

    if ( var_5._id_123AA <= 0 )
        var_5._id_123AA = undefined;

    var_5.fast_rope_over_black = var_5.heightoffset[2] - 250;
    ...
    var_5.frontturret._id_12D19 = "loot_chopper";
    ...
    var_5.rearturret._id_12D19 = "loot_chopper";
    ...
    level.secondsbeforeplacementupdates[level.secondsbeforeplacementupdates.size] = var_5;
    var_5.tell_player_to_pick_up_intel = scripts\mp\gametypes\br_lootchopper::select_lobby_door_one_spawners;
    var_5.enable_lbravo_player_infil = scripts\mp\gametypes\br_lootchopper::select_bunker_interior_groups;
    var_5.reading = ::reading;

    if ( getdvarint( "scr_br_reveal_attack_objective", 0 ) )
        var_5 scripts\mp\gametypes\br_lootchopper::select_bunker_interior_one_spawners();

    var_5 thread scripts\cp_mp\killstreaks\chopper_support::choppersupport_neargoalsettings();
    var_5 thread scripts\cp_mp\killstreaks\chopper_support::checkpoint_carepackage_munitions_think( var_5.pathgoal, 1 );
    return var_5;
}
...
br_ending_delay()
{
    ...
    scripts\mp\vehicles\damage::set_pre_mod_damage_callback( self.vehiclename, ::br_pe_chopper_damage_time );
    scripts\mp\vehicles\damage::set_post_mod_damage_callback( self.vehiclename, ::br_pe_chopper_crates );
    scripts\mp\vehicles\damage::set_death_callback( self.vehiclename, ::br_pe_chopper_crate_index );
