...
setupobjective( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.ninetypercent_music ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "hq" )
            var_4 thread _id_12E30( var_4 );
        else if ( scripts\mp\utility\game::getgametype() == "grnd" )
            var_4 thread _id_12E2F( var_4 );
    }
...
_id_12E30( var_0 )
{
    ...
    playfx( level.pindia_headlights["ghostcat_hw"], var_3 + var_2 );
}

_id_12E2F( var_0 )
{
    scripts\mp\flags::gameflagwait( "prematch_done" );
    playfx( level.pindia_headlights["blood_floor_hw"], getgroundposition( var_0.trigger.origin, 4 ) + ( 0, 0, 2 ) );
}
...
assignchevrons( var_0, var_1 )
{
    ...
    var_2 = _id_11A18( var_2, var_1 );
    var_2 = vfx_htown_stab_blink_1( var_2, var_1 );
    var_3 = [];
...
activatezone()
{
    ...
    if ( isdefined( level.manual_turret_laststandwatcher ) )
        var_0 = [[ level.manual_turret_laststandwatcher ]]();

    var_1 = 1024;

    if ( isdefined( level.manual_turret_munitionused ) )
        var_1 = [[ level.manual_turret_munitionused ]]();
    ...
    self.get_ai_within_range = var_2;
...
deactivatezone()
{
    ...
    foreach ( var_1 in self.get_ai_within_range )
        scripts\mp\spawnlogic::removespawndangerzone( var_1 );

    self.get_ai_within_range = undefined;
...
scriptable_callback( var_0, var_1 )
{
    level endon( "game_ended" );
...
cleanupgametypevips()
{
    level scripts\engine\utility::_id_12E3F( "game_ended", "zone_moved" );
...
zone_onuse( var_0 )
{
    ...
    if ( level.usehqrules && !level.usehprules && self.ownerteam != "neutral" )
    {
        ...
        if ( isdefined( var_3 ) )
        {
            ...
            foreach ( var_11 in var_9 )
            {
                ...
                var_7 scripts\cp\helicopter\chopper_boss::stop_module_on_delay();
            }
        }
    ...
    else
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "hq" && !level.usehprules )
        {
            ...
            animscripted_single_relative( var_1 );
        }
        else if ( level.test_offset_positions )
    ...
    if ( istrue( level.ninetypercent_music ) )
        playfx( level.pindia_headlights["vanish_hw_fr"], level.zone.trigger.origin + ( 0, 0, 40 ) );
}
...
zone_onusebegin( var_0 )
{
    if ( scripts\mp\utility\game::getgametype() == "hq" && !level.usehprules )
    {
        ...
        if ( var_1 != "neutral" && self.claimteam != var_1 )
            animscripted_single_relative( var_1, 1 );
...
zone_onuseupdate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( var_4 == "neutral" && self.claimteam != var_4 )
        animscripted_single_relative( self.claimteam );
    else if ( var_4 != "neutral" && self.claimteam != var_4 )
        animscripted_single_relative( var_4, 1 );
    else if ( level.test_offset_positions )
        level scripts\mp\gamelogic::pausetimer();
...
zone_onuseend( var_0, var_1, var_2 )
{
    ...
    if ( !var_2 )
    {
        if ( scripts\mp\utility\game::getgametype() == "hq" )
        {
            if ( level.usehprules )
            {
                if ( level.test_offset_positions )
                    level scripts\mp\gamelogic::resumetimer();
            }
            else if ( var_3 != "neutral" )
                animscripted_single_relative( var_3 );
        }
        else if ( level.test_offset_positions )
            level scripts\mp\gamelogic::resumetimer();
    }
...
zone_onunoccupied()
{
    ...
    {
        level.zone scripts\mp\gameobjects::setobjectivestatusicons( level.icondefend, level.iconcapture );
        animscripted_single_relative( self.ownerteam );
    }
    else
    {
        level notify( "zone_destroyed" );
...
zone_oncontested()
{
    ...
    level.zone thread scripts\common\utility::_id_12965( level.set_trap_flag, "hill_contested", level.zone.trigger.origin );
    self.didstatusnotify = 1;
}
...
zone_onuncontested( var_0 )
{
    ...
    if ( var_0 == "none" || var_1 == "neutral" )
    {
        ...
        level.zone thread scripts\common\utility::_id_12965( level.set_trap_flag, "hill_empty", level.zone.trigger.origin );
    }
    else
    {
        if ( scripts\mp\utility\game::getgametype() == "koth" )
        ...
        level.zone thread scripts\common\utility::_id_12965( level.set_trap_flag, "hill_uncontested", level.zone.trigger.origin );
    }

    var_7 = ( gettime() - self.extraction_helicoptor_total_plunder ) * 0.001;
    scripts\mp\utility\game::scriptable_token_scriptable_touched_callback( undefined, undefined, "Zone Contested", level.zone.trigger.origin, var_7 + " seconds" );
...
showcapturedhardpointeffecttoplayer( var_0, var_1 )
{
    ...
    if ( var_3 )
    {
        if ( var_1 ismlgfreecamenabled() || var_1 ismlgaerialcamenabled() )
            var_2 = "allies";
        else
...
_id_12F4B()
{
    ...
    foreach ( var_2 in level.teamnamelist )
    {
        self.juggernaut_update_hint_logic[var_2] = 0;
        self.juggernautchallengedone[var_2] = [];
    }

    self.juggernaut_update_hint_logic["neutral"] = 0;
    self.juggernautchallengedone["neutral"] = [];
    self.juggernautchallengedone["none"] = [];
    self.dismount_after_accum_damage = 1;
    ...
    for (;;)
    {
        ...
        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
        {
            ...
            self.juggernaut_update_hint_logic[var_2]++;
            var_6 = var_4.guid;
            var_7 = spawnstruct();
            var_7.player = var_4;
            var_7.starttime = gettime();
            self.juggernautchallengedone[var_2][var_6] = var_7;
            var_4.perkpackage_resetoverridefieldupgrades = 0;
        }

        if ( scripts\mp\utility\player::isreallyalive( var_4 ) && !isdefined( var_4.touchtriggers[var_0] ) )
            var_4 thread juggernautoutsidegoalradius( self );
    }
}
...
juggernautoutsidegoalradius( var_0 )
{
    ...
    if ( isdefined( self ) )
    {
        var_0.juggernautchallengedone[var_1][self.guid] = undefined;
        self.perkpackage_resetoverridefieldupgrades = 0;
    }
    else
    {
        var_2 = [];

        foreach ( var_5, var_4 in var_0.juggernautchallengedone[var_1] )
        {
        ...
        foreach ( var_5 in var_2 )
            var_0.juggernautchallengedone[var_1][var_5] = undefined;
    }

    var_0.juggernaut_update_hint_logic[var_1]--;
}
