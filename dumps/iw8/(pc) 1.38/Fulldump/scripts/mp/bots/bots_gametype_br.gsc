...
main()
{
    if ( scripts\mp\utility\game::missions_clearinappropriaterewards() == "dmz" && scripts\mp\gametypes\br_public::prophaschangesleft() )
...
bot_br_think()
{
    ...
    for (;;)
    {
        if ( scripts\mp\gametypes\br_public::processvoqueue() && !scripts\mp\flags::gameflag( "graceperiod_done" ) )
        {
            self.ignoreall = 0;
            wait 0.05;
            continue;
        ...
        {
            if ( scripts\mp\gametypes\br_public::processvoqueue() && !isdefined( self.play_sparks_power ) )
            {
                self.ignoreall = 1;
            ...
            if ( ( scripts\mp\gametypes\br_public::questrewarduav() || var_0 ) && istrue( level.c130inbounds ) )
            {
        ...
        if ( isdefined( level.br_circle ) && isscriptabledefined() )
        {
            ...
            {
                if ( ( istrue( self isskydiving() ) || istrue( self isparachuting() ) ) && istrue( self.registerpublicevent ) && istrue( scripts\mp\gametypes\br_public::processvoqueue() ) )
                {
...
changing_loadout()
{
    ...
    {
        if ( level.br_circle.circleindex > 0 && istrue( level.dropminigunondeath ) && !scripts\mp\gametypes\br_circle::ispointincurrentsafecircle( self.gethillspawnshutofforigin ) )
            self.gethillspawnshutofforigin = cashleaderbag_attach();
...
lightsshot02()
{
    ...
    if ( istrue( self.inlaststand ) || scripts\mp\gametypes\br_public::questtypes() )
        return undefined;

    if ( !isdefined( var_0 ) || istrue( var_0.inlaststand ) || !isalive( var_0 ) || var_0 scripts\mp\gametypes\br_public::questtypes() )
        return undefined;
