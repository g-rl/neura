...
init()
{
    if ( !isdefined( level.give_player_juggernaut ) )
        level.give_player_juggernaut = getdvarint( "scr_game_disablespawncamera", 0 ) == 1;
...
startspawncamera( var_0, var_1, var_2 )
{
    ...
    scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::startSpawnCamera() START" );
    scripts\mp\utility\player::hideminimap( 1 );
    forceabshotfoot();

    if ( !isdefined( self.spawncameraent ) )
    {
        scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::startSpawnCamera() UNDEFINED SPAWNCAMERAENT!!!" );
    ...
    scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::startSpawnCamera() CameraLinkTo()" );
...
getspawncamera()
{
    ...
    if ( isdefined( self.forcedspawncameraref ) )
    {
        ...
        else if ( issubstr( self.forcedspawncameraref, "squad" ) )
        {
            ...
            if ( isdefined( var_3 ) )
            {
                ...
                var_5 = var_5 + scripts\mp\gametypes\arm::calculatecameraoffset( self.team, var_3.origin );
                var_0 = spawnstruct();
                var_0.origin = var_5;
                var_0.angles = var_7;
            }
        ...
        else if ( issubstr( self.forcedspawncameraref, "vehicle" ) )
        {
            ...
            if ( isdefined( var_9 ) && !istrue( var_9.isdestroyed ) )
            {
                ...
                else
                {
                    ...
                    var_5 = var_5 + scripts\mp\gametypes\arm::calculatecameraoffset( self.team, var_11 );
                }
    ...
    else if ( istrue( level.evaluatespawnforcameraselection ) && isdefined( level.respawndelay ) && isdefined( level.respawndelay[self.team] ) )
    {
        var_14 = level.respawndelay[self.team];
        var_15 = undefined;
        var_16 = undefined;
...
snaptospawncamera()
{
    ...
    while ( !istrue( self.delay_notify_player_target_hit ) )
        waitframe();
    ...
    scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::snapToSpawnCamera()" );
...
{
    self endon( "disconnect" );
    scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::runSlamZoomOnSpawn() START" );
    self waittill( "spawncamera_start" );
    scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::runSlamZoomOnSpawn() spawncamera_start PASSED" );
    ...
    scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::runSlamZoomOnSpawn() CameraLinkTo()" );
    handlemovetoblended();
    deletespawncamera();
    ...
    scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::runSlamZoomOnSpawn() COMPLETE" );
}
...
handlemovetoblended( var_0 )
{
    ...
    scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::handleMoveTo() START" );
    ...
    if ( !var_3 || !var_4 )
    {
        ...
        scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::handleMoveTo() FULL Blend Set" );
    ...
    else
    {
        ...
        scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::handleMoveTo() FIRST HALF Blend Set" );
        ...
        scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::handleMoveTo() SECOND HALF Blend Set" );
        wait 0.3;
        wait 0.2;
    }

    scripts\mp\utility\player::wait_spawn_final_juggernaut( "spawnCamera::handleMoveTo() COMPLETE" );
...
fadeblackforgeo( var_0 )
{
    ...
    if ( istrue( self.spawningintovehicle ) || scripts\cp_mp\utility\game_utility::isnightmap() )
    {
        ...
        if ( isdefined( self._id_12D22 ) && self._id_12D22 == "light_tank" )
        {
...
{
    self endon( "disconnect" );
    self.delay_notify_player_target_hit = 0;
    ...
    self.delay_notify_player_target_hit = 1;
...
startoperatorsound()
{
    ...
    self.tac_cover_adjust_damage = spawn( "script_origin", ( 0, 0, 0 ) );
    self.tac_cover_adjust_damage showonlytoplayer( self );
    ...
    self.tac_cover_blocked_by_turret = var_1;

    if ( scripts\mp\utility\game::missions_clearinappropriaterewards() != "payload" )
        self.tac_cover_adjust_damage playloopsound( var_1 );

    thread hvt_can_lose_health();
}

hvt_can_lose_health()
{
    self endon( "spawned_player" );
    var_0 = self.tac_cover_adjust_damage;
    var_1 = self.tac_cover_blocked_by_turret;
    self waittill( "disconnect" );
