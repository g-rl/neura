...
push_players_back_and_deal_damage( var_0 )
{
    return var_0 >= 100;
}
...
getclasschoice( var_0 )
{
    ...
    if ( isdefined( level.munition_source_getridof ) && level.munition_source_getridof > 0 )
    {
        if ( scripts\mp\utility\game::matchmakinggame() )
...
executeclasschange( var_0, var_1 )
{
    ...
    else
    {
        ...
        var_3 = 1;
        var_4 = scripts\mp\gamelogic::defend_wave_4( scripts\mp\utility\game::missions_clearinappropriaterewards() );
...
handleclassedit( var_0 )
{
    ...
    var_3 = scripts\mp\class::scn_infil_hackney_heli_npc1();
    var_4 = var_2 || var_3;
...
autoassign()
{
    if ( scripts\mp\utility\game::getgametype() == "infect" )
        ...
        thread setteam( "allies" );
    else if ( binoculars_onstatemarkedupdate() )
        thread setteam( "allies" );
    else
    {
        ...
        if ( istrue( level.teammaxfill ) )
        {
            ...
            {
                if ( scripts\mp\teams::_id_11FEB() && var_1 == "team_two_hundred" )
                    continue;
        ...
        if ( var_5.size > 0 )
        {
            ...
            if ( istrue( level.branalytics_playercount ) )
                var_9 = 0;
...
setteam( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::missions_clearinappropriaterewards() != "dmz" && scripts\mp\utility\game::missions_clearinappropriaterewards() != "risk" && scripts\mp\utility\game::missions_clearinappropriaterewards() != "sandbox" && scripts\mp\utility\game::missions_clearinappropriaterewards() != "rumble" )
        {
            ...
            scripts\mp\utility\script::getdropbagspawntypeenum( "setTeam() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_1 );
        }
    ...
    if ( ( self.sessionstate == "spectator" || update_spot_limit() ) && !istrue( self.suicideswitched ) )
    {
...
update_spot_limit()
{
    return isdefined( level.update_spot_limit ) && self [[ level.update_spot_limit ]]();
}
...
leavesquad( var_0, var_1 )
{
    ...
    if ( isdefined( var_0 ) && isdefined( var_1 ) )
        thread scripts\mp\spawnselection::x1opsspawnnpc( var_0, var_1 );
...
binoculars_onstatemarkedupdate()
{
...
waitforclassselect()
{
    ...
    for (;;)
    {
        var_0 = scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::allowclasschoice() && ( !scripts\mp\flags::gameflag( "prematch_done" ) || istrue( level._id_120B6 ) );
        ...
        if ( var_0 || var_1 )
        {
            ...
            if ( !isai( self ) )
            {
                ...
                if ( var_3 && !scripts\mp\utility\game::propaddtocircle() )
...
beginclasschoice( var_0 )
{
    ...
    if ( scripts\mp\utility\game::allowclasschoice() || scripts\mp\utility\game::showfakeloadout() && !isai( self ) )
    {
        if ( getdvarint( "scr_force_cac_sre_callstack", 0 ) == 1 && scripts\mp\utility\game::getgametype() == "br" && scripts\mp\utility\game::missions_clearinappropriaterewards() != "dmz" && scripts\mp\utility\game::missions_clearinappropriaterewards() != "risk" && scripts\mp\utility\game::missions_clearinappropriaterewards() != "sandbox" && scripts\mp\utility\game::missions_clearinappropriaterewards() != "rumble" )
        {
            ...
            scripts\mp\utility\script::getdropbagspawntypeenum( "beginClassChoice() " + self.name + " ui_options_menu = 2, allowClassChoiceFunc = " + var_2 );
        }
...
bypassclasschoice()
{
    ...
    if ( race_monitor_oob() && level.enforceantiboosting && !isbot( self ) )
    {
...
addtoteam( var_0, var_1, var_2 )
{
    ...
    if ( var_0 == "spectator" || var_0 == "follower" )
    {
        ...
        scripts\cp_mp\pet_watch::strafe_minigun_manager_delta();
    }
    else
