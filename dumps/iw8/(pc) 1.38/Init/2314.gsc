...
supportbox_init()
{
    level.binoculars_onstatemarkpendingenter = [];
    scripts\common\interactive::interactive_addusedcallback( ::supportbox_usedcallback, "equip_supportBox" );
    scripts\common\interactive::interactive_addusedcallback( ::bot_fixup_pathnode_issues, "equip_armorBox" );
    scripts\mp\utility\join_team_aggregator::registeronplayerjointeamcallback( ::_id_125A7 );
...
_id_125A7( var_0 )
{
    foreach ( var_2 in level.binoculars_onstatemarkpendingenter )
    {
        if ( isdefined( var_2 ) )
            var_2 _id_125A5( var_0 );
    }
}

checked_missiles( var_0, var_1, var_2, var_3 )
{
    var_0 endon( "death" );
    var_0 thread supportbox_watchdisownedtimeout();
    var_0 thread supportbox_hideandshowaftertime();
    var_0 thread _id_125A8( self );
    ...
    level.binoculars_onstatemarkpendingenter[var_0 getentitynumber()] = var_0;
...
_id_125A8( var_0 )
{
    ...
    var_1 = scripts\engine\utility::_id_12E53( 2, "touching_platform" );
    ...
    if ( var_8["fraction"] < 1.0 )
    {
        ...
        if ( isdefined( var_2 ) )
        {
            if ( pressure_overload_threshold( var_2 ) )
...
{
    var_0 endon( "death" );
    var_0._id_12731 = "vfx/iw8_mp/equipment/vfx_offhand_wm_supportbox_timeout.vfx";
    checked_missiles( var_0, "equip_supportBox", "super_ammo_drop", "hud_icon_equipment_support_box" );
...
{
    var_0 endon( "death" );
    var_0._id_12731 = "vfx/iw8_mp/equipment/vfx_offhand_wm_armorbox_timeout.vfx";
    checked_missiles( var_0, "equip_armorBox", "super_armor_drop", "ui_mp_br_loot_icon_health_armor_box" );
...
supportbox_destroy( var_0 )
{
    ...
    if ( isdefined( self._id_12731 ) )
        var_1 = self._id_12731;
...
supportbox_delete( var_0, var_1, var_2 )
{
    ...
    if ( isdefined( self.equipmentref ) && self.equipmentref == "equip_armorBox" )
        self.owner scripts\cp\helicopter\chopper_boss::stoppingpower_onweaponcreated( "super_armor_drop", self.usedcount, var_0, var_2 );
    else
        self.owner scripts\cp\helicopter\chopper_boss::stoppingpower_onweaponcreated( "super_ammo_drop", self.usedcount, var_0, var_2 );
...
supportbox_playercanuse( var_0 )
{
    ...
    {
        if ( isdefined( self.owner ) && !scripts\engine\utility::array_contains( scripts\mp\gametypes\br_public.gsc::minshotstostage3acc( self.owner.team, self.owner.squadindex ), var_0 ) )
            return 0;
...
{
    self.occupied_rpg_trig = scripts\cp_mp\entityheadicons::setheadicon_singleimage( [], var_0, 20, 1, 1000, 100, undefined, 1 );
...
supportbox_givexpforuse( var_0 )
{
    if ( isdefined( self.owner ) && !scripts\cp_mp\utility\player_utility::playersareenemies( self.owner, var_0 ) )
    {
        if ( self.owner != var_0 )
        {
            ...
            self.owner scripts\cp\helicopter\chopper_boss::_id_11A49( "t9_ch_global_resupply_teammates_for_operator_mission", 1 );
        }

        self.owner scripts\mp\utility\stats::incpersstat( "munitionsBoxUsed", 1 );
        self.owner scripts\mp\supers::eventcallback( "super_ammo_drop" );
        self.usedcount++;
    }
}
...
supportbox_handlemovingplatforms( var_0 )
{
    ...
    if ( isdefined( var_0 ) && scripts\mp\equipment\claymore::pressure_overload_threshold( var_0 ) )
    {
        var_1.travelspeed = 1;
        self _meth_87B9( 1 );
    }
...
_id_125A5( var_0 )
{
    ...
    var_1 = self.occupied_rpg_trig;

    if ( !isdefined( var_1 ) )
        return;

    var_2 = self.oceanrock;

    if ( !isdefined( var_2 ) )
        return;

    if ( scripts\mp\utility\game::getgametype() == "br" && scripts\mp\menus::_id_1237A() && isdefined( self.owner ) )
        var_3 = scripts\engine\utility::array_contains( scripts\mp\gametypes\br_public.gsc::minshotstostage3acc( self.owner.team, self.owner.squadindex ), var_0 );
    else
    {
        if ( isdefined( self.owner ) )
    ...
    if ( var_3 && !var_5 )
    {
        scripts\cp_mp\entityheadicons::_id_11EAF( var_1, var_0 );
        scripts\cp_mp\entityheadicons::_id_11EB0( var_2, var_0 );
    }
    else if ( var_3 )
    {
        scripts\cp_mp\entityheadicons::_id_11EB0( var_1, var_0 );
        scripts\cp_mp\entityheadicons::_id_11EAF( var_2, var_0 );
    }
    else
    {
        scripts\cp_mp\entityheadicons::_id_11EB0( var_1, var_0 );
...
supportbox_usedcallback( var_0, var_1 )
{
    ...
    if ( var_0 supportbox_playercanuse( var_1 ) )
    {
        if ( isdefined( level.supply_crate_vo_when_used ) )
            var_2 = var_0 [[ level.supply_crate_vo_when_used ]]( var_1 );
        else
        ...
        if ( var_2 )
        {
            ...
            var_0 _id_125A5( var_1 );
            var_0 supportbox_givexpforuse( var_1 );
        }
    }
}

bot_fixup_pathnode_issues( var_0, var_1 )
{
    ...
    if ( var_0 supportbox_playercanuse( var_1 ) )
    {
        if ( isdefined( level.stop_counter_beep_sfx_on_bomb_vests ) )
            var_2 = var_0 [[ level.stop_counter_beep_sfx_on_bomb_vests ]]( var_1 );
        else
            var_2 = 0;

        if ( var_2 && !istrue( var_0._id_12A47 ) )
        {
            var_0.playersused[var_1 getentitynumber()] = var_1;
            var_0 _id_125A5( var_1 );
            var_0 supportbox_givexpforuse( var_1 );
