...
{
    wait_until_truck_arrives_at_station();

    foreach ( var_1 in level.players )
        wait_unload_jeep( var_1 );
}
...
setplayerscoreboardinfo()
{
    ...
    if ( var_0 < 200 )
    {
        ...
        if ( scripts\mp\utility\game::getgametype() == "br" )
        {
            var_12 = scripts\mp\gametypes\br.gsc::damageskipburndownlow( self );
            ...
            {
                var_15 = scripts\mp\gamescore::modeaddtoteamlives();
                var_16 = var_15[self.team];
                setclientmatchdata( "players", self.clientmatchdataid, "placement", var_16 );
                var_17 = scripts\mp\gametypes\br_gametype_dmz.gsc::target_show_damage();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
                var_18 = 0;

                if ( isdefined( self.select_bunker_courtyard_spawners ) )
                    var_18 = self.select_bunker_courtyard_spawners;

                setclientmatchdata( "players", self.clientmatchdataid, "extrascore5", var_18 );
            }
            else if ( scripts\mp\utility\game::missions_clearinappropriaterewards() == "kingslayer" )
            {
                var_15 = scripts\mp\gamescore::modeaddtoteamlives();
                var_16 = var_15[self.team];
                setclientmatchdata( "players", self.clientmatchdataid, "placement", var_16 );
                var_17 = scripts\mp\gametypes\br_gametype_kingslayer.gsc::target_show_damage();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
                var_18 = 0;
            ...
            {
                if ( isdefined( self._id_1268D ) )
                    setclientmatchdata( "players", self.clientmatchdataid, "placement", self._id_1268D );

                var_17 = scripts\mp\gametypes\br_gametype_rebirth.gsc::cone();
                setclientmatchdata( "players", self.clientmatchdataid, "extrascore4", var_17 );
            }
            else if ( isdefined( self._id_1268D ) )
                setclientmatchdata( "players", self.clientmatchdataid, "placement", self._id_1268D );
        }
        ...
        foreach ( var_3 in level.teamnamelist )
        {
            if ( scripts\mp\menus::shouldmodesetsquads() )
            {
                foreach ( var_5 in level.squaddata[var_3] )
                {
                    ...
                    {
                        scripts\mp\gamelogic::bottompercentagetoadjusteconomy( var_8 );
