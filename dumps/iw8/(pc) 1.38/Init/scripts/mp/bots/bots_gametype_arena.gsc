...
setup_callbacks()
{
    ...
    {
        level.bot_funcs["dropped_weapon_think"] = ::bot_cache_entrances_to_other_zones;
        level.bot_funcs["dropped_weapon_cancel"] = ::bot_can_switch_to_defender;
    }

    level.bot_funcs["gametype_think"] = ::cargo_truck_mg_deletenextframe;
}
...
cargo_truck_mg_deletenextframe()
{
    ...
    if ( !scripts\mp\gametypes\arena.gsc::isnormalloadouts() )
    {
        if ( self botgetdifficultysetting( "advancedPersonality" ) && self botgetdifficultysetting( "strategyLevel" ) > 0 )
            scripts\mp\bots\bots_gametype_gun.gsc::cbreakers( self getcurrentweapon().basename, self.pers["bot_original_personality"] );
    }

    self.move_objective_icon = 0;
    ...
    for (;;)
    {
        if ( isdefined( level.arenaflag ) && level.arenaflag.visibleteam == "any" )
        {
            ...
            if ( !scripts\mp\bots\bots_util::bot_is_capturing() || !self.move_objective_icon )
...
bot_cache_entrances_to_other_zones()
{
    ...
    for (;;)
    {
        ...
        if ( bot_br_circle_think() || scripts\mp\bots\bots_util::cashleaderbag_detach( 0.33 ) )
        {
            if ( self [[ level.bot_funcs["should_pickup_weapons"] ]]() && !scripts\mp\bots\bots_util::bot_is_remote_or_linked() )
            {
                ...
                {
                    var_4 = var_3[0];
                    level.helidestroyvehiclestouchtrace = var_4;
                    bot_cache_entrances_to_other_radios( var_4 );
                }
            }
        }
            ...
            if ( var_6 )
            {
                ...
                foreach ( var_10 in var_8 )
                {
                    ...
                    if ( isdefined( var_10.closest_point_on_grid ) )
                    {
                        ...
                        bot_cache_entrances_to_other_radios( var_10 );
                    }
                }
            }
            else if ( var_5 )
...
bot_cache_entrances_to_other_radios( var_0 )
{
    if ( scripts\mp\bots\bots_strategy::bot_has_tactical_goal( "seek_dropped_weapon", var_0 ) == 0 )
    {
        ...
        if ( isdefined( var_0.check_bonus_leads_xp ) )
            var_13 = var_0.check_bonus_leads_xp;
...
bot_can_switch_to_defender( var_0 )
{
    if ( bot_arena_think() > 0 )
    {
