...
main()
{
    ...
    level.binoculars_getpendingendtime = var_0;
    ...
    while ( !isdefined( level._id_128BB ) )
        waitframe();
    ...
    switch ( level.trial["missionScript"] )
    {
        case "gun_nonlinear":
        case "gun":
            thread [[ level._id_128BB["gun"] ]]();
            break;
        case "clear":
            thread [[ level._id_128BB["clear"] ]]();
            break;
        case "sniper":
            thread [[ level._id_128BB["sniper"] ]]();
            break;
        case "jugg":
            thread [[ level._id_128BB["jugg"] ]]();
            break;
        case "lava":
            thread [[ level._id_128BB["lava"] ]]();
            break;
        case "pitcher":
            thread [[ level._id_128BB["pitcher"] ]]();
            break;
        case "gunslinger":
            thread [[ level._id_128BB["gunslinger"] ]]();
            break;
        case "race":
            thread [[ level._id_128BB["race"] ]]();
            break;
        case "arm_course":
            thread [[ level._id_128BB["arm_course"] ]]();
            break;
        default:
            break;
    }

    _closepurchasemenuwithresponse::init();
...
_id_12902( var_0, var_1 )
{
    level notify( "exitLevel_called" );
    processlobbydata();

    if ( isdefined( level._id_1289C ) )
        [[ level._id_1289C ]]();
...
trial_mission_data_init()
{
    var_0 = _debug_rooftop_raid_exfil::_id_128AC();
...
getspawnpoint()
{
    ...
    if ( isdefined( level._id_128D3 ) )
    {
        var_3 = scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_getavailablevehicleseats( level._id_128D3, 1 );
        var_4 = spawnstruct();
        var_4.useonspawn = 1;
        var_4.enterstartwaitmsg = "spawned_player";
        thread scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_enter( level._id_128D3, var_3[0], self, var_4 );
...
onplayerconnect( var_0 )
{
    ...
    level.player scripts\mp\gametypes\br.gsc::unset_forced_aitype_armored();
    wait 1;
...
_id_128C9()
{
    for (;;)
    {
        ...
        {
            if ( !isdefined( level.proxy_include ) || !level.proxy_include )
...
processlobbydata()
{
    wait_until_trap_room_clear( level.player );
