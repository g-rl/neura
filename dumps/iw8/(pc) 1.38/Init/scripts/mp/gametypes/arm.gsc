...
main()
{
    ...
    level.hostoverride = getdvarint( "scr_arm_enableLittleBirdMG", 0 );
    ...
    level._id_12644 = getdvarint( "scr_arm_tankRespawnTime", 120 );
    level.bomber_test = getdvarint( "scr_arm_apcRespawnTime", 120 );
    level.longdialoguecooldown = getdvarint( "scr_arm_longDialogueCooldown", 1 );
    level.setup_jugg_maze_kill_trigger = getdvarint( "scr_arm_mercyEndingNuke", 1 );
    level.setup_last_enemies_standing = getdvarint( "scr_arm_mercyEndingTime", 30 );
    level.start_coop_escape = getdvarint( "scr_arm_numNonRallyVehicles", 25 );
    level.give_munition_currency = getdvarint( "scr_arm_disablePersonalNuke", 0 );
    level.total_spawns = getdvarint( "scr_personalNukeCostOverride", 30 );
    level._id_12061 = getdvarint( "scr_arm_showTeamTanks", 1 );
    level._id_12060 = getdvarint( "scr_arm_showTeamLittleBirds", 1 );
    level._id_122FD = getdvarint( "scr_arm_spawnSelectionAFKTime", 120 );
    level.set_player_hurt_trigger = getdvarint( "scr_arm_manualAdjustLittleBirdLocs", 1 );
    level.handlepersistentteamproximity = getdvarint( "scr_arm_dontSpawnJeep", 0 );
    level.handlepersistentrelicsquadlink = getdvarint( "scr_arm_dontSpawnCargoTruck", 0 );
    level._id_12B8E = getdvarint( "scr_arm_usePingSystem", 1 );

    if ( istrue( level._id_12061 ) )
    {
        level._id_127D7 = [];
        level._id_127D7["axis"] = [];
        level._id_127D7["allies"] = [];
    }

    if ( istrue( level._id_12060 ) )
    {
        level._id_127D6 = [];
        level._id_127D6["axis"] = [];
        level._id_127D6["allies"] = [];
        level._id_127D6["untouched"] = [];
    }
...
onstartgametype()
{
    ...
    if ( istrue( level.setup_jugg_maze_kill_trigger ) )
        level thread spotlight_target_found_dist_sq();

    highesttraceindex();

    if ( istrue( level._id_12B8E ) )
...
highesttraceindex()
{
    ...
    scripts\cp_mp\utility\game_utility::_id_11A27( "vehicle_volume", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11A27( "vehicle_volume_simplified", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11A27( "super", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11A27( "militarybase", "script_noteworthy" );
    scripts\cp_mp\utility\game_utility::_id_11A27( "location_volume", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_11A27( "locale_area_trigger", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_11A27( "shadow_blocker", "targetname" );
    scripts\cp_mp\utility\game_utility::_id_11A28( "door_prison_cell_metal_mp", 1 );
    scripts\cp_mp\utility\game_utility::_id_11A28( "veh8_mil_air_acharlie130", 1 );
    ...
    scripts\cp_mp\utility\game_utility::_id_11A26( "light" );
    scripts\cp_mp\utility\game_utility::_id_11A26( "trigger_use_touch" );

    if ( isdefined( level.localeid ) && level.localeid == "locale_6" )
        scripts\cp_mp\utility\game_utility::_id_11A27( "locale_8", "script_noteworthy" );
...
updatedomscores()
{
    ...
    level thread scripts\mp\spawnselection::_id_12AF8();
...
updatescores()
{
    ...
    var_5 = scripts\mp\gamescore::deactivate_battle_station();
    ...
    var_8 = scripts\mp\gamescore::deactivate_battle_station();

    if ( var_5 != var_8 )
        scripts\mp\gamescore::vehicle_is_ambient( var_8, 1, var_5 );
}
...
runobjectives( var_0 )
{
    ...
    if ( istrue( level.useobjectives ) )
    {
        ...
        foreach ( var_2 in level.allfobs )
        {
            ...
            if ( istrue( level.keypad_increase_failnum ) )
                var_2._id_1231E = scripts\mp\gametypes\obj_dom.gsc::_id_1231F;
    ...
    hackfixcameras();
    level thread binoculars_iswithinprojectiondistance_compute();
}

binoculars_iswithinprojectiondistance_compute()
{
...
objective_manageobjectivesintrovisibility()
{
    ...
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.axishqloc.increase_total_count_per_module_call.objidnum );
    scripts\mp\objidpoolmanager::objective_playermask_hidefromall( level.gw_objstruct.allieshqloc.increase_total_count_per_module_call.objidnum );
...
sethqmarkerobjective()
{
    ...
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.axishqloc.increase_total_count_per_module_call = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
    ...
    scripts\mp\objidpoolmanager::update_objective_setbackground( var_2.objidnum, 7 );
    var_2.lockupdatingicons = 1;
    level.gw_objstruct.allieshqloc.increase_total_count_per_module_call = var_2;
    level.uncapturableobjectives[level.uncapturableobjectives.size] = var_2;
...
spawnselection_showenemyhq()
{
    ...
    if ( self.team == "allies" )
    {
        ...
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.axishqloc.increase_total_count_per_module_call.objidnum, self );
    }
    else
    {
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( level.gw_objstruct.allieshqloc.increase_total_count_per_module_call.objidnum, self );
    ...
    if ( self.team == "axis" )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.allieshqloc.increase_total_count_per_module_call.objidnum, self );
    else
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( level.gw_objstruct.axishqloc.increase_total_count_per_module_call.objidnum, self );
}
...
spotlight_target_found_dist_sq()
{
    scripts\mp\flags::gameflagwait( "prematch_done" );

    if ( istrue( level.useobjectives ) )
        level thread spotlight_target_found_speed();
}

spotlight_target_found_speed()
{
    level endon( "game_ended" );
    ...
    for (;;)
    {
        if ( dangercircletick_carriable( "axis" ) == level.objectives.size )
            level thread spreadshotdamagemod( "axis" );
        else if ( dangercircletick_carriable( "allies" ) == level.objectives.size )
            level thread spreadshotdamagemod( "allies" );

        waitframe();
    }
}

spreadshotdamagemod( var_0 )
{
    ...
    setomnvar( "ui_nuke_timer_time", level.setup_last_enemies_standing );
    setomnvar( "ui_nuke_countdown_active", 1 );
    var_1 = 0;
    var_2 = gettime();
    var_3 = level.setup_last_enemies_standing * 1000 + var_2;
    setomnvar( "ui_nuke_end_milliseconds", level.setup_last_enemies_standing * 1000 + var_2 );

    while ( dangercircletick_carriable( var_0 ) == level.objectives.size )
    {
        waitframe();
...
sq_movequestlocale( var_0 )
{
    ...
    foreach ( var_5 in level.players )
    {
        ...
        {
            if ( isdefined( var_5.tac_cover_adjust_damage ) )
            {
                var_5 clearsoundsubmix( "iw8_mp_spawn_camera" );
                var_5.tac_cover_adjust_damage stoploopsound( var_5.tac_cover_blocked_by_turret );
                var_5.tac_cover_adjust_damage delete();
                var_5.tac_cover_adjust_damage = undefined;
                var_5.tac_cover_blocked_by_turret = undefined;
            }
        }
    }
    ...
    if ( isdefined( level.teamdata[var_0]["alivePlayers"][0] ) )
    {
        ...
        var_7 scripts\mp\trials\trial_race::tryusenuke();
    }
    else
...
{
    thread scripts\mp\spawnselection::x1opsspawnnpc( var_0.team, var_0.squadindex );
}
...
createc130( var_0, var_1 )
{
    ...
    var_2 thread scripts\mp\gametypes\br_c130.gsc::gunship_spawnvfx();
    return var_2;
}
...
listenkick( var_0, var_1 )
{
    ...
    var_2 = var_0 scripts\mp\gametypes\br_helicopters.gsc::calctrailpoint();
...
c130_pickrandomflightpath()
{
    ...
    level.c130pathstruct_a = scripts\mp\gametypes\br_c130.gsc::makepathstruct( var_3 );
    var_3.randomangle = var_3.randomangle + 180;
    level.c130pathstruct_b = scripts\mp\gametypes\br_c130.gsc::makepathstruct( var_3 );
    var_4 = 0.2;
    var_5 = 0;
    var_6 = 0;
...
init_groundwarvehicles()
{
    ...
    foreach ( var_1 in level.vehicleinfo )
    {
        ...
        foreach ( var_9 in var_1.vehiclespawns )
        {
            if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_9.script_noteworthy ) && var_9.script_noteworthy == level.localeid )
            {
                ...
                if ( istrue( level.handlepersistentteamproximity ) && var_1.refname == "jeep" || istrue( level.handlepersistentrelicsquadlink ) && var_1.refname == "cargo_truck" )
                    continue;
                else
                {
    ...
    var_28 = level.start_coop_escape;
...
_id_12C1D( var_0, var_1 )
{
    ...
    var_0._id_122FE = var_2;
    thread _id_12CF7( var_0 );
    level._id_127D7[var_1][level._id_127D7[var_1].size] = var_0;
}

_id_12CF7( var_0 )
{
    var_1 = var_0._id_122FE.objidnum;
    var_2 = var_0.team;
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    level._id_127D7[var_2] = scripts\engine\utility::array_remove( level._id_127D7[var_2], var_0 );
}
...
_id_12CE0()
{
    ...
    foreach ( var_1 in level._id_127D7[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_122FE.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level._id_127D7["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_122FE.objidnum, self );

    foreach ( var_1 in level._id_127D7["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_122FE.objidnum, self );
}
...
_id_12C1C( var_0, var_1 )
{
    ...
    var_0._id_122FE = var_2;
    thread _id_12CF6( var_0 );
    level._id_127D6["untouched"][level._id_127D6["untouched"].size] = var_0;
}
...
_id_12CF6( var_0 )
{
    ...
    var_0 waittill( "death" );
    scripts\mp\objidpoolmanager::returnobjectiveid( var_1 );
    var_2 = var_0.revive_friendly;

    if ( !isdefined( var_0.revive_friendly ) )
        var_2 = "untouched";

    level._id_127D6[var_2] = scripts\engine\utility::array_remove( level._id_127D6[var_2], var_0 );
}
...
_id_12CE1()
{
    ...
    foreach ( var_1 in level._id_127D6[self.team] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_122FE.objidnum, self );

    foreach ( var_1 in level._id_127D6["untouched"] )
        scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( var_1._id_122FE.objidnum, self );

    while ( self.inspawnselection )
        waitframe();

    foreach ( var_1 in level._id_127D6["axis"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_122FE.objidnum, self );

    foreach ( var_1 in level._id_127D6["allies"] )
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( var_1._id_122FE.objidnum, self );
...
_id_12CD3( var_0 )
{
    if ( isdefined( self.revive_friendly ) )
        level._id_127D6[self.revive_friendly] = scripts\engine\utility::array_remove( level._id_127D6[self.revive_friendly], self );
    else
        level._id_127D6["untouched"] = scripts\engine\utility::array_remove( level._id_127D6["untouched"], self );

    self.revive_friendly = var_0;
    level._id_127D6[var_0][level._id_127D6[var_0].size] = self;
    scripts\mp\objidpoolmanager::update_objective_icon( self._id_122FE.objidnum, "icon_minimap_littlebird_spawn_selection" );

    foreach ( var_2 in level.players )
    {
        scripts\mp\objidpoolmanager::objective_playermask_hidefrom( self._id_122FE.objidnum, var_2 );

        if ( isdefined( var_2 ) && isdefined( var_2.team ) && var_2.team == var_0 && istrue( var_2.inspawnselection ) )
        {
            scripts\mp\objidpoolmanager::objective_playermask_addshowplayer( self._id_122FE.objidnum, var_2 );
            continue;
        }
...
init_rallyvehicles()
{
    ...
    if ( !istrue( level.disablelittlebirdrally ) )
    {
        foreach ( var_2 in var_0 )
        {
            ...
            if ( level.hostoverride )
                var_6 = _findnewlocaleplacement::savedangles( var_4, var_5 );
            else
    ...
    {
        if ( level.localeid == "locale_6" && level.mapname == "mp_downtown_gw" && istrue( level.set_player_hurt_trigger ) )
        {
            var_10 = [];
        ...
        foreach ( var_2 in var_0 )
        {
            ...
            level thread _id_12C1C( var_6, "neutral" );
        }
    }
...
arm_initoutofbounds()
{
    ...
    foreach ( var_2 in var_0 )
    {
        if ( isdefined( scripts\cp_mp\utility\game_utility::getlocaleid() ) && isdefined( var_2.script_noteworthy ) && var_2.script_noteworthy == level.localeid && scripts\mp\utility\game_utility_mp::show_marker_to_tv_station( var_2 ) )
        {
...
dangercircletick_carriable( var_0 )
{
    var_1 = 0;
