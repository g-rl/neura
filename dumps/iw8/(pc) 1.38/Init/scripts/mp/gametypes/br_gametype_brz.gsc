...
init()
{
    scripts\mp\gametypes\br_gametypes.gsc::hostdamagepercentmedium( "ignoreZombiesLastStandWipe" );
    ...
    scripts\mp\gametypes\br_gametypes.gsc::zombienumhitshuman( "remainingPlayersAliveOnTeam", ::_id_119D9 );
...
skipfirstraise()
{
    for (;;)
    {
        ...
        {
            var_2 = scripts\mp\gametypes\br_public.gsc::minplunderextractions( var_1 );

            for ( var_3 = 0; var_3 < var_2.size; var_3++ )
            {
                var_4 = var_2[var_3];
                var_5 = scripts\mp\gametypes\br_public.gsc::minigun_warning_time( var_1, var_4 );
...
setincomingcallback( var_0, var_1 )
{
    ...
    if ( !istrue( self.gulag ) && scripts\mp\gametypes\br_zones.gsc::quickdropaddtoexisting( "plague", var_0 ) )
    {
        if ( var_3 )
            var_2 = var_0 scripts\mp\gametypes\br_alt_mode_zxp.gsc::vehicle_collision_dvarupdate( var_1 );
    }
    else if ( !var_0 scripts\mp\gametypes\br_public.gsc::update_timer_for_bomb_vest_detonator_holder() )
    ...
    if ( scripts\mp\flags::gameflag( "prematch_done" ) && !var_2 )
    {
        if ( var_0 scripts\mp\gametypes\br_public.gsc::update_timer_for_bomb_vest_detonator_holder() )
            thread scripts\mp\gametypes\br_alt_mode_zxp.gsc::unpause_dmz_scoring();
...
ragdoll_on_vehicle_death( var_0, var_1, var_2 )
{
    ...
    var_4 = scripts\mp\gametypes\br_public.gsc::minshotstostage3acc( var_0, var_1 );
    ...
    foreach ( var_6 in var_4 )
    {
        ...
        if ( istrue( var_3 ) && var_6 scripts\mp\gametypes\br_public.gsc::questtypes() )
            return 1;

        var_7 = var_6 scripts\mp\gametypes\br_public.gsc::update_timer_for_bomb_vest_detonator_holder() || var_6 scripts\mp\gametypes\br_alt_mode_zxp.gsc::update_operator_east_char_loc();

        if ( !var_7 && isalive( var_6 ) && !istrue( var_6.inlaststand ) )
            return 1;

        if ( istrue( var_6.inlaststand ) && ( istrue( var_6.obj_room_fire_06 ) || var_6 scripts\mp\gametypes\br_gulag.gsc::uid() ) )
            return 1;
    }
...
{
    var_3 = scripts\mp\gametypes\br_public.gsc::minshotstostage3acc( var_0, var_1 );
    ...
    foreach ( var_5 in var_3 )
    {
        ...
        if ( var_5 scripts\mp\gametypes\br_alt_mode_zxp.gsc::update_operator_east_char_loc() )
        {
            var_5 scripts\mp\gametypes\br_alt_mode_zxp.gsc::usepingsystem( 0, 0 );
            scripts\mp\gametypes\br.gsc::set_spawner_type( var_5 );
...
vehicle_compass_mp_init( var_0 )
{
    ...
    if ( level.cleanupswaploadoutflags.pickclassbr )
    {
        ...
        scripts\mp\gametypes\br_alt_mode_zxp.gsc::vandalize_spotlight_speed( var_2, var_3, var_1 );
    }
    else
    {
        if ( !var_4 )
    ...
    {
        var_7 = scripts\mp\gametypes\br.gsc::clear_and_give_killstreak_loadout_recon();
        scripts\mp\gametypes\br.gsc::monitor_traversal_watcher( var_7, 0 );
    }
    ...
    {
        if ( self != var_9 )
            scripts\mp\gametypes\br_public.gsc::cloud_cover( "zmb_teammate_back_human", var_9 );
    }
}
...
_id_12D73( var_0, var_1 )
{
    if ( scripts\mp\gametypes\br_public.gsc::update_timer_for_bomb_vest_detonator_holder() )
        ...
        scripts\mp\gametypes\br_vip_quest.gsc::_id_12D6D( self, var_0, var_1 );
}
...
_id_119D9( var_0 )
{
    return scripts\mp\gametypes\br_alt_mode_zxp.gsc::_id_119D9( var_0 );
}
