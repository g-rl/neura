...
main()
{
    ...
    if ( var_0 )
        level.propwaitminigameinit = getdvarint( "scr_infect_groundwarInfect", 0 );
    ...
    updategametypedvars();
    level.initlocs_donetsk = 0;
    level.initlocs_keypads = getdvarint( "scr_infect_time_increase_cap", 20 );
    ...
    level.ammobox_giverandomattachment = ::play_smoke_sequence_on_ent_looped;
    level.keypad_check_levelinput = ::keypad_check_levelinput;
    ...
    level.play_skit_and_watch_for_endons = getdvarint( "scr_infect_nvgMode", 0 );
    level.play_scramble_for_player_until_cleared = getdvarint( "scr_infect_disable_nvg", 0 );
    level.strict_ff = getdvarint( "scr_infect_onlineprimaryoverride", 0 );

    if ( level.propwaitminigameinit )
    {
        ...
        level.play_smuggler_arriving_vo = var_3;
        level.binoculars_setpendingtimer = getdvarint( "scr_infect_alwaysTacInsertSkySpawn", 0 );
        level._id_120CC = getdvarint( "scr_infect_skyDiveOnTacInsertPlacement", 1 );
        level.binoculars_onstateenterfunc = getdvarint( "scr_infect_allowSkyDiveCutParachute", 0 );
        level.toppercentagetoadjusteconomy = ::toppercentagetoadjusteconomy;
        level._id_125F7 = ::_id_125F7;
    }
...
onstartgametype()
{
    ...
    level.vehicle_damage_registerdefaultstates = 0;
    ...
    scripts\cp_mp\utility\script_utility::registersharedfunc( "vehicle_compass", "shouldBeVisibleToPlayer", ::_id_12C06 );

    if ( istrue( level.propwaitminigameinit ) )
    {
        scripts\mp\gametypes\arm.gsc::highesttraceindex();
        thread player_infil_landlord();
        thread search_update_delay();
    }
...
updategametypedvars()
{
    ...
    level._id_125B2 = getdvar( "scr_infect_superSurvivorTwo", "none" );
    ...
    level.play_smoke_fx = getdvar( "scr_infect_superInfectTwo", scripts\engine\utility::ter_op( level.propwaitminigameinit, "super_tac_insert", "none" ) );
    ...
    level.play_quarry_intro_vo = getdvarint( "scr_infect_infectBonusScore", 150 );
    level.play_random_sound_event = getdvarint( "scr_infect_infectBonusSuperOnSpawn", 0 );
    level.play_reset_priming_anim = getdvarint( "scr_infect_infectBonusSuperOnTacInsert", 0 );
    level.is_greater_than_equal_to = getdvarint( "scr_infect_finalSuvivorCount", 1 );
    level.play_sound_countdown = getdvarint( "scr_infect_setRadarOnNumSurvivors", 8 );
    level._id_125B1 = [];
    var_0 = getdvar( "scr_infect_survivorStreakOverride", "" );

    if ( var_0 != "" )
        level._id_125B1 = strtok( var_0, "," );

    var_1 = scripts\cp_mp\utility\game_utility::getmapname();

    if ( issubstr( var_1, "mp_m_" ) && var_1 != "mp_m_speed" )
    {
        level.survivorsuper = isbunkeraltenabled( 0, 0 );
        level._id_125B2 = isbunkeraltenabled( 0, 1 );
        level.infectedsuper = isbunkeraltenabled( 1, 0 );
        level.play_smoke_fx = isbunkeraltenabled( 1, 1 );
    }

    if ( level.survivorsuper == level._id_125B2 )
        level._id_125B2 = "none";
    else if ( level.survivorsuper == "none" && level._id_125B2 != "none" )
    {
        level.survivorsuper = level._id_125B2;
        level._id_125B2 = "none";
    }

    if ( level.infectedsuper == level.play_smoke_fx )
        level.play_smoke_fx = "none";
    else if ( level.infectedsuper == "none" && level.play_smoke_fx != "none" )
    {
        level.infectedsuper = level.play_smoke_fx;
        level.play_smoke_fx = "none";
    }
...
buildandloadweapons()
{
    ...
    foreach ( var_1 in level.allweapons )
    {
        if ( var_1 != "none" )
        {
            ...
            if ( level.play_skit_and_watch_for_endons )
                game["arenaRandomLoadout"][0]["loadoutPrimaryAttachment"] = modeiskillstreakallowed();
...
{
    if ( istrue( var_1 ) )
        var_2 = scripts\engine\utility::ter_op( var_0, level.play_smoke_fx, level._id_125B2 );
    else
...
onspawnfinished()
{
    ...
    if ( self.pers["team"] == "allies" )
    {
        ...
        if ( level.propwaitminigameinit )
            thread monitor_victim_cash_drops();

        spawnwithplayersecondary();

        if ( level.play_skit_and_watch_for_endons && !level.play_scramble_for_player_until_cleared && !isbot( self ) )
    ...
    else if ( self.pers["team"] == "axis" )
    {
        if ( istrue( level.ninetypercent_music ) )
        {
            ...
            self.protect_obj_a_internal = 1;
        }

        if ( istrue( level.binoculars_onstateenterfunc ) )
    ...
    if ( istrue( self.spawn_custom_type ) )
    {
        if ( !level.propwaitminigameinit || level.propwaitminigameinit && level.mapname == "mp_aniyah" )
    ...
    if ( istrue( self.quest_secretdistmax ) )
    {
        self.quest_secretdistmax = undefined;
        thread scripts\cp_mp\parachute::playac130animinternal( 1 );
    }

    thread scripts\mp\supers::givesuperpoints( level.play_random_sound_event, undefined, 1 );
    thread _id_12E33();
}
...
_id_12E33()
{
    ...
    if ( istrue( level.ninetypercent_music ) && self.team == "axis" )
...
setfirstinfected( var_0 )
{
    ...
    while ( !scripts\mp\utility\player::isreallyalive( self ) || scripts\mp\utility\player::isusingremote() || isdefined( self._id_12D1C ) )
        waitframe();
    ...
    if ( isdefined( var_1 ) )
    {
        ...
        var_2.init_silo_thrust_obj = "INVOLUNTARY";
    ...
    if ( var_0 )
    {
        ...
        thread scripts\mp\music_and_dialog::play_reset_sequences();
    }

    self.isinitialinfected = 1;
    scripts\mp\utility\stats::incpersstat( "firstInfected", 1 );
    self.pers["gamemodeLoadout"] = level.infect_loadouts["axis_initial"];
    scripts\mp\equipment\tac_insert::_id_122E5( self.origin, self.angles );
    ...
    if ( isdefined( scripts\mp\supers::getcurrentsuper() ) && scripts\mp\supers::getcurrentsuperref() == "super_deadsilence" )
        thread _id_1247B();
    else if ( level.propwaitminigameinit )
        thread scripts\mp\supers::givesuperpoints( 350, undefined, 1 );

    if ( var_0 )
        level.infect_players[self.name] = 1;

    level.isanyplayertouching = 1;

    if ( !level.isanyplayertouching )
...
onplayerkilled( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    {
        if ( isdefined( level.stack_patch_waittill_context_far ) && self == level.stack_patch_waittill_context_far )
        {
            var_10 = 0;
            var_11 = 0;
        }
    ...
    if ( var_10 )
    {
        ...
        if ( scripts\mp\utility\dvars::getwatcheddvar( "timelimit" ) != 0 )
        {
            ...
            if ( scripts\mp\utility\game::matchmakinggame() )
            {
                level.initlocs_donetsk++;
                var_17 = level.initlocs_donetsk <= level.initlocs_keypads;
            }
...
{
    self.spawn_custom_type = 1;

    if ( level.propwaitminigameinit && level.mapname != "mp_aniyah" )
        _id_11EA4();
    ...
    if ( level.infect_teamscores["allies"] > 1 )
    {
        if ( level.propwaitminigameinit )
        {
            level.vehicle_damage_registerdefaultstates++;

            if ( level.vehicle_damage_registerdefaultstates > 3 )
                level.vehicle_damage_registerdefaultstates = 1;

            if ( level.infect_teamscores["allies"] == level.play_sound_countdown )
                level thread _id_12495();
        }

        if ( level.vehicle_damage_registerdefaultstates == 1 )
        {
        ...
        if ( !var_1 )
        {
            ...
            foreach ( var_5 in level.players )
            {
                ...
                if ( var_5.team == "axis" && var_5 != var_0 && var_5 != self )
                    var_5 thread scripts\mp\utility\points::giveunifiedpoints( "assist", undefined, level.play_quarry_intro_vo );
            }
        }
    }
    else if ( level.infect_teamscores["allies"] == level.is_greater_than_equal_to )
        onfinalsurvivor();
...
search_update_delay()
{
    ...
    while ( level.infect_teamscores["allies"] > level.play_sound_countdown )
    {
        foreach ( var_1 in level.players )
        {
            if ( var_1.team == "axis" )
...
enduavonlatejoiner( var_0 )
{
    ...
    for (;;)
    {
        if ( level.infect_teamscores["allies"] > level.is_greater_than_equal_to )
        {
...
monitordisconnect()
{
    ...
    if ( isdefined( self.infect_isbeingchosen ) || level.infect_chosefirstinfected )
    {
        if ( level.infect_teamscores["axis"] && level.infect_teamscores["allies"] )
        {
            if ( level.propwaitminigameinit )
            {
                if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.play_sound_countdown )
                    level thread _id_12495();
            }

            if ( var_0 == "allies" && level.infect_teamscores["allies"] == level.is_greater_than_equal_to )
                onfinalsurvivor();
...
setspecialloadouts()
{
    ...
    var_0 = 0;
    var_1 = scripts\mp\gametypes\gun.gsc::managevehiclehealthui( level.survivorprimaryweapon );
    var_0 = scripts\mp\class::_id_125DA( level.survivorprimaryweapon, var_1 );
    var_2 = 0;
    var_3 = scripts\mp\gametypes\gun.gsc::managevehiclehealthui( level.survivorsecondaryweapon );
    var_2 = scripts\mp\class::_id_125DA( level.survivorsecondaryweapon, var_3 );
    ...
    else if ( level.strict_ff )
    {
        ...
        if ( level._id_125B1.size > 0 )
        {
            level.infect_loadouts["allies"]["loadoutKillstreak1"] = level._id_125B1[0];
            level.infect_loadouts["allies"]["loadoutKillstreak2"] = level._id_125B1[1];
            level.infect_loadouts["allies"]["loadoutKillstreak3"] = level._id_125B1[2];
        ...
        level.infect_loadouts["allies"]["loadoutFieldUpgrade2"] = level._id_125B2;
    }
    else
    {
        level.infect_loadouts["allies"]["loadoutPrimary"] = level.survivorprimaryweapon;
    ...
    {
        var_6 = scripts\mp\gametypes\gun.gsc::managevehiclehealthui( level.initialprimaryweapon );
        var_4 = scripts\mp\class::_id_125DA( level.initialprimaryweapon, var_6 );
    }
    ...
    {
        var_7 = scripts\mp\gametypes\gun.gsc::managevehiclehealthui( level.initialsecondaryweapon );
        var_5 = scripts\mp\class::_id_125DA( level.initialsecondaryweapon, var_7 );
    }
    ...
    else if ( level.strict_ff )
    {
        ...
        level.infect_loadouts["axis_initial"]["loadoutFieldUpgrade2"] = level.play_smoke_fx;
    }
    else
    {
        level.infect_loadouts["axis_initial"]["loadoutPrimary"] = level.initialprimaryweapon;
    ...
    {
        var_9 = 0;
        var_10 = scripts\mp\gametypes\gun.gsc::managevehiclehealthui( level.infectedprimaryweapon );
        var_9 = scripts\mp\class::_id_125DA( level.infectedprimaryweapon, var_10 );
    }
    ...
    else
    {
        ...
        level.infect_loadouts["axis"]["loadoutFieldUpgrade2"] = level.play_smoke_fx;
    ...
    for (;;)
    {
        if ( self isnightvisionon() )
        {
            ...
            if ( var_0 >= 10 )
            {
                ...
                scripts\common\utility::binoculars_cansee( 0 );
                wp_watchgameend();
                scripts\common\utility::binoculars_cansee( 1 );
                var_1 = 0;
...
end_unlock_silo()
{
    ...
    self.loadoutfieldupgrade2 = level.play_smoke_fx;
    wait 0.1;
    ...
    if ( isdefined( self ) && scripts\mp\utility\player::isreallyalive( self ) )
    {
        ...
        var_0 = "gamemode";
        var_1 = scripts\mp\class::scn_infil_hackney_heli_npc4();
        var_1 = scripts\mp\class::score_event_civilian_hit( var_1, var_0 );
        ...
        scripts\mp\equipment::giveequipment( level.infectedtactical, "secondary" );
        thread scripts\mp\supers::givesuperpoints( level.play_reset_priming_anim, undefined, 1 );
    }

    if ( istrue( level._id_120CC ) )
        thread _id_1257B();
    else
...
player_infil_landlord()
{
    ...
    var_18 = level.start_coop_escape;
...
{
    level endon( "game_ended" );
    level.play_rooftop_success_vo = [];
    level thread _id_11F74();
    scripts\mp\flags::gameflagwait( "prematch_done" );
    wait( level.droptime );
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_0 ) && scripts\mp\utility\killstreak::currentactivevehiclecount() < scripts\mp\utility\killstreak::maxvehiclesallowed() && level.fauxvehiclecount + var_1 < scripts\mp\utility\killstreak::maxvehiclesallowed() && scripts\cp_mp\killstreaks\airdrop::getnumdroppedcrates() < 8 )
        {
            ...
            var_3 = manualturret_disablecrouchpronemantle();
...
{
    var_0 = level.play_rooftop_success_vo[0];
    level.play_rooftop_success_vo[level.play_rooftop_success_vo.size] = level.play_rooftop_success_vo[0];
    level.play_rooftop_success_vo[0] = undefined;
    level.play_rooftop_success_vo = scripts\engine\utility::array_removeundefined( level.play_rooftop_success_vo );
    return var_0;
}
...
play_smoke_sequence_on_ent_looped()
{
...
keypad_check_levelinput( var_0, var_1, var_2, var_3, var_4 )
{
    var_5 = 0;
