...
setupobjective( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    if ( isdefined( var_0.iconname ) )
        var_7 = var_0.iconname;
    else
        var_7 = var_0.script_label;

    if ( istrue( var_2 ) )
        var_8 = getreservedobjid( var_6 );
    else
        var_8 = undefined;
    ...
    var_9 = scripts\mp\gameobjects::createuseobject( var_1, var_0, var_5, ( 0, 0, 100 ), var_8, var_3, var_4 );
    var_9 scripts\mp\gameobjects::allowuse( "enemy" );
    var_9 scripts\mp\gameobjects::cancontestclaim( 1 );
    var_9 scripts\mp\gameobjects::setusetime( level.flagcapturetime );

    if ( isdefined( level.capturetype ) )
        var_9 scripts\mp\gameobjects::setcapturebehavior( getcapturetype() );

    var_9.objectivekey = var_6;
    var_9.iconname = var_7;

    if ( !istrue( var_3 ) )
    {
        var_9 scripts\mp\gameobjects::setvisibleteam( "any", undefined, 1 );
        var_9.onuse = ::dompoint_onuse;
        var_9.onbeginuse = ::dompoint_onusebegin;
        var_9.onuseupdate = ::dompoint_onuseupdate;
        var_9.onenduse = ::dompoint_onuseend;
        var_9.oncontested = ::dompoint_oncontested;
        var_9.onuncontested = ::dompoint_onuncontested;
        var_9.onunoccupied = ::dompoint_onunoccupied;
        var_9.onpinnedstate = ::dompoint_onpinnedstate;
        var_9.onunpinnedstate = ::dompoint_onunpinnedstate;
        var_9.stompprogressreward = ::dompoint_stompprogressreward;
    }

    var_9.nousebar = 1;
    var_9.id = "domFlag";
    var_9.claimgracetime = level.flagcapturetime * 1000;
    var_9.firstcapture = 1;
    var_9 scripts\mp\gameobjects::pinobjiconontriggertouch();

    if ( istrue( level.keypad_increase_failnum ) )
        var_9._id_1231E = ::_id_1231F;
    else if ( istrue( level.ninetypercent_music ) && scripts\mp\utility\game::getgametype() != "br" )
        var_9._id_1231E = ::_id_12320;
    ...
    var_15 = checkmapoffsets( var_9 );
    var_9.baseeffectpos = var_14["position"] + var_15;
    var_16 = vectortoangles( var_14["normal"] );
    var_17 = checkmapfxangles( var_9, var_16 );
    var_9.baseeffectforward = anglestoforward( var_17 );
    var_18 = spawn( "script_model", var_9.baseeffectpos );
    var_18 setmodel( "dom_flag_scriptable" );
    var_18.angles = generateaxisanglesfromforwardvector( var_9.baseeffectforward, var_18.angles );
    var_9.scriptable = var_18;
    var_9.vfxnamemod = "";
    var_9.noscriptable = 1;

    if ( istrue( level.multiteambased ) )
        var_9.noscriptable = 1;

    var_9.flagmodel = spawn( "script_model", var_9.baseeffectpos );

    if ( istrue( level.ninetypercent_music ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_19 = "decor_halloween_scarecrow";
        var_9.flagmodel.raid_seq4_objectives_func = 1;
    }
    else
        var_19 = "military_dom_flag_neutral";

    var_9.flagmodel setmodel( var_19 );
    var_9.flagmodel.angles = display_message_to_teammates( var_9 );
    var_9.outlineent = var_9.flagmodel;

    if ( istrue( level.ninetypercent_music ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        var_9 thread nexttrackindex();
        var_9 thread handleendgamenonwinnersplash();
    }

    var_9 initializematchrecording();

    if ( !istrue( var_3 ) )
        var_9 domflag_setneutral( undefined, 1 );
...
dompoint_setcaptured( var_0, var_1 )
{
    ...
    if ( isdefined( self.assisttouchlist[var_0] ) )
    {
        ...
        foreach ( var_8 in var_6 )
        {
            ...
            var_9 scripts\cp\helicopter\chopper_boss::stop_module_on_delay();
        }
    }
...
dompoint_onuse( var_0 )
{
    ...
    if ( !self.neutralized )
    {
        ...
        if ( isdefined( level.onobjectivecomplete ) )
        {
            ...
            var_5 = "Flag " + maxdogtags() + " Captured";

            if ( var_2 == "neutral" || var_2 == "none" )
                scripts\mp\utility\game::scriptable_token_scriptable_touched_callback( var_0, undefined, var_5, var_0.origin, "neutral_flag" );
            else
                scripts\mp\utility\game::scriptable_token_scriptable_touched_callback( var_0, undefined, var_5, var_0.origin );
        }
    }
...
dompoint_onusebegin( var_0 )
{
    ...
    {
        if ( var_1 != var_0.team )
            var_2 = make_heli_blade_patch_clip();
        else
            var_2 = 0;
    }
...
{
    self.extraction_helicoptor_total_plunder = gettime();
...
dompoint_onuncontested( var_0 )
{
    ...
    var_2 = ( gettime() - self.extraction_helicoptor_total_plunder ) * 0.001;
    var_3 = "Flag " + maxdogtags() + " Contested";
    scripts\mp\utility\game::scriptable_token_scriptable_touched_callback( undefined, undefined, var_3, undefined, var_2 + " seconds" );
    self.extraction_helicoptor_total_plunder = undefined;
...
_id_1231F()
{
    ...
    if ( var_7["fraction"] != 1 )
    {
        ...
        if ( var_7["fraction"] != 1 )
        {
            playfx( level.move_hvt_from_under_heli["gos_fireworks"], var_7["position"] );
            playfx( level.move_hvt_from_under_heli["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
            playfx( level.move_hvt_from_under_heli["gos_fireworks"], var_7["position"], anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
            level thread scripts\mp\gametypes\common.gsc::_id_12E31( var_7["position"] );
        }
    }
    else
    {
        playfx( level.move_hvt_from_under_heli["gos_fireworks"], var_2 );
        playfx( level.move_hvt_from_under_heli["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( -10, -10, 0 ) );
        playfx( level.move_hvt_from_under_heli["gos_fireworks"], var_2, anglestoforward( self.flagmodel.angles ) + ( 10, 10, 0 ) );
        level thread scripts\mp\gametypes\common.gsc::_id_12E31( var_2 );
    }
}

_id_12320()
{
    if ( isdefined( self.pingedenemies ) )
        return;
    else
    {
        self.pingedenemies = 1;
        var_0 = 20;
        var_1 = 600;
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.pindia_headlights["blood_ceil_hw"], var_7["position"] - ( 0, 0, 5 ), ( -180, -180, 0 ), ( -180, -180, -180 ) );

        playfx( level.pindia_headlights["blood_floor_hw"], self.flagmodel.origin + ( 0, 0, 2 ) );
        var_0 = 4000;
        var_1 = 20;
        var_2 = self.flagmodel.origin;
        var_3 = var_2 + ( 0, 0, var_0 );
        ...
        if ( var_7["fraction"] != 1 )
            playfx( level.pindia_headlights["bats_fly_hw"], var_7["position"] + ( 0, 0, 50 ) );
        else
            playfx( level.pindia_headlights["bats_fly_hw"], self.flagmodel.origin + ( 0, 0, 50 ) );
    }
}
...
nexttrackindex()
{
    ...
    if ( var_7["fraction"] != 1 )
        playfx( level.pindia_headlights["bats_hw"], var_7["position"] );
    else
        playfx( level.pindia_headlights["bats_hw"], self.flagmodel.origin + ( 0, 0, 200 ) );
}
...
updateflagstate( var_0, var_1, var_2, var_3 )
{
    ...
    if ( istrue( level.ninetypercent_music ) && scripts\mp\utility\game::getgametype() != "br" )
    {
        if ( istrue( var_3 ) )
            ...
            self.flagmodel thread _id_11C79();
        else if ( isdefined( var_2 ) )
            playfx( level.pindia_headlights["vanish_hw_fr"], self.flagmodel.origin + ( 0, 0, 60 ) );
    }
    else if ( isdefined( var_2 ) )
    {
        if ( var_2 == "allies" )
...
giveflagcapturexp( var_0, var_1, var_2 )
{
    ...
    if ( isplayer( var_3 ) )
    {
        ...
        var_3 thread scripts\common\utility::_id_12965( level.set_trap_flag, "capture", var_3.origin );
    }
...
awardgenericmedals( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( var_0 ) )
    {
        ...
        {
            if ( istrue( var_0.raid_seq4_objectives_func ) )
                return;
        }
    }
            ...
            if ( var_20 || var_21 )
            {
                if ( var_18.claimteam == var_13.team )
                {
                    if ( !var_11 )
                    {
                        ...
                        var_18 notify( "assault", var_1 );
                        thread scripts\common\utility::_id_12965( level.set_total_successful_vehicle_spawns_from_module, var_9, "assaulting" );
                        continue;
                    }
                }
                else if ( var_18.claimteam == var_1.team )
                {
                    if ( !var_12 )
                    {
                        ...
                        thread scripts\common\utility::_id_12965( level.set_total_successful_vehicle_spawns_from_module, var_9, "defending" );
                        continue;
                    }
                }
            }
...
_id_11C79()
{
    ...
    wait 0.55;
    playfx( level.pindia_headlights["vanish_hw_fr"], self.origin + ( 0, 0, 60 ) );
...
handleendgamenonwinnersplash()
{
    ...
    for (;;)
    {
        ...
        if ( isplayer( var_0 ) )
        {
            if ( !isdefined( self._id_123AC ) )
            {
                self._id_123AC = var_0;
                thread _id_12F64( self.flagmodel, var_0 );
            }
        }
...
_id_12F64( var_0, var_1 )
{
    level endon( "game_ended" );
    self notify( "new_stalker_target" );
    self endon( "new_stalker_target" );

    while ( isdefined( self._id_123AC ) )
    {
        if ( !_id_12BC3( var_0, var_1 ) )
        {
            self._id_123AC = undefined;
            self._id_123AB = 0;
            self.initlocs_test = undefined;
            self.blueprintextract_beforepickupspawned = undefined;
            self.x1opspreplayertransition = undefined;
            self.flagmodel rotateto( ( 0, self.flagmodel.angles[1], self.flagmodel.angles[2] ), 0.2, 0.1, 0.1 );
        }
        else if ( isdefined( self._id_123AC ) && scripts\mp\utility\player::isreallyalive( self._id_123AC ) )
        {
...
_id_12F5C()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_1 ) && isdefined( self._id_123AC ) && self._id_123AC == var_1 )
            self._id_123AB = self._id_123AB + var_0;

        if ( isdefined( self._id_123AB ) )
        {
            if ( self._id_123AB > 100 && !isdefined( self.initlocs_test ) )
                self.initlocs_test = 1;

            if ( self._id_123AB > 200 && !isdefined( self.blueprintextract_beforepickupspawned ) )
            {
                self.blueprintextract_beforepickupspawned = 1;
                playfx( level.pindia_headlights["vanish_hw_en"], self.flagmodel.origin + ( 0, 0, 80 ) );
            }

            if ( self._id_123AB > 300 && !isdefined( self.x1opspreplayertransition ) )
            {
                self.x1opspreplayertransition = 1;
                thread _id_11AC2( var_1 );
            }
        }
...
{
    var_2 = 0.05;
    var_3 = var_1 missed_shots();
    var_4 = var_0 minigamewinnersettings();
