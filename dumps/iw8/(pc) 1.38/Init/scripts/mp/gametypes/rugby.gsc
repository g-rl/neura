...
main()
{
    ...
    level.show_label = ::delay_start_escort_enter_vehicle_objective;
...
initrules()
{
    ...
    updategametypedvars();
    level.ontimelimitgraceperiod = maphitloctoburningpart();
...
initrugbyents()
{
    level.plunderextractalertnearby = scripts\engine\utility::array_combine( level.outofboundstriggers, level.minetriggers, level.hurttriggers, level.radtriggers );
    initendzoneents();
...
initjuggcratesetupents()
{
    ...
    foreach ( var_3 in var_1 )
    {
        var_3 = isbotpracticematch( var_3 );
...
modeonteamchangedeath( var_0 )
{
    if ( isdefined( level._id_11BB8 ) )
        var_0 setclientomnvar( "ui_rugby_jugg_friendly", scripts\engine\utility::ter_op( var_0.team == level._id_11BB8.team, 0, 1 ) );
}
...
onplayerspawned( var_0 )
{
    ...
    if ( isdefined( level._id_11BB8 ) && isdefined( level._id_11BB8.team ) && isdefined( var_0.team ) )
        var_0 setclientomnvar( "ui_rugby_jugg_friendly", scripts\engine\utility::ter_op( var_0.team == level._id_11BB8.team, 0, 1 ) );
}
...
awardgenericmedals( var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9 )
{
    ...
    if ( isdefined( level._id_11BB8 ) )
    {
        ...
        {
            if ( var_1 == level._id_11BB8 )
                var_1 thread scripts\mp\utility\points::giveunifiedpoints( "kill_as_juggernaut" );
            else if ( var_12 == level._id_11BB8 )
                var_1 thread scripts\mp\utility\points::giveunifiedpoints( "kill_juggernaut" );
            else if ( var_1.team == level._id_11BB8.team && var_1 != level._id_11BB8 )
            {
                var_16 = distancesquared( level._id_11BB8.origin, var_14 );
            ...
            else if ( var_1.team != level._id_11BB8.team && var_12.team == level._id_11BB8.team )
            {
                var_16 = distancesquared( level._id_11BB8.origin, var_13 );
...
ontimelimitot()
{
    if ( level.rugby.activejuggernauts.size > 0 )
    {
        ...
        {
            setomnvar( "ui_overtime_time", gettime() + maphitloctoburningpart() * 1000 );
            level.rugby.inot = 1;
            level.canprocessot = 1;
            _id_11EB7();
            level thread register_vehicle_max_ai();
        }
    }
    else
    {
...
_id_11F59( var_0 )
{
    ...
    var_0 thread br_ammo_player_is_maxed_out( var_1, var_0.trigger.targetname );
...
br_ammo_player_is_maxed_out( var_0, var_1 )
{
    ...
    var_2 = vfx_htown_hadirj_blink( var_2, var_1 );
    var_3 = [];
...
vfx_htown_hadirj_blink( var_0, var_1 )
{
    var_2 = [];
    var_3 = [];
...
forceexplosivedeath( var_0, var_1, var_2 )
{
    var_0.origin = var_1;
    var_0.angles = var_2;
    return var_0;
...
activatenewjuggernaut( var_0 )
{
    ...
    level thread triggerovertimetimer( var_0.team, var_6 );
...
triggerovertimetimer( var_0, var_1 )
{
    wait 3;
...
deactivatejuggernaut( var_0 )
{
    ...
    level.rugby.has_filled_amped_bar = undefined;
    level.rugby.waitforallcrates = undefined;
    level.rugby.registerontimerexpired = undefined;
    level.rugby.registeronrespawn = undefined;
...
watchjuggprogress()
{
    ...
    for (;;)
    {
        ...
        if ( var_9 > 0.8 )
            level thread do_manual_splash_damage_when_frag_explodes( var_9, var_0, var_2 );
        ...
        if ( isdefined( level._id_11BB8 ) )
        {
            ...
            {
                var_7 = var_9;
                level._id_11BB8 thread scripts\mp\utility\points::giveunifiedpoints( "rugby_obj_push" );
            }
        }
...
do_manual_splash_damage_when_frag_explodes( var_0, var_1, var_2 )
{
    if ( !isdefined( level.rugby.has_filled_amped_bar ) )
    {
        level.rugby.has_filled_amped_bar = 1;
        level.rugby.waitforallcrates = gettime();
        level.rugby.registerontimerexpired = var_0;
        level.rugby.registeronrespawn = var_0;
    }
    else if ( isdefined( level.rugby.waitforallcrates ) && level.rugby.waitforallcrates + 30000 < gettime() )
    {
        level.rugby.has_filled_amped_bar = 1;
        level.rugby.waitforallcrates = gettime();
        level.rugby.registerontimerexpired = var_0;
    ...
    if ( istrue( level.rugby.has_filled_amped_bar ) )
    {
    ...
    level.rugby.has_filled_amped_bar = 0;
}
...
handlejuggjumpspam()
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            var_2 = scripts\engine\utility::_id_12E53( 1, "jugg_jumped" );
...
{
    var_0 = 0;
    level.registerontimerupdate = spawnstruct();
    level.registerontimerupdate.objidnum = scripts\mp\objidpoolmanager::requestreservedid( var_0 );
}
...
activatejuggcrate( var_0, var_1, var_2 )
{
    ...
    if ( !rear_spotlight_angles_offset( var_0 ) )
    {
        var_0 = level.finale_main;
        var_1 = level.finale;
    }
...
rear_spotlight_angles_offset( var_0 )
{
    var_1 = 1;

    foreach ( var_3 in level.plunderextractalertnearby )
    {
...
juggcratemanageuse( var_0 )
{
    ...
    var_1.rear_minigun_tag = 1;
    self._id_12B78 = scripts\mp\gameobjects::createholduseobject( "neutral", var_1, self.visuals, ( 0, 0, 64 ) );

    foreach ( var_3 in level.teamnamelist )
        self._id_12B78.teamprogress = undefined;

    self._id_12B78.trigger._id_12B8D = 16900;
    self._id_12B78.trigger setuserange( 130 );
    self._id_12B78.trigger sethintstring( &"MP_MODE_RUGBY/CRATE_USE" );
    self._id_12B78.trigger makeusable();
    self._id_12B78.trigger setcursorhint( "HINT_BUTTON" );
    self._id_12B78.trigger setuseholdduration( "duration_none" );
    self._id_12B78.trigger setusehideprogressbar( 1 );
    self._id_12B78.trigger setusepriority( -3 );
    self._id_12B78.trigger sethintonobstruction( "hide" );
    self._id_12B78 scripts\mp\gameobjects::setusetime( maphint_keypadscriptableused() );
    self._id_12B78.interactteam = "any";
    self._id_12B78.curprogress = 0;
    self._id_12B78.defaultusetime = self._id_12B78._id_12B9D;
    self._id_12B78.userate = 1;
    self._id_12B78.id = "rugby_jugg";
    self._id_12B78.exclusiveuse = 0;
    self._id_12B78.exclusiveclaim = 0;
    self._id_12B78.skiptouching = 1;
    self._id_12B78.onbeginuse = ::_id_11BB5;
    self._id_12B78.onuse = ::_id_11BB4;
    self._id_12B78.onenduse = ::_id_11BB6;
    self._id_12B78.oncantuse = ::_id_11BB7;
    self._id_12B78.inuse = 0;

    foreach ( var_6 in level.teamnamelist )
    {
        self._id_12B78.numtouching[var_6] = 0;
        self._id_12B78.touchlist[var_6] = [];
    }

    self._id_12B78.cancontestclaim = 0;
    self._id_12B78.stalemate = 0;
    self._id_12B78.wasstalemate = 0;
    self._id_12B78.cancontestclaim = 0;
    self._id_12B78.majoritycapprogress = 1;
    self._id_12B78.wasmajoritycapprogress = 0;
    self._id_12B78.resetprogress = 1;

    if ( !scripts\mp\flags::gameflag( "prematch_done" ) )
        level scripts\engine\utility::_id_12E3F( "prematch_done", "start_mode_setup" );

    self._id_12B78.type = "useObject";
    self._id_12B78.offset3d = ( 0, 0, 32 );
    self._id_12B78 scripts\mp\gameobjects::requestid( 1, 1 );
    self._id_12B78 scripts\mp\gameobjects::setobjectivestatusicons( "icon_waypoint_jugg_crate" );
    self._id_12B78 scripts\mp\gameobjects::setvisibleteam( "any" );
    scripts\mp\objidpoolmanager::objective_set_play_intro( self._id_12B78.objidnum, 0 );
    scripts\mp\objidpoolmanager::objective_set_play_outro( self._id_12B78.objidnum, 0 );
}
...
_id_11BB4( var_0 )
{
    ...
    if ( isdefined( self._id_12B78 ) )
        self._id_12B78 delete();
...
delay_start_escort_enter_vehicle_objective( var_0 )
{
    ...
    var_2 = var_0 scripts\mp\juggernaut::register_chopper_boss_combat_action( var_1 );
...
updateoverheadcamerapos( var_0 )
{
    ...
    var_10 = scripts\cp_mp\utility\killstreak_utility::mark_danger_timeout();
