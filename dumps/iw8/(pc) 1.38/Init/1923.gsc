...
playingcoughdamagesound()
{
    level._id_12FEF = loadweaponranktable( "mp/t9_weaponranktable.csv" );
...
ml_p2_fail_start( var_0 )
{
    if ( var_0 == 0 )
        return 0;

    for ( var_1 = matchdata_logplayerdeath() - 1; var_1 >= 0; var_1-- )
    {
        if ( var_0 >= ml_p2_func( var_1 ) )
            return var_1;
    }
...
matchdata_logplayerdeath()
{
    return level._id_12FEF.maxrank;
}
...
ml_p2_func( var_0 )
{
    return level._id_12FEF.rankinfo[var_0].minxp;
}
...
modifyfalldamage()
{
    if ( !isdefined( level.vehicle_deletenextframelate ) )
        level.vehicle_deletenextframelate = getdvarfloat( "scr_playlist_weaponxp_scalar", 1.0 );

    return level.vehicle_deletenextframelate;
}
...
managejumpmasterinfodisplay()
{
    if ( !isdefined( level.kickplayer ) )
        level.kickplayer = float( tablelookup( "mp/gametypesTable.csv", 0, scripts\mp\utility\game::getgametype(), 19 ) );
...
manageafktracking()
{
    if ( !isdefined( level.keypadkeys ) )
        level.keypadkeys = getdvarint( "scr_KPH_override", int( tablelookup( "mp/gametypesTable.csv", 0, scripts\mp\utility\game::getgametype(), 20 ) ) );
