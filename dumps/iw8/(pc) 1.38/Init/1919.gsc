...
init()
{
    ...
    level.multiteambased = level.teamnamelist.size > 2 || scripts\mp\utility\game::getgametype() == "br" && scripts\mp\menus::_id_1237A();
    level.teamdata = [];
    ...
    {
        scripts\mp\utility\teams::mlgiconfullflag( var_9 );
    ...
    if ( isdefined( level.ispayloadautomaticallymoving ) )
        level thread [[ level.ispayloadautomaticallymoving ]]();

    if ( getdvarint( "scr_debug_teams", 0 ) == 1 )
        level thread get_landing_spots_in_current_circle();
}
...
createoperatorcustomization()
{
    ...
    var_10.clothtype = maxmuncurrencycap( var_9 );
    var_10.superfaction = getoperatorsuperfaction( var_8 );
    var_10.execution = getoperatorexecution( var_8 );
    var_10.init_pipe_traps = maxnumsites( var_8 );
    var_10.init_placed_equipment = maxperkbonustier( var_8 );
    var_10.suit = var_3;
    var_10.rebuild = 0;
    var_10.outputfunc = quest_domdistmin( var_1 );

    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        var_11 = maxlootleadermarkcount( var_9 );

        if ( var_11 != "" )
            var_10.clear_legacy_pickup_munitions = var_11;
    }

    var_12 = spawnstruct();
    var_12.apc = modevalidatekillcam( "apc" );
    var_12.bomber_think = modify_juggernaut_damage( "apc" );
    var_12.br_practice_match = modevalidatekillcam( "atv" );
    var_12.delete_keypad_display_models = modevalidatekillcam( "cargo_truck" );
    var_12.redeployspawns = modevalidatekillcam( "jeep" );
    var_12.safehouse_vo_return_start = modevalidatekillcam( "little_bird" );
    var_12._id_125E9 = modevalidatekillcam( "tac_rover" );
    var_12._id_12633 = modevalidatekillcam( "tank_east" );
    var_12._id_12634 = modify_juggernaut_damage( "tank_east" );
    var_12._id_1263E = modevalidatekillcam( "tank_west" );
    var_12._id_1263F = modify_juggernaut_damage( "tank_west" );
    var_12.soldier_agent_lwfn3 = modevalidatekillcam( "motorcycle" );
    var_12.bomb_vest_timer_yellow_starting_frame = modevalidatekillstreakslot( "apc", 4 );
    var_12.br_prematchspawnlocations = modevalidatekillstreakslot( "atv", 6 );
    var_12.delete_laser_entities = modevalidatekillstreakslot( "cargo_truck", 8 );
    var_12.redployanywhere = modevalidatekillstreakslot( "jeep", 10 );
    var_12.safehouse_vo_start = modevalidatekillstreakslot( "little_bird", 12 );
    var_12._id_125EA = modevalidatekillstreakslot( "tac_rover", 14 );
    var_12.sortbylastzombietime = modevalidatekillstreakslot( "motorcycle", 16 );
    var_12.br_squadrevivestatus = modify_blast_shield_damage( "atv" );
    var_12._id_125EE = modify_blast_shield_damage( "tac_rover" );
    var_12.saw_headicons = modify_blast_shield_damage( "little_bird" );
    var_12.spawn_ai_group = modify_blast_shield_damage( "motorcycle" );
...
lookupcurrentoperator( var_0 )
{
    ...
    else if ( !isdefined( level.playercustomizationdata[var_4][var_0] ) )
    {
        ...
        if ( isai( self ) )
            ...
            var_6.operatorref = self getplayerdata( level.loadoutsgroup, "customizationSetup", "operators", var_1 );
    ...
    if ( getdvarint( "scr_forceHeadlessCustomization", 1 ) == 1 && !isagent( self ) && self isplayerheadless() )
    {
        ...
        if ( !isdefined( self.off_max ) )
        {
            foreach ( var_9 in var_7 )
            {
                ...
                if ( !isdefined( level.ogangles ) )
                    level.ogangles = [];

                if ( !isdefined( level.ogangles[var_9] ) || level.ogangles[var_9] > var_10.size )
                    level.ogangles[var_9] = 0;

                for ( var_11 = var_10[level.ogangles[var_9]]; isdefined( var_11 ) && ( var_11 == "default_western" || var_11 == "default_eastern" ); var_11 = var_10[level.ogangles[var_9]] )
                {
                    level.ogangles[var_9] = level.ogangles[var_9] + 1;

                    if ( level.ogangles[var_9] > var_10.size )
                        level.ogangles[var_9] = 0;
                }
...
lookupcurrentoperatorskin( var_0 )
{
    ...
    else
    {
        if ( getdvarint( "scr_forceHeadlessCustomization", 1 ) == 1 && !isagent( self ) && self isplayerheadless() )
        {
            ...
            {
                if ( !isdefined( level.og_mbradial ) )
                    playersetomnvarattraction();

                var_5 = level.og_mbradial[var_1]["curIndex"];
                level.playercustomizationdata[var_3][var_0].operatorskinindex = level.og_mbradial[var_1]["lootIDs"][var_5];
                level.og_mbradial[var_1]["curIndex"] = level.og_mbradial[var_1]["curIndex"] + 1;

                if ( level.og_mbradial[var_1]["curIndex"] >= level.og_mbradial[var_1]["maxIndex"] )
                    level.og_mbradial[var_1]["curIndex"] = 0;
            }
        }
        else if ( !isdefined( level.playercustomizationdata[var_3][var_0].operatorskinindex ) )
        {
            if ( isai( self ) )
            {
                if ( !isdefined( self.botskinid ) )
                    check_trigger_spawnflags( var_1 );
...
getoperatorcustomization()
{
    ...
    if ( isdefined( level.shouldcheckcustomclosetoscorelimit ) )
        [var_0, var_1] = [[ level.shouldcheckcustomclosetoscorelimit ]]( self, var_0, var_1 );
    ...
    if ( ( var_0 == "default_western" || var_0 == "default_eastern" ) && ( var_1 == 274 || var_1 == 275 ) )
    {
        ...
        var_3 = level.defaultoperatorskins[var_8]["body"][self.pers["defaultOperatorSkinIndex"]];

        if ( !isdefined( self.pers["defaultOperatorHeadIndex"] ) )
            self.pers["defaultOperatorHeadIndex"] = scripts\engine\utility::random( level.defaultoperatorskins[var_8]["head"][self.pers["defaultOperatorSkinIndex"]] );

        var_4 = self.pers["defaultOperatorHeadIndex"];
...
checkforafk()
{
    ...
    if ( istrue( self.obj_room_fire_01 ) )
        return;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && istrue( self.client_hint ) )
    {
    ...
    if ( scripts\mp\utility\game::isroundbased() && scripts\mp\utility\game::getgametype() != "ctf" )
    {
        ...
        if ( var_0 && !isgamebattlematch() && !istrue( self.apc_rus_adjustdriverturretammo ) )
        {
            self.apc_rus_adjustdriverturretammo = 1;
            self.pers["roundsAFK"]++;

            if ( self.pers["roundsAFK"] > 1 )
                level thread relic_award_bullets( self );
        }
...
getjointeampermissions( var_0 )
{
    ...
    if ( level.maxteamsize == 0 || var_1 < level.maxteamsize )
        ...
        return 1;
    else if ( scripts\mp\menus::binoculars_onstatemarkedupdate() )
        return 1;
    else if ( scripts\mp\utility\game::getgametype() == "br" && ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" ) )
        return 1;
    else
    {
...
{
    level endon( "game_ended" );
    scripts\engine\utility::_id_12E3F( "death_or_disconnect", "joined_team" );
...
{
    level endon( "game_ended" );
    scripts\engine\utility::_id_12E40( "death_or_disconnect", "joined_team", "joined_squad" );
    scripts\mp\utility\outline::outlinedisable( var_0, self );
}
...
getoperatorexecution( var_0 )
{
    ...
    else
    {
        ...
        if ( var_1 > 0 )
            var_2 = tablelookup( "mp_cp/executiontable.csv", 0, var_1, 1 );

        if ( var_2 == "" )
            var_2 = tablelookup( "operators.csv", 1, var_0, 24 );
...
maxnumsites( var_0 )
{
    ...
    if ( var_1 == 0 )
        self.scorelimitreached = tablelookup( "operators.csv", 1, var_0, 23 );
    else
        self.scorelimitreached = tablelookup( "operatorquips.csv", 0, var_1, 6 );

    return self.scorelimitreached;
}

maxperkbonustier( var_0 )
{
    self.scorerequiresbanking = tablelookup( "mp_cp/executiontable.csv", 1, self.loadoutexecution, 19 );
    return self.scorerequiresbanking;
}
...
modevalidatekillstreakslot( var_0, var_1 )
{
    var_2 = 0;
...
playersetomnvarattraction()
{
    ...
    for ( var_1 = 0; var_1 < var_0; var_1++ )
    {
        if ( tablelookupbyrow( "operatorskins.csv", var_1, 18 ) != "" )
        {
            ...
            if ( !isdefined( level.og_mbradial[var_2] ) )
            {
                level.og_mbradial[var_2]["lootIDs"] = [];
                level.og_mbradial[var_2]["curIndex"] = 0;
                level.og_mbradial[var_2]["maxIndex"] = 0;
            }

            level.og_mbradial[var_2]["lootIDs"][level.og_mbradial[var_2]["lootIDs"].size] = int( var_3 );
            level.og_mbradial[var_2]["maxIndex"] = level.og_mbradial[var_2]["maxIndex"] + 1;
        }
    }
}
...
initoperatorcustomization()
{
    ...
    for (;;)
    {
        ...
        var_5 = maxplunderdropondeath( var_1 );
    ...
    for (;;)
    {
        var_7 = int( tablelookupbyrow( "operatorskins.csv", var_6, 20 ) );
        var_8 = tablelookupbyrow( "operatorskins.csv", var_6, 1 );
        ...
        if ( var_7 )
        {
            var_1 = tablelookupbyrow( "operatorskins.csv", var_6, 2 );
            var_9 = tablelookupbyrow( "operatorskins.csv", var_6, 4 );
            var_10 = tablelookupbyrow( "operatorskins.csv", var_6, 5 );
            ...
            var_11[0] = var_9;
            var_11[1] = var_10;
            level.operatorcustomization[var_3][var_1][var_8] = var_11;
        }
    ...
    if ( getdvarint( "scr_customization_missing_skins_fixup", 1 ) == 1 )
    {
        ...
        for (;;)
        {
            ...
            if ( var_4 && isdefined( level.operatorcustomization[var_3][var_1] ) && level.operatorcustomization[var_3][var_1].size == 0 )
                level.operatorcustomization[var_3][var_1] = undefined;
    ...
    if ( !var_1 )
    {
        ...
        if ( !isdefined( level.playercustomizationdata[var_2][self.team] ) )
        {
            ...
            if ( isai( self ) )
                ...
                var_4.operatorskinindex = self getplayerdata( level.loadoutsgroup, "customizationSetup", "operatorCustomization", var_0, "skin" );
...
total_puddle_count()
{
    ...
    if ( istrue( level.onlinegame ) )
    {
        ...
        if ( isdefined( var_2 ) && var_2.size > 0 )
        {
            foreach ( var_4 in var_2 )
            {
                ...
                {
                    var_0 = var_4.team;
                    self._id_1269D = var_4._id_1269D;
                    break;
                }
            }
        }
    ...
    if ( !isdefined( var_0 ) )
        var_0 = is_main_pilot( var_1 );

    if ( !isdefined( self._id_1269D ) )
        self._id_1269D = gettime();
...
is_main_pilot( var_0 )
{
    ...
    foreach ( var_3 in level.teamnamelist )
    {
        ...
        if ( var_4 > 0 )
        {
            ...
            if ( isdefined( var_6[0]._id_1269D ) && gettime() > var_6[0]._id_1269D + 300000 )
                continue;
        }
...
_id_11FEB()
{
    return getdvarint( "scr_br_zombie_encounters", 0 ) > 0 || scripts\mp\utility\game::chopperexfil_sh060_start();
}
