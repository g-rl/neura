...
recharge_equipment_update_state( var_0 )
{
    if ( var_0 == modsforclass() && ( isdefined( level._id_130BD ) || chopperexfil_sh060_start() ) )
        return 1;
...
{
    return getdvarint( "scr_br_use_agents", 0 ) > 0 || istrue( level.cleanupallbutxcratesforteam );
}
...
nextroundisfinalround()
{
    ...
    {
        if ( level recharge_equipment_update_state( var_2 ) )
            continue;
...
{
    return level.systemlink || privatematch() && getdvarint( "NQORMNOQQM", 0 ) || _func_422();
}
...
isanymlgmatch()
{
    ...
    return getdvarint( "LOMTKQTRTM" ) || istrue( level.dialog_kill_watcher_civ );
}

ismlgsystemlink()
{
    if ( level.systemlink && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.dialog_kill_watcher_civ ) ) )
        return 1;
...
ismlgprivatematch()
{
    if ( privatematch() && ( getdvarint( "LOMTKQTRTM" ) || istrue( level.dialog_kill_watcher_civ ) ) )
        return 1;
...
{
    var_0 = getgametype();
    var_1 = missions_clearinappropriaterewards();
    return var_0 != var_1;
}

reaper_waitingformissilereload()
{
    if ( missions_clearinappropriaterewards() == "br" )
        return 1;
    else if ( missions_clearinappropriaterewards() == "jugg" )
        return 1;
...
matchdata_loggameevent()
{
    level.tag_usb_with_head_icon = 5;
    return level.tag_usb_with_head_icon;
}

matchdata_logmultikill()
{
    if ( !isdefined( level.tags_used ) )
        level.tags_used = getdvarint( "scr_br_payload_retreat_time", 30 );

    return level.tags_used;
}

matchdata_logkillstreakevent()
{
    if ( !isdefined( level.tagautopickup ) )
        level.tagautopickup = getdvarint( "scr_br_payload_redraw_time", 10 );

    return level.tagautopickup;
}
...
scriptable_token_scriptable_touched_callback( var_0, var_1, var_2, var_3, var_4 )
{
...
race_set_checkpoint()
{
    return istrue( level.race_requires_vehicle );
}

propwaitminigamehudinit()
{
    return istrue( level.propwaitminigameinit ) || istrue( level.propwaitminigamerun ) || istrue( level.propwaitminigamehudsetpoint );
}
...
delay_delete_alerted_icon( var_0 )
{
    var_1 = 0;
    var_2 = 0;
    var_3 = scripts\cp_mp\utility\game_utility::getmapname();

    if ( ( getgametype() == "arm" || propwaitminigamehudinit() ) && rear_spotlight_target_found_dist_sq() || getdvarint( "scr_game_forceEnableParachuteCut", 0 ) == 1 )
    {
        if ( var_3 != "mp_aniyah" )
        {
            ...
            if ( istrue( level.targetsshot ) && !istrue( var_0.getvolength ) )
                var_2 = 1;
        }
    }
...
randomize_tv_station_player_start_pos()
{
...
dynamic_door()
{
    ...
    else if ( isdefined( level.munition_source_getridof ) && level.munition_source_getridof > 0 )
        return 1;

    return 0;
}

randomly_play_tape_reel_anims()
{
    return getdvarint( "scr_enable_server_halloween_content", 0 ) != 0;
}

race_is_player_driving_vehicle()
{
    switch ( getgametype() )
    {
        case "brtdm":
            return 0;
        case "br":
            switch ( missions_clearinappropriaterewards() )
            {
                case "kingslayer":
                case "risk":
                case "dmz":
