...
init()
{
    var_0 = scripts\mp\utility\dvars::maxtagradiussq( "scr_" + scripts\mp\utility\game::getgametype() + "_healthregentime", "scr_player_healthregentime" );
...
healhregenthink( var_0 )
{
    ...
    if ( var_2 < 0.55 && !scripts\mp\utility\player::isusingremote() && !istrue( self.play_counter_beep_sfx_on_bomb_vests ) )
        onenterdeathsdoor();
    ...
    for (;;)
    {
        ...
        if ( isdefined( level.x1questmanager ) )
            var_7 = [[ level.x1questmanager ]]( var_7 );
    ...
    if ( var_6 )
    {
        ...
        scripts\cp\helicopter\chopper_boss::_id_11A49( "t9_ch_global_stim_shot_health_recovery_for_operator_mission", var_10 );
        scripts\cp\helicopter\chopper_boss::_id_11A49( "t9_ch_global_stim_shot_health_recovery_for_operator_mission_s3", var_10 );
        scripts\cp\helicopter\chopper_boss::supersbyextraweapon( var_10 );
        scripts\mp\analyticslog::logevent_playerhealed( self, var_10, self );
    }
    else if ( !isai( self ) )
        scripts\mp\analyticslog::scriptcircleat( self, var_10 );
...
waitforhealthregendelay()
{
    ...
    while ( self.currentregendelay - self.regendelayreduction > 0 )
    {
        ...
        if ( isdefined( level.x1playertransition ) )
            var_5 = [[ level.x1playertransition ]]( var_5 );
...
applyshrapnelfxinternal()
{
    ...
    level endon( "game_ended" );
    var_0 = scripts\engine\utility::_id_12E48( "shrapnel_ended_early", "shrapnel_ended", "force_regeneration" );
