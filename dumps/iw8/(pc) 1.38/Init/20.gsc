...
main()
{
    ...
    if ( isdefined( level.challengestarttime ) )
        [[ level.challengestarttime ]]();
    else
    ...
    foreach ( var_1 in level.players )
    {
        if ( isbot( var_1 ) )
        {
            if ( isalive( var_1 ) )
            {
                ...
                var_1.check_for_moody_traversal = 1;
...
{
    var_0 scripts\engine\utility::_id_12E3F( "bots_human_spawned", "bots_human_disconnected" );
    ...
    for (;;)
    {
        ...
        if ( var_46 )
        {
            ...
            if ( var_45 > 0 && var_44 > 0 )
                level scripts\engine\utility::_id_12E39( "spawned_enemies", "spawned_allies" );
            else if ( var_45 > 0 )
                level waittill( "spawned_enemies" );
            else if ( var_44 > 0 )
...
bot_think()
{
    ...
    if ( !isdefined( self.check_for_moody_traversal ) )
    {
        var_1 = 1;
        self.check_for_moody_traversal = 1;
    ...
    for (;;)
    {
        ...
        scripts\mp\bots\bots_loadout::change_stealth_state_to_combat_warpper();
        change_yaw_angle();
...
bot_think_seek_dropped_weapons()
{
    ...
    for (;;)
    {
        ...
        if ( scripts\mp\bots\bots_util::cashleaderbag_detach( 0.33 ) )
        {
...
bot_crate_valid( var_0 )
{
    ...
    else if ( celebration_end( var_0 ) )
        return 0;

    return isdefined( var_0 );
}

celebration_end( var_0 )
{
    return 0;
}
...
revive_watch_for_finished( var_0 )
{
    ...
    self endon( "bad_path" );
    self endon( "goal" );
    var_0 scripts\engine\utility::_id_12E3F( "death", "revived" );
...
handle_smoke()
{
    ...
    thread flag_on_vip_cash_collect( var_0 );
}
