...
autoattachtoplayer()
{
    ...
    level.respawnfade = game["lastSpectatedPlayer"];

    for (;;)
    {
        var_0 = undefined;

        if ( !self ismlgspectator() || self isspectatingplayer() || self ismlgfreecamenabled() || self ismlgaerialcamenabled() )
            break;

        if ( level.players.size > 1 )
        {
            if ( level.currentround > 1 && isdefined( level.respawnfade ) && self.team != "follower" )
                var_0 = level.respawnfade;
            else if ( self.team == "follower" )
            {
                var_1 = self getfollowedplayer();
                var_2 = 0;
...
updatemlgspectator()
{
    ...
    for (;;)
    {
        if ( self ismlgspectator() && self isspectatingplayer() )
        {
            ...
            if ( isdefined( var_0 ) )
                level.respawnfade = var_0.clientid;
        }
...
_id_11C5B()
{
    ...
    game["lastSpectatedPlayer"] = level.respawnfade;
}
