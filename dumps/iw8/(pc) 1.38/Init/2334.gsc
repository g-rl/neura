...
tryuseautosentry( var_0, var_1 )
{
    ...
    if ( var_2 )
        scripts\common\utility::_id_12965( level.set_ui_omnvar_for_relics, level.sentrysettings["sentry_minigun"].streakname, self.origin );
...
tryusesam( var_0, var_1 )
{
    ...
    if ( var_2 )
        scripts\common\utility::_id_12965( level.set_ui_omnvar_for_relics, level.sentrysettings["sam_turret"].streakname, self.origin );
...
tryuseshocksentry( var_0 )
{
    ...
    if ( var_1 )
        scripts\common\utility::_id_12965( level.set_ui_omnvar_for_relics, var_0.streakname, self.origin );
    else
        waitframe();
...
setcarryingsentry( var_0, var_1, var_2, var_3 )
{
    ...
    for (;;)
    {
        var_4 = scripts\engine\utility::_id_12E49( "place_sentry", "cancel_sentry", "force_cancel_placement", "emp_applied" );
...
sentry_oncarrierdeathoremp( var_0, var_1 )
{
    ...
    var_0 scripts\engine\utility::_id_12E3F( "death", "emp_applied" );
...
sentry_oncarrierchangedteam( var_0 )
{
    self endon( "placed" );
    self endon( "death" );
    var_0 scripts\engine\utility::_id_12E3F( "joined_team", "joined_spectators" );
    self delete();
}
...
sam_acquiretarget()
{
    ...
    if ( !isdefined( self.samtargetent ) )
    {
        ...
        {
            var_1 = [];
            var_2 = scripts\mp\utility\teams::getenemyteams( self.team );
            ...
            {
                if ( isdefined( var_6.isleaving ) && var_6.isleaving )
                    continue;

                if ( sighttracepassed( var_0, var_6.origin, 0, self ) )
                    return var_6;
            }
            ...
            foreach ( var_6 in level.remote_uav )
            {
                if ( !isdefined( var_6 ) )
                    continue;
                ...
                if ( sighttracepassed( var_0, var_6.origin, 0, self, var_6 ) )
                    return var_6;
            }
        }
        else
        {
            foreach ( var_6 in level.uavmodels )
            {
                if ( isdefined( var_6.isleaving ) && var_6.isleaving )
                    continue;
...
sentryshocktargets()
{
    ...
    for (;;)
    {
        var_0 = scripts\engine\utility::_id_12E53( 1, "turret_on_target" );
...
searchforshocksentryairtarget()
{
    ...
    if ( isdefined( level.helis ) )
    {
        foreach ( var_10 in level.helis )
        {
            ...
            if ( targetvisibleinfront( var_10 ) )
                return var_10;
        }
    }
}
...
marktargetlaser( var_0 )
{
    ...
    self laseron();
    self.laser_on = 1;
    scripts\engine\utility::_id_12E3F( "done_firing", "carried" );
    self laseroff();
    self.laser_on = 0;
}
