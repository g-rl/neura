...
init()
{
    ...
    level.setburningdamage = getdvarint( "scr_ks_jugg_team_max", 0 );
}

jugg_makejuggernaut( var_0, var_1 )
{
    var_2 = register_chopper_boss_combat_action( var_0 );
    ...
    if ( isdefined( level.endac130infilanimsinternal ) )
        self [[ level.endac130infilanimsinternal ]]();
    ...
    if ( istrue( self.trackriotshield_monitorshieldattach ) )
    {
        scripts\mp\playeractions::allowactionset( "fakeJugg", 1 );
        self.trackriotshield_monitorshieldattach = undefined;
    }
    ...
    else
    {
        ...
        foreach ( var_11 in var_9 )
        {
            ...
            if ( regendelayspeedfunc() )
            {
                self givemaxammo( var_11 );
                thread register_grenade_settings_for_module( var_11 );
            }
        }
    }
    ...
    thread register_script_model_animation();
    ...
    thread register_valid_objectives_for_create_script();

    if ( regendelayreduce_delayedregen() )
    {
        self skydive_setbasejumpingstatus( 1 );
        self skydive_setdeploymentstatus( 1 );
    }

    if ( isdefined( level.ammobox_getbufferedattachmentweapon ) )
        self [[ level.ammobox_getbufferedattachmentweapon ]]();
...
jugg_createconfig( var_0, var_1 )
{
    ...
    var_2.setheadicon_removeclientfrommask = 5;

    if ( level.gametype == "br" )
        var_2.setheadicon_removeclientfrommask = 3;
...
register_script_model_animation()
{
    if ( regen_delay() )
    {
        ...
        if ( var_0 )
        {
            ...
            if ( register_fuel_stability_event( var_0 ) )
                var_1 = "flames";
...
jugg_getjuggmodels()
{
    ...
    if ( regen_delay() )
    {
        ...
        if ( var_1 )
        {
            if ( register_fuel_stability_event( var_1 ) )
            {
...
register_chopper_boss_combat_action( var_0 )
{
    var_1 = undefined;
    ...
    if ( var_2 != "stand" )
    {
        ...
        else if ( var_2 == "crouch" )
        {
            if ( !var_3 )
            {
                var_6 = scripts\engine\trace::ray_trace_passed( self.origin, scripts\mp\utility\player::missing_window_blockers( "stand" ), self, var_5 );
        ...
        else if ( var_2 == "prone" )
        {
            if ( !var_4 )
            {
                if ( var_3 )
                {
                    var_7 = scripts\engine\trace::ray_trace_passed( self.origin, scripts\mp\utility\player::missing_window_blockers( "crouch" ), self, var_5 );
...
jugg_watchoverlaydamagestates( var_0 )
{
    ...
    for (;;)
    {
        scripts\engine\utility::_id_12E3F( "damage", "jugg_health_regen" );
...
regendelayspeedfunc()
{
    return istrue( self.juggcontext.juggconfig._id_12B94 );
}

regenhealthaddfunc()
{
    return istrue( self.juggcontext.juggconfig._id_12BA4 );
}

register_grenade_settings_for_module( var_0 )
{
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
    var_1 = self getweaponammostock( var_0 );
    thread register_vehicle_as_ambient( var_0, var_1 );
    thread register_sequence_4_objectives( var_0, var_1 );
...
register_vehicle_as_ambient( var_0, var_1 )
{
    self endon( "death_or_disconnect" );
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
...
refreshsquadspawnareaomnvars()
{
    ...
    thread register_valid_gametypes_for_create_script();
}
...
register_valid_objectives_for_create_script()
{
    self endon( "death_or_disconnect" );
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
    var_0 = self.juggcontext.juggconfig;
    var_0.originalownerteam = 0;

    for (;;)
    {
        self waittill( "perform_hero_drop" );

        if ( !istrue( var_0.originalownerteam ) )
        {
            var_0.originalownerteam = 1;
            self radiusdamage( self.origin, 500, 2000, 500, self, "MOD_CRUSH" );
            thread register_seat_data( var_0 );
        }
    }
}

register_seat_data( var_0 )
{
    self endon( "death_or_disconnect" );
    self endon( "juggernaut_end" );
    level endon( "game_ended" );
...
register_jugg_maze_objectives( var_0, var_1 )
{
    ...
    {
        var_4 = 7;
        var_5 = var_3.setheadicon_removeclientfrommask;
    }
...
register_jugg_maze_interactions( var_0 )
{
    return int( var_0 / 2 );
}
...
{
    return istrue( level.ninetypercent_music ) && var_0 == 1;
}

br_pickupdenyparachuting()
{
    if ( istrue( level.setburningdamage ) && level.activejuggernauts.size > 0 )
    {
        var_0 = 0;
        ...
        for ( var_2 = 0; var_2 < var_1; var_2++ )
        {
            ...
            if ( var_0 >= level.setburningdamage )
                return 1;
        }
    }
