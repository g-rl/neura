...
init()
{
    ...
    level.laststandhealth = scripts\mp\utility\dvars::maxtagsvisible( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandHealth", "scr_player_lastStandHealth" );
    level.laststandrevivehealth = scripts\mp\utility\dvars::maxtagsvisible( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveHealth", "scr_player_lastStandReviveHealth" );
    ...
    level.laststandinvulntime = scripts\mp\utility\dvars::maxtagradiussq( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandInvulnTime", "scr_player_lastStandInvulnTime" );
    level.retrieve_data_objective = scripts\mp\utility\dvars::maxtagradiussq( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveDecayScale", "scr_player_lastStandReviveDecayScale" );
    level.laststandrevivetimer = scripts\mp\utility\dvars::maxtagradiussq( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandReviveTimer", "scr_player_lastStandReviveTimer" );
    level.laststandsuicidetimer = scripts\mp\utility\dvars::maxtagradiussq( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandSuicideTimer", "scr_player_lastStandSuicideTimer" );
    level.laststandtimer = scripts\mp\utility\dvars::maxtagradiussq( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandTimer", "scr_player_lastStandTimer" );
    level.return_enemy_type_mask = scripts\mp\utility\dvars::maxtagradius( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandWeapon", "scr_player_lastStandWeapon" );
    level.return_num_if_under_vehicle_cap = scripts\mp\utility\dvars::maxtagradiussq( "scr_" + scripts\mp\utility\game::getgametype() + "_lastStandWeaponDelay", "scr_player_lastStandWeaponDelay" );
...
laststandthink()
{
    ...
    onenter();
    var_0 = scripts\engine\utility::_id_12E52( "last_stand_heal_success", "last_stand_revived", "last_stand_bleedout", "death", "disconnect", "last_stand_self_revive" );
...
onenter()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
    {
        ...
        {
            self.laststandbegintime = gettime();
            scripts\mp\gametypes\br_gametypes.gsc::_id_11BE4( "onLastStandEnter" );
            var_1 = scripts\mp\gametypes\br_public.gsc::minigunbackup( self.team, self.squadindex );
    ...
    var_5 = level.return_enemy_type_mask;
    ...
    else
    {
        ...
        if ( isdefined( level.run_debug_start_objective ) )
            self thread [[ level.run_debug_start_objective ]]();
        ...
        thread nuke_killplayer( var_5 );

        if ( isdefined( level.ammobox_onplayerholduse ) )
            self [[ level.ammobox_onplayerholduse ]]();
        else
            addoverheadicon();
        ...
        if ( isdefined( self.respawntagvisibility ) && self.respawntagvisibility scripts\mp\utility\perk::_hasperk( "specialty_reduce_regen_delay_on_kill" ) )
            self.respawntagvisibility scripts\mp\perks\perkfunctions::regendelayreduce_onkill();
        ...
        if ( isdefined( self.pers["squadMemberIndex"] ) )
        {
            ...
            self.retreat_vehicle_collision_clear = scripts\mp\utility\outline::outlineenableforteam( self, self.team, var_7, "laststand" );
        }
...
{
    if ( !istrue( self.get_circle_back_start_node ) )
    {
        self.get_circle_back_start_node = 1;
...
nuke_killplayer( var_0 )
{
    ...
    {
        var_2 = level.return_num_if_under_vehicle_cap;
...
disableweaponsovertime( var_0 )
{
    ...
    scripts\engine\utility::_id_12E53( var_0, "death_or_disconnect" );
...
dodamagewhiledown()
{
    ...
    if ( !istrue( level.retry_no_votes ) )
        thread suicidesetup();
    ...
    while ( self.health > 0 )
    {
        ...
        if ( !istrue( scripts\mp\playerlogic::lootchopper_setupdamagefunctionality() ) )
        {
            var_5 = int( var_4 + var_2 ) - int( var_4 );
            var_4 = var_4 + var_2;
            self.punishwavechaseplayers = 1;
            self dodamage( var_5, self.origin, self, undefined, "MOD_TRIGGER_HURT", var_3, "none" );
            self.punishwavechaseplayers = undefined;
        }
...
_id_11D8A()
{
    ...
    for (;;)
    {
        if ( _id_11D88( var_1 ) && !istrue( var_1.rallypoint_showafterprematch ) && var_1 isonground() )
        {
            var_0 notify( "self_revive_start" );
            var_1 setlaststandselfreviving( 1 );
            var_0 thread _id_11D89();
        }
...
_id_11D89()
{
    ...
    var_1._id_12E25 = 0;
    var_1 thread _id_11D87();

    while ( scripts\mp\utility\player::isreallyalive( var_0 ) && _id_11D88( var_0 ) && var_1.curprogress < var_1.usetime )
    {
        while ( !var_0 isonground() && _id_11D88( var_0 ) && !istrue( var_0.rallypoint_showafterprematch ) )
            waitframe();
        ...
        if ( !istrue( var_0.rallypoint_showafterprematch ) )
        {
            var_0 thread scripts\mp\gametypes\br_public.gsc::updatelocationbesttimehud( "iw8_ges_plyr_self_revive", 10 );

            if ( !istrue( var_0._id_124D7 ) )
            {
                var_0 attach( "offhand_wm_stim", "tag_accessory_left" );
                var_0._id_124D7 = 1;
            }

            var_0.rallypoint_showafterprematch = 1;
        ...
        if ( var_1.curprogress >= var_1.usetime )
        {
            ...
            if ( scripts\mp\utility\game::getgametype() == "br" )
                scripts\mp\gametypes\br_analytics.gsc::circlesettingsassert( var_0 );

            var_0 is_same_combat_action( "self_revive_success", var_0 );

            if ( isdefined( level._id_11A33 ) )
                var_0 [[ level._id_11A33 ]]();
    ...
    if ( !istrue( var_0.beingrevived ) )
    {
        scripts\mp\gametypes\br.gsc::_id_12B2D( var_0, var_0, var_1.curprogress, 1 );
...
_id_11D87()
{
    ...
    var_0 scripts\engine\utility::_id_12E4F( "last_stand_finished", "stopped_self_revive", "death_or_disconnect" );
    ...
    var_0.rallypoint_showafterprematch = 0;
}
...
onexitcommon( var_0 )
{
    ...
    var_1 = level.return_enemy_type_mask;
    ...
    if ( istrue( scripts\mp\playerlogic::lootchopper_setupdamagefunctionality() ) )
        scripts\mp\playerlogic::_id_11E85( 0 );

    if ( level.gametype == "br" )
    {
        scripts\mp\gametypes\br.gsc::_id_12B2D( self, self, 0, 1 );
    ...
    if ( isdefined( level.shouldpickup ) )
        self [[ level.shouldpickup ]]( var_0 );
...
onrevive( var_0, var_1 )
{
    ...
    scripts\common\utility::bhasthermitestucktoshield( 1 );

    if ( getdvar( "scr_br_gametype", "" ) == "dmz" || getdvar( "scr_br_gametype", "" ) == "risk" )
        scripts\mp\gametypes\br_plunder.gsc::vehicle_occupancy_errormessage( self, 1, 1 );
    ...
    var_3 = level.return_enemy_type_mask;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" && !istrue( self.gulag ) )
    {
        ...
        var_5 = scripts\mp\gametypes\br_public.gsc::minigunbackup( self.team, self.squadindex );
    ...
    self.respawntokenenabled = undefined;
...
onbleedout()
{
    ...
    if ( level.gametype == "br" )
    {
        ...
        {
            self.get_circle_back_start_node = 0;
...
ondeath( var_0 )
{
    ...
    if ( isdefined( var_0 ) && isdefined( self.respawntagvisibility ) && istrue( self.respawntagvisibility.inlaststand ) )
        self.respawntagvisibility thread streamhintenabled( var_0, self.restoreweaponstates, self.return_same_module_as_next_module );

    if ( istrue( self.rallypoint_showafterprematch ) )
...
revivesetup( var_0 )
{
    ...
    if ( var_0 scripts\mp\gametypes\br_public.gsc::obj_room_fire_06() )
        var_0.laststandreviveent _id_11D8A();
}
...
_id_11D30()
{
    ...
    var_11 = var_2.respawntagvisibility;
    ...
    if ( scripts\mp\utility\game::getgametype() == "br" )
        scripts\mp\gametypes\br_analytics.gsc::circlerunning( var_0, var_11, var_2, var_5, var_6, var_7, var_8, var_12, var_13, var_14, var_16 );

    var_0 is_same_combat_action( "self_revive_on_kill_success", var_0 );
...
streamhintenabled( var_0, var_1, var_2 )
{
    var_3 = var_0.victim;
...
{
    self endon( "death" );
    self.owner scripts\engine\utility::_id_12E40( "death_or_disconnect", "last_stand_finished", "last_stand_heal_active" );
    self delete();
}
...
revivetriggerthink( var_0 )
{
    ...
    for (;;)
    {
        ...
        if ( istrue( var_1.playlandingbreath ) )
        {
            ...
            while ( istrue( var_1.playlandingbreath ) && var_1 usebuttonpressed() )
                waitframe();
        }
        ...
        self.owner scripts\mp\playerlogic::_id_11E85( 1 );
        var_2 = 0;
        ...
        var_1 thread _id_125C9( self.owner );

        if ( level.gametype == "br" )
            self.owner scripts\mp\gametypes\br_public.gsc::_id_11EDA( 1 );

        var_3 = scripts\engine\utility::_id_12E4F( "use_hold_revive_success", "use_hold_revive_fail", "death_or_disconnect" );

        if ( var_3 == "use_hold_revive_success" )
            var_2 = 1;

        var_1 notify( "finish_buddy_reviving" );
        self.owner scripts\mp\playerlogic::_id_11E85( 0 );
        var_2 = self.owner is_same_combat_action( var_3, var_1 );
...
_id_125C9( var_0 )
{
    ...
    var_3._id_11B12 = var_2;

    for ( var_4 = 0; !var_4 || istrue( var_1.calloutmarkerping_initcommon ); var_1._id_12E26 = 1 )
    {
        var_4 = var_1 scripts\cp_mp\killstreaks\killstreakdeploy::switchtodeployweapon( var_2, var_3, ::_id_12689, undefined, ::support_box_get_open_anim_length, undefined, ::support_box_is_scriptable_model, 0 );
        waitframe();
    }

    var_1.calloutmarkerping_initcommon = 1;
    var_1._id_12E26 = 0;
    var_1 thread _id_12F2B();
    var_1 thread _id_12F46( var_0 );
    var_1 thread _id_12F47( var_0 );
}

support_box_get_open_anim_length( var_0, var_1 )
{
    var_2 = self;
    var_2 disableweaponswitch();
}

support_box_is_scriptable_model( var_0, var_1 )
{
    var_2 = self;
    ...
    waitframe();
    var_2.calloutmarkerping_initcommon = 0;
}
...
_id_12689( var_0 )
{
    ...
    if ( !istrue( var_1._id_11B13 ) )
        return;

    var_1 scripts\engine\utility::_id_12E4F( "death_or_disconnect", "finish_buddy_reviving", "last_stand_start" );
}
...
is_same_combat_action( var_0, var_1 )
{
    var_2 = self;
    var_3 = 0;
    ...
    var_2.rallypoint_showafterprematch = 0;
    ...
    if ( level.gametype == "br" )
    {
        ...
        var_2 scripts\mp\gametypes\br_public.gsc::_id_11EDA( 0 );
        var_2 scripts\mp\gametypes\br_public.gsc::_id_11EE3( 0 );
    }
    ...
    if ( var_3 )
    {
        ...
        if ( level.gametype == "br" )
        {
            if ( !istrue( level.gameended ) )
            {
                var_2 scripts\mp\gametypes\br_public.gsc::_id_11EDC( 0 );
                scripts\mp\events::revivedplayer( var_1, var_2 );
                scripts\cp\helicopter\chopper_boss::streampoint( var_1, var_2 );
                var_2 scripts\cp\helicopter\chopper_boss::_id_12478( "alive_not_downed" );

                if ( var_1 != var_2 )
                    var_1 scripts\mp\utility\stats::incpersstat( "rescues", 1 );

                var_2 scripts\mp\gametypes\br_gametypes.gsc::_id_11BE4( "onLastStandRevive", var_1 );
                var_2.get_circle_back_start_node = 0;
...
decayreviveprogress()
{
    ...
    level endon( "game_ended" );
    var_0 = level.retrieve_data_objective;
...
useholdthinkcleanup( var_0, var_1 )
{
    ...
    var_3 = self.owner;
    var_4 = var_3 scripts\engine\utility::_id_12E50( "death_or_disconnect", "use_hold_think_success", "use_hold_think_fail", "last_stand_finished" );
    self.inuse = 0;
    var_1 delete();
...
suicidesetup()
{
    ...
    for (;;)
    {
        ...
        if ( self stancebuttonpressed() && self isinexecutionvictim() == 0 && !scripts\mp\playerlogic::lootchopper_setupdamagefunctionality() )
        {
...
_id_1257D()
{
    ...
    while ( !var_4 && !var_1 )
    {
        ...
        var_5 = scripts\engine\utility::_id_12E57( 0.5, "stance_pressed_down", "stance_pressed_up", "last_stand_finished", "last_stand_self_revive", "last_stand_bleedout" );
...
handlerevivemessage()
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            var_2 = scripts\engine\utility::_id_12E4E( "super_ready", "handle_revive_message" );
...
laststandmonitor()
{
    ...
    if ( istrue( level.restorekillstreakplayerangles ) )
        return;
