...
molotov_init()
{
    var_0 = spawnstruct();
    level.molotov = var_0;
    var_0.setdropbagdelay = getdvarint( "scr_molotovMaxPools", 100 );
    var_0.setarenaomnvarhealthtype = getdvarint( "scr_molotovMaxCastsPerFrame", 8 );
    var_0.play_extract_3mins = getdvarint( "scr_molotovInstantCleanup", 1 ) > 0;
    var_0._id_12A44 = 0;
    var_0.vehicleunlink = [];
    var_0.scriptables = [];
    var_0.triggers = [];
...
molotov_init_cast_data()
{
    var_0 = siege_bot_team_triple_cap_check();
...
molotov_used( var_0 )
{
    ...
    var_4 = self.name;
    var_0 waittill( "missile_stuck", var_5 );
    level notify( "grenade_exploded_during_stealth", var_0, "molotov_mp", var_4 );
    var_6 = ( gettime() - var_2 ) / 1000;
    var_7 = var_3 + ( 0, 0, -800 * var_6 );

    if ( isplayer( self ) )
    {
        self.ai_weapons_free = landing_damage_watcher();
        thread _id_12ED3( self.ai_weapons_free );
    }
...
assault3_hangar( var_0, var_1 )
{
    var_2 = gettime();
    var_3 = var_0.origin;
    var_4 = var_0.angles;
    var_1 waittill( "missile_stuck", var_5, var_6, var_7, var_8, var_9, var_10 );
...
_id_12ED3( var_0 )
{
    ...
    self endon( "kill_instance_of_molotov" + var_0 );
    self endon( "disconnect" );
    level endon( "game_ended" );
    self.moloachievementvictims = 0;

    if ( istrue( self.monitor_inside_building ) )
        return;
    ...
    for (;;)
    {
        if ( self.moloachievementvictims > 3 )
        {
            ...
            self.monitor_inside_building = 1;
            self notify( "kill_instance_of_molotov" + var_0 );
        }
...
molotov_stuck_player( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isagent( var_1 ) && istrue( var_1.buildweapon_blueprintwithcustomattachments ) )
    {
    ...
    var_11 = physics_raycast( var_7, var_9, var_10, var_0, 0, "physicsquery_closest", 1 );

    if ( isdefined( var_11 ) && var_11.size > 0 )
    {
        var_9 = var_11[0]["position"];
        var_12 = var_11[0]["normal"];
        var_13 = var_11[0]["entity"];
...
molotov_simulate_impact( var_0, var_1, var_2, var_3, var_4, var_5 )
{
    ...
    var_22 = molotov_create_branch( var_18, var_20, var_21, undefined, var_13, var_34, var_3, 0, var_26, var_27 );
    var_18.branches[var_18.branches.size] = var_22;
    silencer_pick_up_monitor( var_18 );
...
silencer_pick_up_monitor( var_0 )
{
    var_0.caststotal++;
    var_0.caststhisframe++;
    var_0.frametimestamp = gettime();
    var_1 = siege_bot_team_triple_cap_check();
...
silo_door( var_0 )
{
    ...
    if ( var_1.scriptables.size > var_1.setdropbagdelay )
        shutdownattractionicontrigger();
}
...
silo_door_clip( var_0 )
{
    ...
    if ( var_1.triggers.size > var_1.setdropbagdelay )
        shutdowngulagforalivecount();
}

sidehouse_intel_sequence()
{
    var_0 = siege_bot_team_triple_cap_check();
    ...
    if ( !isdefined( var_1 ) )
        var_0.vehicleunlink = scripts\engine\utility::array_remove( var_0.vehicleunlink, self.id );
...
shutdownattractionicontrigger( var_0 )
{
    ...
    foreach ( var_3 in var_1.vehicleunlink )
    {
    ...
    var_0 = istrue( var_0 ) || var_1.play_extract_3mins;

    if ( !isdefined( var_2 ) )
        shuttingdown( var_3, var_0 );
}
...
shuttingdown( var_0, var_1 )
{
    var_2 = siege_bot_team_triple_cap_check();
    ...
    if ( isdefined( var_3 ) )
    {
        if ( !istrue( var_1 ) )
        {
            ...
            var_2.vehicleunlink = scripts\engine\utility::array_remove( var_2.vehicleunlink, var_0 );
            var_3 thread molotov_pool_end();
            return;
        }

        var_3 thread sidehouse_intel_sequence();
    }

    var_4 = var_2.triggers[var_0];

    if ( isdefined( var_4 ) )
        var_4 thread sidekillcount();
}
...
showtacmaphint( var_0 )
{
    ...
    if ( var_1.caststhisframe >= var_1.setarenaomnvarhealthtype )
        return 0;
...
silo_door_cover_jugg()
{
    ...
    foreach ( var_2 in self.branches )
    {
        ...
        {
            if ( isdefined( var_4 ) )
                var_0.stealth_alert_music[var_0.stealth_alert_music.size] = var_4;
        }
    }
}
...
molotov_start_branch()
{
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            if ( !showtacmaphint( self.shareddata ) )
            {
                waitframe();
                continue;
            }
...
{
    silencer_pick_up_monitor( self.shareddata );
    self.casts++;
...
{
    var_10 = easepower( "equip_molotov_pool_mp_p", var_0, var_1 );
    ...
    if ( isdefined( var_2 ) )
    {
        ...
        var_10 scriptablesetparententity( var_2, var_11, var_12 );
    }
...
molotov_branch_create_pool( var_0, var_1, var_2 )
{
    ...
    var_7.id = signal_strength();
    var_7 silo_door( self.shareddata );
    return var_7;
}
...
molotov_pool_end()
{
    ...
    if ( isdefined( self.trigger ) )
        self.trigger thread sidekillcount();

    wait 3.5;
    thread sidehouse_intel_sequence();
}
...
molotov_create_pool_trigger( var_0, var_1, var_2 )
{
    ...
    var_4 thread silo_door_cover_spawners();
    var_4 silo_door_clip();
    return var_4;
}

silo_door_cover_spawners()
{
    self endon( "death" );
...
molotov_watch_pool_trigger_enter()
{
    ...
    for (;;)
    {
        ...
        if ( isagent( var_0 ) && istrue( var_0.buildweapon_blueprintwithcustomattachments ) && isdefined( self.inflictor ) )
        {
...
siege_bot_team_triple_cap_check()
{
...
{
    var_0 = siege_bot_team_triple_cap_check();
    var_1 = var_0._id_12A44;
    var_0._id_12A44++;
    var_0.vehicleunlink = scripts\engine\utility::array_add( var_0.vehicleunlink, var_1 );
    return var_1;
}

molotov_get_cast_data( var_0 )
{
    var_1 = siege_bot_team_had_advantage();
    var_2 = spawnstruct();
...
molotov_update_burning()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_2 ) )
        {
            ...
            if ( isagent( self ) )
            {
                if ( !istrue( self._id_12622 ) )
                {
                    if ( isplayer( var_4.attacker ) && var_4.attacker silo_door_right() )
                    {
                        if ( !istrue( var_4.attacker.monitor_inside_building ) )
            ...
            {
                var_10 = "MOD_EXPLOSIVE";

                if ( isagent( self ) )
                    var_10 = "MOD_FIRE";

                self dodamage( var_6, var_7, var_4.attacker, var_4.inflictor, var_10, "molotov_mp" );
...
silo_door_right()
{
    if ( isdefined( self.reset_button_init ) && gettime() - self.reset_button_init < 4000 )
        return 1;
    else
        return 0;
}
...
molotov_begin_fx()
{
    ...
    self.vehicle_damage_setvehiclehitdamagedata = 1;
...
molotov_end_fx()
{
    ...
    if ( istrue( self.vehicle_damage_setvehiclehitdamagedata ) )
    {
        self setscriptablepartstate( "equipMtovFXWorld", "neutral", 0 );
        self setscriptablepartstate( "equipMtovFXView", "neutral", 0 );
    }

    self.vehicle_damage_setvehiclehitdamagedata = undefined;
}
    ...
    if ( isdefined( var_0.shareddata ) )
    {
        if ( isdefined( var_0.shareddata.branches ) )
        {
            foreach ( var_2 in var_0.shareddata.branches )
                showteamlittlebirds( var_2 );
        }

        if ( isdefined( var_0.shareddata.stealth_alert_music ) )
        {
            foreach ( var_5 in var_0.shareddata.stealth_alert_music )
            {
                if ( isdefined( var_5 ) )
