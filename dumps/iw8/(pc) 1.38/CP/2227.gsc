...
init()
{
    ...
    if ( !isdefined( level.dropbrplatepouch ) )
        level.dropbrplatepouch = ::choppersupport_movetolocation;
    ...
    level.heli_structs_entrances = scripts\cp_mp\utility\game_utility::markedfordelete( "ks_heli_entrance" );
    level.heli_structs_goals = scripts\cp_mp\utility\game_utility::markedfordelete( "ks_heli_goal" );
    level.heli_structs_paths = scripts\cp_mp\utility\game_utility::markedfordelete( "ks_heli_path" );
...
spawnchopper( var_0, var_1 )
{
    ...
    var_22.fast_rope_over_black = var_2[2] - 750;
    var_22._id_123A8 = 50;
    var_22._id_123A9 = 25;
    var_22._id_123AA = undefined;
    var_22.shot_to_miss_for_dialog = 7;
    var_22.should_ai_take_damage = 12;
...
choppersupport_monitorowner()
{
    ...
    self.owner scripts\engine\utility::_id_12E3F( "joined_team", "disconnect" );
    thread choppersupport_leave();
}
...
choppersupport_engageturrettarget( var_0 )
{
    ...
    for (;;)
    {
        if ( !istrue( var_0.turreton ) || istrue( var_0._id_129D1 ) )
        {
            waitframe();
            continue;
        }
        ...
        if ( isdefined( var_1 ) && var_1.size > 0 )
        {
            ...
            {
                if ( istrue( self.brjugg_dropfunc ) && isdefined( self.gametypeoverridedomsearchparams ) )
                    self thread [[ self.gametypeoverridedomsearchparams ]]();
                else if ( isdefined( self.tell_player_to_pick_up_intel ) )
                    self thread [[ self.tell_player_to_pick_up_intel ]]();
                else
...
choppersupport_acquireturrettarget( var_0, var_1 )
{
    ...
    if ( isdefined( var_4 ) )
    {
        ...
        if ( istrue( var_5 ) && self.currenttarget == var_4 && !istrue( self.brjugg_dropfunc ) )
            self thread [[ level.dropbrplatepouch ]]( var_4, 1 );
...
choppersupport_clearcurrenttarget( var_0 )
{
    ...
    if ( istrue( var_0._id_1265B ) )
        var_0._id_1265B = undefined;
...
choppersupport_fireonturrettarget( var_0, var_1, var_2, var_3, var_4 )
{
    ...
    thread dropbrlootchoppercrateforpublicevent( var_0 );
    thread choppersupport_watchtargetlos( var_0, var_2 );
    thread choppersupport_watchtargettimeout( var_0 );
    thread dropbrkillstreak( var_0 );
    var_5 = weaponfiretime( "chopper_support_turret_mp" );
    var_6 = 0;
    var_7 = 100;

    if ( isdefined( self._id_123A8 ) )
        var_7 = self._id_123A8;

    var_8 = 20;

    if ( isdefined( self.shot_to_miss_for_dialog ) )
        var_8 = self.shot_to_miss_for_dialog;

    var_9 = 40;

    if ( isdefined( self.should_ai_take_damage ) )
        var_9 = self.should_ai_take_damage;

    if ( istrue( var_4 ) )
    {
        var_10 = 3750;

        while ( istrue( dropbrcustompickupitem( var_0, var_1 ) ) )
        {
    ...
    while ( istrue( dropbrcustompickupitem( var_0, var_1 ) ) )
    {
        if ( istrue( var_0._id_1265B ) )
        {
            if ( isdefined( self.currenttarget ) && var_1 == self.currenttarget && !istrue( self.brjugg_dropfunc ) )
                self [[ level.dropbrplatepouch ]]( self.currenttarget, 1 );
            else
        ...
        else if ( var_0 choppersupport_turretlookingattarget() )
        {
            ...
            if ( var_6 == var_8 )
            {
                ...
                if ( isdefined( self._id_123A9 ) )
                    var_7 = self._id_123A9;
            }
            else if ( var_6 == var_9 )
            {
                var_7 = undefined;

                if ( isdefined( self._id_123AA ) )
                    var_7 = self._id_123AA;
            }
...
dropbrcustompickupitem( var_0, var_1 )
{
...
dropbrlootchoppercrateforpublicevent( var_0 )
{
    ...
    if ( !scripts\cp_mp\utility\game_utility::punchcard_use_think() )
        return;
    ...
    for (;;)
    {
        if ( isdefined( var_0.attackingtarget ) )
        {
            ...
            {
                if ( !istrue( self.participantxp ) )
                    self.participantxp = 1;
...
choppersupport_watchtargetlos( var_0, var_1 )
{
    ...
    for (;;)
    {
        if ( !istrue( var_0._id_1265B ) && isdefined( var_0.attackingtarget ) )
        {
...
choppersupport_patrolfield( var_0 )
{
    ...
    if ( isdefined( self.tell_player_to_pick_up_intel ) )
        self [[ self.tell_player_to_pick_up_intel ]]( var_0 );
    else
    {
        if ( self.currentaction != "patrol" )
        ...
        for (;;)
        {
            ...
            if ( isdefined( var_3 ) && !istrue( self.brjugg_dropfunc ) )
                self [[ level.dropbrplatepouch ]]( var_3, 1 );
            else
            {
                var_5 = [];
                var_6 = ( 0, 0, 0 );
                ...
                if ( isdefined( var_6 ) && var_5.size > 0 && !istrue( self.brjugg_dropfunc ) )
                {
                    var_11 = var_6 / var_5.size;
                    self [[ level.dropbrplatepouch ]]( var_11 );
                }
            }
...
checkpoint_carepackage_munitions_think( var_0, var_1 )
{
    self endon( "death" );
    self endon( "leaving" );
    self endon( "crashing" );
    ...
    for (;;)
    {
        ...
        if ( istrue( self.brjugg_dropfunc ) && isdefined( self.gametypeoverrideassassinsearchparams ) )
            break;
        else
        {
            var_7 = 20;
...
choppersupport_cleanup( var_0 )
{
    ...
    if ( isdefined( self.enable_lbravo_player_infil ) )
        self [[ self.enable_lbravo_player_infil ]]();

    if ( isdefined( self.streakinfo ) )
        self.streakinfo.init_swivelroom_currsolution_marquee = istrue( var_0 );
...
choppersupport_gettargets( var_0, var_1, var_2, var_3 )
{
    ...
    for ( var_7 = 0; var_7 < var_5.size; var_7++ )
    {
        ...
        if ( istrue( var_10 ) )
        {
            var_13 = spawnstruct();
            var_13.player = var_8;
            var_13._id_11D1E = var_11;
            var_13._id_1266F = var_12;
...
choppersupport_istarget( var_0, var_1, var_2, var_3 )
{
    ...
    if ( isdefined( self.reading ) && ![[ self.reading ]]( var_1 ) )
        return [ 0, 0, 0 ];

    if ( !dropbrequipment( var_1 ) )
        return [ 0, 0, 0 ];
...
choppersupport_getbesttarget( var_0, var_1 )
{
    ...
    {
        if ( !dropbrequipment( var_7.player ) )
            continue;

        if ( choppersupport_isactivetarget( var_7.player ) && !istrue( var_7._id_1266F ) )
            continue;
        ...
        if ( istrue( var_7._id_11D1E ) )
        {
            var_8 = 1;
            var_10 = var_10 + 40;
        }

        if ( istrue( var_7._id_1266F ) )
        {
            var_9 = 1;
            var_10 = var_10 + 20;
        }
...
choppersupport_explode()
{
    ...
    if ( isdefined( self.gamestatedisplaymonitor ) )
...
choppersupport_crash( var_0 )
{
    ...
    if ( istrue( self.brjugg_dropfunc ) && isdefined( self.gamemodemolotovfunc ) )
    {
        self setneargoalnotifydist( 300 );
        var_1 = 1;
        var_2 = [[ self.gamemodemolotovfunc ]]();
    }
    else
...
{
    var_3 = self.origin;
    var_4 = self.fast_rope_over_black;
    var_5 = undefined;
...
beginevasivemaneuvers()
{
    ...
    var_0 = scripts\engine\utility::_id_12E53( 3.0, "death" );
...
choppersupport_handledeathdamage( var_0 )
{
    ...
    if ( istrue( self.randomize_wires_on_bombs ) && isdefined( level._id_12111 ) )
    {
        if ( isdefined( var_0.objweapon ) )
            [[ level._id_12111._id_12123 ]]( var_0.objweapon.basename );
        else
            [[ level._id_12111._id_12123 ]]( "none" );
    }
