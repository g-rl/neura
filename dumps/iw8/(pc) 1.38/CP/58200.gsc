...
init()
{
    ...
    level.nuke_expl_struct = scripts\cp_mp\utility\game_utility::markedentitieslifeindices( "nuke_expl_pos" );
}
...
tryusenukefromstruct( var_0 )
{
    ...
    if ( !isdefined( level.nukeincoming ) )
    {
        ...
        level.stack_patch_waittill_context_far = self;
    }
    else
    {
    ...
    else if ( !istrue( level.setup_level_for_nightvision ) )
    ...
    if ( !istrue( var_4 ) || level.gameended )
    {
        ...
        level.stack_patch_waittill_context_far = undefined;
        var_0 notify( "killstreak_finished_with_deploy_weapon" );
        return 0;
    }
    ...
    if ( var_1 != 1 && !istrue( level.setup_level_for_nightvision ) )
        scripts\common\utility::_id_12965( level.set_ui_omnvar_for_relics, "nuke", self.origin );
...
{
    if ( !isdefined( level.sq_entergulag ) )
    {
        createheadiconatorigin( "nuke" );
        level.sq_entergulag = 1;
    }
    ...
    if ( isdefined( level.nukeinfo.player ) )
    {
        ...
        if ( !istrue( level.setup_level_for_nightvision ) )
        {
            if ( isdefined( level.nukeinfo.player ) && isdefined( var_1 ) )
                level.nukeinfo.player scripts\cp_mp\utility\killstreak_utility::x1loadout( var_1 );
        }
    }
