...
main()
{
    ...
    setdvar( "dvar_E547FE8DFE1679F6", 30 );
    setdvar( "dvar_36374C04E23480B6", 1 );
    ...
    if ( isdefined( level.outofboundstriggers ) )
    {
        foreach ( trigger in level.outofboundstriggers )
        {
            if ( isdefined( trigger.script_team ) )
                trigger thread _id_924A93F46B348A86();
        }
    }
}

_id_924A93F46B348A86()
{
    ...
    for (;;)
    {
        ...
        if ( isplayer( ent ) && isalive( ent ) )
        {
            if ( isdefined( ent.carryobject ) )
                ent.carryobject thread scripts\mp\gameobjects::_id_1069580BC0A235CB( ent, ent.carryobject._id_EA5E94E328A4B626 );
        }
        else if ( ent scripts\common\vehicle::isvehicle() )
        {
            if ( isdefined( ent.occupants ) )
            {
                foreach ( player in ent.occupants )
                {
                    ...
                    {
                        player.carryobject scripts\mp\gameobjects::_id_1069580BC0A235CB( player, player.carryobject._id_EA5E94E328A4B626 );
                        level.teamflags[scripts\mp\utility\game::getotherteam( player.team )[0]] scripts\mp\gameobjects::_id_025261199A27252E( getclosestpointonnavmesh( player.origin ) );
                    }
                }
            }
        }
...
initializematchrules()
{
    ...
    setdynamicdvar( "dvar_A628CE6D2CCF681A", getmatchrulesdata( "kothData", "zoneLifetime" ) );
    setdynamicdvar( "dvar_BF8ECE70150BE9DC", getmatchrulesdata( "kothData", "zoneCaptureTime" ) );
    setdynamicdvar( "dvar_8089ACAE7C7DFEC4", getmatchrulesdata( "kothData", "zoneActivationDelay" ) );
    setdynamicdvar( "dvar_03C161B296C1897D", getmatchrulesdata( "kothData", "randomLocationOrder" ) );
    setdynamicdvar( "dvar_6C5E3D11225E952C", getmatchrulesdata( "kothData", "additiveScoring" ) );
    setdynamicdvar( "dvar_B0E9E227D52251DA", getmatchrulesdata( "kothData", "pauseTime" ) );
    setdynamicdvar( "dvar_CAE6BA1D384F03A1", getmatchrulesdata( "tdefData", "spawnDelay" ) );
    scripts\mp\utility\game::registerhalftimedvar( "bigctf", 0 );
}
...
onstartgametype()
{
    ...
    if ( istrue( level.userallypointvehicles ) )
    {
        ...
        if ( getdvarint( "dvar_C6C5667B81CD74B7", 1 ) == 1 )
        {
            _id_CF8266A9263B0005( "veh9_palfa" );
            _id_CF8266A9263B0005( "little_bird" );
        }
...
_id_CF8266A9263B0005( _id_7731ADEF63E19B0C )
{
    foreach ( vehicle in scripts\cp_mp\vehicles\vehicle_spawn::_id_B08E7E3A0B14F76F( _id_7731ADEF63E19B0C ) )
...
ondrop( player )
{
    ...
    if ( isdefined( player ) )
    {
        if ( !scripts\mp\utility\player::isreallyalive( player ) )
            player.carryobject._id_8E9C043FFF68AFDB = [];
    ...
    scripts\mp\utility\dialog::leaderdialog( "ally_dropped", scripts\mp\utility\game::getotherteam( self.originalownerteam )[0], "status" );
    thread _id_788C913D9AAFBC7A();
    ...
    if ( isdefined( level.outofboundstriggers ) )
    {
        foreach ( trigger in level.outofboundstriggers )
        {
            ...
            {
                if ( !scripts\engine\utility::is_equal( trigger.script_team, player.team ) )
                    break;
...
onpickup( player, _id_5760E0F038D1BAA3, defused )
{
    ...
    self._id_856484AF8370F6A1 = undefined;
    team = player.pers["team"];
    ...
    if ( team != ownerteam )
    {
        thread scripts\mp\hud_util::teamplayercardsplash( "callout_flagreturn", player );
        ...
        player playsoundtoteam( "uin_mp_flag_enemy_returned", scripts\mp\utility\game::getotherteam( team )[0] );
    ...
    else
    {
        ...
        if ( getdvarint( "dvar_83FCAA8032225364", 1 ) == 0 )
            level.capzones[otherteam] scripts\mp\gameobjects::allowuse( "none" );

        level.capzones[otherteam] scripts\mp\gameobjects::setvisibleteam( "any" );
        level.capzones[otherteam] scripts\mp\gameobjects::setobjectivestatusallicons( level._id_94475B5B42EBA645, level._id_0903BC4863FEF659, level.mlgiconfullflag );
        player playsoundtoteam( "uin_mp_flag_ally_pickup", team );
        player playsoundtoteam( "uin_mp_flag_enemy_pickup", scripts\mp\utility\game::getotherteam( team )[0] );
        ...
        if ( !isdefined( self._id_8E9C043FFF68AFDB ) )
            self._id_8E9C043FFF68AFDB = [];

        if ( !isdefined( self._id_8E9C043FFF68AFDB[player.name] ) || self._id_8E9C043FFF68AFDB[player.name] <= gettime() )
        {
            player thread scripts\mp\utility\points::_id_0366980B6A8796AE( "stat_91372813B1BCAF92" );
            self._id_8E9C043FFF68AFDB[player.name] = gettime() + 10000;
        }
...
returnaftertime()
{
    ...
    while ( _id_8E53D4CA3DE8531A < level.idleresettime )
    {
        ...
        if ( self.claimteam == "none" )
        {
            ...
            progress = _id_8E53D4CA3DE8531A / level.idleresettime;

            if ( _id_8E53D4CA3DE8531A >= level.idleresettime )
                progress = 1;
        }
    }
...
createflagsandhud()
{
    ...
    level._id_94475B5B42EBA645 = "waypoint_home_flag";
    level._id_0903BC4863FEF659 = "waypoint_enemy_home_flag";
    setupwaypointicons();
...
createteamflag( team, _id_B7BC03D62FDB8D7F )
{
    ...
    if ( !isdefined( trigger ) )
    {
        ...
        trigger = spawn( "trigger_radius", _id_B7BA378AE30BEDB4.origin - ( 0, 0, _id_B7BA378AE30BEDB4.radius / 2 ), 0, _id_B7BA378AE30BEDB4.radius, 65 );
    ...
    _id_3BC2B88A59B6D062.ui_ctf_securing = 0;
    ...
    _id_3BC2B88A59B6D062.onpickupfailed = ::onpickupfailed;
    _id_3BC2B88A59B6D062.ondrop = ::ondrop;
    _id_3BC2B88A59B6D062.onreset = ::onreset;
    _id_3BC2B88A59B6D062.pinobj = 1;
    _id_3BC2B88A59B6D062.cancontestclaim = 0;
    _id_3BC2B88A59B6D062.majoritycapprogress = 0;
    _id_3BC2B88A59B6D062.alwaysstalemate = 1;

    if ( _id_3BC2B88A59B6D062.originalownerteam == "allies" )
        setomnvar( "ui_ctf_flag_axis", -2 );
    else
        setomnvar( "ui_ctf_flag_allies", -2 );

    if ( getdvarint( "dvar_5299A4635AE94475", 1 ) != 0 )
        _id_3BC2B88A59B6D062 scripts\mp\gameobjects::_id_316D9DA870E12A03( [ visuals[0] ], trigger, 1.0, 80, 0 );
...
createcapzone( team, _id_B7BC03D62FDB8D7F )
{
    ...
    if ( !isdefined( trigger ) )
    {
        ...
        trigger.linktoenabledflag = 1;
        trigger.baseorigin = trigger.origin;
        trigger.height = _id_B7BA378AE30BEDB4.radius;
    }

    _id_25D213B4714288CE = spawn( "trigger_radius", trigger.origin, 0, _id_B7BA378AE30BEDB4.radius, trigger.height );
    trigger = _id_25D213B4714288CE;
    visuals = [];
    ...
    _id_E1C2BB288FC23E0D scripts\mp\gameobjects::setvisibleteam( "none" );
    _id_E1C2BB288FC23E0D thread _id_A012C67B437C38BF();
...
flag_default_origins()
{
    ...
    if ( !isdefined( level._id_3E5F11DDB56644D6 ) )
    {
        _id_A24FCD086E08D7B3 = getdvar( "dvar_99AD48D73B20DCCD" );

        if ( !isdefined( _id_A24FCD086E08D7B3 ) )
            level.default_flag_origins[game["attackers"]] = scripts\cp_mp\utility\game_utility::getlocaleent( level._id_6121D66FFC5B8EC9["allies"] ).origin;
        else
            level.default_flag_origins[game["attackers"]] = scripts\engine\utility::stringtovec3( _id_A24FCD086E08D7B3 );
    }
    else
        level.default_flag_origins[game["attackers"]] = level._id_3E5F11DDB56644D6;

    if ( !isdefined( level._id_D0CA59AC91E54957 ) )
    {
        _id_F15D0E42732EF9EA = getdvar( "dvar_99AD48D73B20DCCD" );

        if ( !isdefined( _id_F15D0E42732EF9EA ) )
        {
            level.default_flag_origins[game["defenders"]] = scripts\cp_mp\utility\game_utility::getlocaleent( level._id_6121D66FFC5B8EC9["axis"] ).origin;
            return;
        }

        level.default_flag_origins[game["defenders"]] = scripts\engine\utility::stringtovec3( _id_F15D0E42732EF9EA );
        return;
    }
    else
        level.default_flag_origins[game["defenders"]] = level._id_D0CA59AC91E54957;
}

_id_60DB244685153D79()
{
    _id_F7FA5CE20B84C0B7 = "ctf_flag_axis_CS";
    _id_FE64D65ADB1592CC = "ctf_flag_allies_CS";
    _id_B70903BC2623A801 = scripts\engine\utility::getstructarray( _id_F7FA5CE20B84C0B7, "script_noteworthy" );
    _id_3F7009B45A5F82BE = scripts\engine\utility::getstructarray( _id_FE64D65ADB1592CC, "script_noteworthy" );

    if ( isdefined( _id_B70903BC2623A801 ) )
        level._id_D0CA59AC91E54957 = scripts\engine\utility::drop_to_ground( _id_B70903BC2623A801[0].origin, 10, -100 );

    if ( isdefined( _id_3F7009B45A5F82BE ) )
        level._id_3E5F11DDB56644D6 = scripts\engine\utility::drop_to_ground( _id_3F7009B45A5F82BE[0].origin, 10, -100 );
}
...
updategametypedvars()
{
    ...
    level.numflagsscoreonkill = scripts\mp\utility\dvars::dvarintvalue( "snumFlagsScoreOnKill", 0, 0, 3 );
    ...
    level.idleresettime = scripts\mp\utility\dvars::dvarfloatvalue( "idleResetTime", 60, 0, 300 );
    ...
    level.returntime = scripts\mp\utility\dvars::dvarfloatvalue( "returnTime", 20, -1, 60 );
    level.watchdvars["dvar_F1E3320C1FDD7C60"].value = 1;
    scripts\mp\utility\game::setovertimelimitdvar( scripts\mp\utility\dvars::dvarfloatvalue( "overTimeLimit", 300, 0, 1200 ) );
...
setupwaypointicons()
{
    ...
    foreach ( _id_DFBD7318EF3CC9BD, _id_D442547D75DFFD09 in level._id_0B23156D776B1D85._id_038F2A11237246AC )
    {
        ...
        _id_0E7865AE7BE35347 = &"MP_INGAME_ONLY/OBJ_RECOVER_CAPS";
        ...
        scripts\mp\gamelogic::setwaypointiconinfo( level._id_94475B5B42EBA645, 2, "friendly", "OBJECTIVES/FLAG_HOME", "icon_waypoint_flag_base_gone", 0, 13 );
        scripts\mp\gamelogic::setwaypointiconinfo( level._id_0903BC4863FEF659, 2, "enemy", "OBJECTIVES/FLAG_AWAY", "icon_waypoint_flag_base_gone", 0, 13 );
...
onuse( player )
{
    if ( !level.gameended )
    {
        ...
        _id_929A6DF466DC4F22 = int( game["teamScores"][team] );
        _id_AC865A705B3E1162 = int( game["teamScores"][otherteam] );

        if ( _id_929A6DF466DC4F22 == _id_AC865A705B3E1162 && _id_929A6DF466DC4F22 == level.roundscorelimit - 1 && _id_AC865A705B3E1162 == level.roundscorelimit - 1 )
        {
        ...
        if ( _id_929A6DF466DC4F22 == level.roundscorelimit - 1 )
        {
