...
_id_FA1BFB7A091B5790( agent )
{
    if ( scripts\cp_mp\utility\script_utility::issharedfuncdefined( "agents", "canActivateAgent", agent ) && ![[ scripts\cp_mp\utility\script_utility::getsharedfunc( "agents", "canActivateAgent" ) ]]( agent ) )
        return 0;

    if ( !istrue( agent.isactive ) )
    {
        if ( !istrue( agent.waitingtodeactivate ) )
            return 1;
    }
...
getfreeagent( agent_type )
{
    ...
    foreach ( index, agent in level.agentarray )
    {
        if ( _id_FA1BFB7A091B5790( agent ) )
        {
            agent.agent_type = agent_type;
            agent._id_6843D82ED42EA7DE = index;
            agent initagentscriptvariables();
            return agent;
        }
    }
}
