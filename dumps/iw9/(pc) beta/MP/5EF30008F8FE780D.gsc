...
cyberattack()
{
    ...
    visuals[0] = _id_8A65B17BA13A8FDD[level.empspawnindex];
    ...
    visuals[0] setmodel( "offhand_wm_emp" );
    visuals[0].origin = visuals[0].origin;
    ...
    visuals[0].angles = vectortoangles( trace["normal"] ) + ( 180, 0, 0 );

    for ( _id_AC0E594AC96AA3A8 = 0; _id_AC0E594AC96AA3A8 < _id_8A65B17BA13A8FDD.size; _id_AC0E594AC96AA3A8++ )
    {
        if ( _id_AC0E594AC96AA3A8 == level.empspawnindex )
            continue;

        _id_8A65B17BA13A8FDD[_id_AC0E594AC96AA3A8] delete();
    }
...
createbombzone( team, trigger, defusetrig )
{
    ...
    _id_EEF26A325310D3AF.useweapon = makeweapon( "emp_bomb_mp" );
...
onpickup( player, _id_5760E0F038D1BAA3, defused )
{
    ...
    player scripts\cp_mp\utility\inventory_utility::_giveweapon( "iw8_cyberemp_mp" );

    if ( !istrue( defused ) && !player scripts\mp\utility\killstreak::isjuggernaut() && !isbot( player ) )
        player scripts\cp_mp\utility\inventory_utility::_switchtoweaponimmediate( "iw8_cyberemp_mp" );
...
ondrop( player )
{
    ...
    player scripts\cp_mp\utility\inventory_utility::_takeweapon( "iw8_cyberemp_mp" );
    player scripts\mp\utility\inventory::switchtolastweapon();
}
...
bombplanted( _id_2810BD61A09D19F5, team )
{
    ...
    _id_EFDFC6EBE7A152C5 = spawnfx( level._effect["emp_detonation"], _id_9AF72BBDFDEACE5D, ( 0, 0, 1 ), ( cos( _id_CB89120314447D62 ), sin( _id_CB89120314447D62 ), 0 ) );
...
awardgenericmedals( einflictor, attacker, idamage, smeansofdeath, objweapon, vdir, shitloc, psoffsettime, deathanimduration, _id_61B5D0250B328F00 )
{
    ...
    if ( isdefined( level.cyberemp.carrier ) )
    {
        ...
        {
            if ( isdefined( attacker.isbombcarrier ) && attackerisinflictor && isdefined( objweapon ) && objweapon.basename == "iw8_cyberemp_mp" )
            {
