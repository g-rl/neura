...
any_groups_in_combat( stealthgroups )
{
    ...
    {
        if ( isdefined( stealthgroups ) && !scripts\engine\utility::array_contains( stealthgroups, group.name ) )
            continue;

        if ( scripts\stealth\group::group_anyoneincombat( group.name ) )
            return 1;
    }
