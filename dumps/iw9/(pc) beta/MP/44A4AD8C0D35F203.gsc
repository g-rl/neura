...
init( _id_8611E1D824BBDB4A )
{
    ...
    level._effect["whizby_med"] = loadfx( "vfx/iw9/core/weapons/whizby/vfx_test_whizby" );
}
...
_id_A6E6611EFB4164D5( attacker, distance, position, forward )
{
    ...
    if ( _id_43D3999D682DE782( attacker, distance, position, forward, time ) )
    {
        ...
        _id_2E141A5600AF7C32( attacker, position, forward );
...
_id_2E141A5600AF7C32( attacker, position, forward )
{
    if ( isdefined( attacker ) && isdefined( position ) )
    {
        if ( isdefined( attacker.currentweapon ) && attacker tagexists( "tag_flash" ) )
            _id_3C0DBCD729E6D642 = attacker gettagorigin( "tag_flash" );
        else
            _id_3C0DBCD729E6D642 = attacker.origin + ( 0, 0, 50 );

        if ( distance( _id_3C0DBCD729E6D642, position ) < 400 )
            return;

        playfx( level._effect["whizby_med"], position, forward * -1, ( 0, 0, 1 ), self );
    }
}
