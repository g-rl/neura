...
main()
{
    ...
    if ( !isdefined( level.waypointcolors ) || !isdefined( level.waypointcolors["bounty_enemy"] ) )
        scripts\mp\gamelogic::setwaypointiconinfo( "bounty_enemy", 0, "enemy", "MP_INGAME_ONLY/OBJ_KILL_CAPS", "icon_waypoint_kill", 0 );

    if ( !isdefined( level.waypointcolors ) || !isdefined( level.waypointcolors["bounty_friendly"] ) )
        scripts\mp\gamelogic::setwaypointiconinfo( "bounty_friendly", 0, "friendly", "MP_INGAME_ONLY/OBJ_DEFEND_CAPS", "icon_waypoint_escort", 0 );
...
_id_2C66AE09A0A7C438()
{
    if ( !isdefined( self.trackedobject ) )
    {
        ...
        scripts\mp\objidpoolmanager::objective_add_objective( trackedobject.pingobjidnum, "done", self.origin );
