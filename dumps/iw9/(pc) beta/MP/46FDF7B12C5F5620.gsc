...
_id_F5A4FD44B6A9E2AC()
{
    ...
    if ( istrue( level._id_37E52C7FB7267EFC._id_2713A9CF5318D984 ) )
    {
        foreach ( _id_C5F4970069850479 in _func_B4B9401715CF98EA( "gulag" ) )
            _id_C5F4970069850479 hudoutlineenableforclient( player, "outlinefill_depth_red" );
    }
...
_id_A4B9CC7EDE16275C( arena )
{
    ...
    playsoundatpos( _id_D6635C91EEFF8D6B.origin, "iw8_mp_snatch_fight_start" );
...
_id_7CFCE31D9C561B2D( arenaplayers )
{
    ...
    if ( level._id_37E52C7FB7267EFC._id_54C7916BCEA2B113 == 0 )
    {
        ...
        arena thread _id_5427C6229B93668E();
    }
...
_id_5427C6229B93668E()
{
    arena = self;
    level endon( "game_ended" );

    if ( !isdefined( arena.fightspawns ) )
        return;

    foreach ( player in arena.arenaplayers )
        player thread _id_F3C05E86869E397C();

    position = arena.fightspawns[0].origin + ( 0, 0, 50 );
    playfx( scripts\engine\utility::getfx( "glsmoke" ), position, anglestoup( ( 0, 90, 0 ) ) );
    arena thread scripts\mp\equipment\gas_grenade::gas_createtrigger( position, arena.arenaplayers[1], arena.arenaplayers[1].team, 11, 0.6 );
}

_id_F3C05E86869E397C()
{
    player = self;
    ...
    while ( player _id_2CEDCC356F1B9FC8::isplayerinorgoingtogulag() )
    {
        ...
        player thread _id_870D349C1ED60C21();
    }
}

_id_870D349C1ED60C21()
{
    player = self;
