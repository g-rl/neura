...
infil_init()
{
    ...
    if ( game["roundsPlayed"] > 0 || getdvarint( "dvar_C55DC89EF275CDAA", 0 ) == 1 || getdvarint( "dvar_7194076AB4888F2B", 0 ) == 1 || scripts\cp_mp\utility\game_utility::_id_0BEFF479639E6508() || getdvarint( "dvar_704AFE6F6C582EE0" ) < 15 || ( scripts\mp\utility\game::getgametype() == "war" || scripts\mp\utility\game::getgametype() == "war_mgl" ) && scripts\cp_mp\utility\game_utility::isdonetskmap() )
    {
        game["infil"] = undefined;
    ...
    if ( scripts\mp\utility\game::gamehasinfil() && isdefined( _id_BA9A385288D1A919 ) )
    {
        ...
        level.matchcountdowntime = _id_BA9A385288D1A919 + 2.0;
        level.prematchperiodend = _id_BA9A385288D1A919 + 2.0;
...
waitforstreamsynccomplete()
{
    ...
    if ( isdefined( self.team ) )
    {
        ...
        if ( scripts\mp\flags::gameflag( "infil_will_run" ) && !scripts\mp\flags::gameflag( "prematch_done" ) )
        {
            ...
            self _meth_8B338BF92F461E6C( _id_BA487744C1313B1F );
        }
    }
    ...
    if ( istrue( level._id_91CF3CF3867DBB33 ) )
        scripts\mp\playerlogic::_id_21004EE0DC2CE538();

    if ( isdefined( level._id_A95727228D4A83E8 ) )
    {
        _id_897C2BAC1E48E4CD = ( gettime() - starttime ) / 1000;

        if ( _id_897C2BAC1E48E4CD < level._id_A95727228D4A83E8 )
            wait( level._id_A95727228D4A83E8 - _id_897C2BAC1E48E4CD );
    }
...
infil_scene_fade_in( delay_time, _id_C82EABB722C361A7 )
{
    if ( scripts\mp\flags::gameflag( "infil_started" ) )
        return;
    ...
    while ( _id_C8D49BBA66D8DB34 > 0.0 )
    {
        _id_C8D49BBA66D8DB34 = _id_C8D49BBA66D8DB34 - level.framedurationseconds;
        _id_C8D49BBA66D8DB34 = max( _id_C8D49BBA66D8DB34, 0.0 );
...
_id_4DCA5340DFD36C76( scene_node, vehicle, waittime, animname, _id_643BCFEC059B4AE2 )
{
    ...
    _id_19B99157406B12A1[0].origin = getoriginforanimtime( _id_54B174A893636475, _id_9CA1B8FD292FEFFA, level.scr_anim[animname][_id_643BCFEC059B4AE2], 1.0 );
    _id_19B99157406B12A1[0].angles = getanglesforanimtime( _id_54B174A893636475, _id_9CA1B8FD292FEFFA, level.scr_anim[animname][_id_643BCFEC059B4AE2], 1.0 );
    _id_19B99157406B12A1[0] setscriptablepartstate( "visibility", "hide" );
    wait( waittime );
    vehicle delete();
