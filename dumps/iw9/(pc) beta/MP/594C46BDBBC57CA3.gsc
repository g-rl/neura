...
perk_adsmarktarget_confirmtargetandmark( target )
{
    ...
    if ( self playerads() > 0.5 && perk_adsmarktarget_check( target ) )
    {
        ...
        _id_F98E48F2527D4205 = _id_BE331B5A39B1738A thread scripts\cp_mp\entityheadicons::setheadicon_singleimage( getlivingplayers_team( self.team ), "icon_navbar_enemy", 35, 1, 5000, 0, undefined, 1 );
...
proximityrevivethink()
{
    ...
    for (;;)
    {
        ...
        foreach ( player in players )
        {
            if ( istrue( player.inlaststand ) && !istrue( player.stuckinlaststand ) && !istrue( player.laststandhealisactive ) && !istrue( player scripts\mp\playerlogic::getbeingrevivedinternal() ) && !isdefined( _id_D72D6D8C00A1B78F.owner ) )
        ...
        if ( isdefined( level.revivetriggers ) )
        {
            foreach ( _id_DBCE45A33308630D in level.revivetriggers )
            {
                ...
                {
                    if ( !istrue( _id_DBCE45A33308630D.trigger.owner scripts\mp\playerlogic::getbeingrevivedinternal() ) && !isdefined( _id_D72D6D8C00A1B78F.owner ) )
                    {
...
proximityrevive( player, _id_D72D6D8C00A1B78F, reviveent )
{
    ...
    player scripts\mp\playerlogic::setbeingrevivedinternal( 1 );
...
watchproximityrevivefail( player, _id_D72D6D8C00A1B78F, reviveent )
{
    ...
    if ( isdefined( player ) )
    {
        player scripts\mp\playerlogic::setbeingrevivedinternal( 0 );
        player scripts\mp\gameobjects::updateuiprogress( _id_D72D6D8C00A1B78F, 0 );
        player scripts\mp\utility\player::_freezecontrols( 0, undefined, "proximityRevive" );
...
_id_448448AC4206D715()
{
    ...
    if ( scripts\cp_mp\utility\game_utility::_id_0BEFF479639E6508() )
    {
        if ( isdefined( scripts\mp\supers::getcurrentsuper() ) && scripts\mp\supers::getcurrentsuperpoints() < scripts\mp\supers::getsuperpointsneeded() )
        {
