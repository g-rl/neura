...
setupspawnlocations()
{
    ...
    level.usespawnselection = getdvarint( "dvar_6E73595F4CE7501E", 0 ) == 1 && level.spawnselectionlocations.size > 0;
...
keepstreamposfresh( origin )
{
    ...
    if ( !isdefined( level.worldstreamingkeepalive ) )
        level.worldstreamingkeepalive = max( getdvarint( "dvar_A3F00D15E3C5FDF8", 20000 ) * 0.5 / 1000, 1.0 );

    for (;;)
    {
        self _meth_8B338BF92F461E6C( origin, 1 );
        wait( level.worldstreamingkeepalive );
    }
}
...
issquadmateindanger( player )
{
    ...
    if ( isdefined( player ) && ( isdefined( player.lastdamagetime ) && player.lastdamagetime + 5000 > currenttime ) || isdefined( player.lasttimedamaged ) && player.lasttimedamaged + 5000 > currenttime )
        return 1;
