...
_id_C28362A9638E7931( dropstruct )
{
    ...
    _id_BD693DCF6BECE704 = self.lifetime;
    _id_11F20C6978F7D6A6 = _id_BD693DCF6BECE704 * dropstruct._id_F01F4AB1D5D21F9C;
    _id_00D4DAE76EAC378B = self.centerpt + ( 0, 0, level._id_5D2AF95280A3CF58 );
    _id_F8AAE4B48DFCCACE = _id_95A507DC67864770( _id_00D4DAE76EAC378B );
    _id_6060A1273859D593 = _id_F8AAE4B48DFCCACE + ( 0, 0, level._id_5D2AF95280A3CF58 );

    for ( _id_2534C2313066C75F = distance( self.origin, _id_00D4DAE76EAC378B ); _id_2534C2313066C75F > 250; _id_2534C2313066C75F = _id_849292C6FA4510C4 )
    {
        waitframe();
        _id_849292C6FA4510C4 = distance( self.origin, _id_00D4DAE76EAC378B );

        if ( _id_849292C6FA4510C4 > _id_2534C2313066C75F )
            break;
    }
...
_id_52F17E5FBEC318A6( _id_8EE83E2CBD3D747D, dropstruct )
{
...
{
    _id_FDFE2D4AAF8EC33D = level._id_5D2AF95280A3CF58;
    _id_D4858C7C75F6AB04 = _id_8EE83E2CBD3D747D + ( 0, 0, _id_FDFE2D4AAF8EC33D );
    crate = undefined;
    ...
    if ( isdefined( crate ) )
    {
        ...
        if ( isdefined( dropstruct.funcs["supplyDropFall"] ) )
            crate thread [[ dropstruct.funcs["supplyDropFall"] ]]( _id_8EE83E2CBD3D747D );
    }

    return crate;
}

_id_7DEC3B199D2E73B8( point )
{
    self endon( "death" );
    _id_3E6243388773499C = scripts\engine\utility::randomvector( 1 ) * 1000;
    _id_3E6243388773499C = scripts\engine\utility::drop_to_ground( point + _id_3E6243388773499C );

    for ( timer = 0; distancesquared( scripts\engine\utility::drop_to_ground( self.origin, -100 ), self.origin ) > 10000000; timer = timer - 3 )
    {
        _id_191EBD2DD0D5F975 = randomfloat( 100 ) + 400;
        _id_67A81E668B8A7DBD = randomfloat( 500 ) + 100;
        dir = _id_3E6243388773499C - self.origin;
    ...
    endpos = scripts\engine\utility::drop_to_ground( self.origin );
