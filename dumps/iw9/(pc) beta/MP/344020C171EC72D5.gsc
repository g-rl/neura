...
waitandspawnclient( _id_9156B53BCF7CE573 )
{
    ...
    if ( !istrue( level.disablespawncamera ) && _id_FA3CDBD484FA39C3 && !istrue( self.skipspawncamera ) && !istrue( level.usespawnselection ) && timeuntilspawn > 0 )
    {
...
spawnplayer( _id_13FE2B86C5E85A64, _id_9156B53BCF7CE573 )
{
    ...
    else
    {
        ...
        if ( getdvar( "dvar_87485ECA39B418A6" ) == "1" && scripts\mp\utility\game::_id_BA5574C7F287C587() && !istrue( self.gulag ) )
...
_id_9055412313C726AD()
{
    ...
    for (;;)
    {
        ...
        foreach ( _id_83A92033FBDB65F8 in enemies )
        {
            ...
            {
                _id_80D2F6C978D12FE7 = _id_D2E4BDDEDDC9B4F9 / _id_68A6CD6D3CB58E79.maxdist * ( _id_68A6CD6D3CB58E79._id_07F870143D9150C8 - _id_68A6CD6D3CB58E79._id_779315766223441A ) + _id_68A6CD6D3CB58E79._id_779315766223441A;
                angle = scripts\engine\math::anglebetweenvectors( _id_AA59B8437A574636, anglestoforward( level.player getplayerangles() ) );

                if ( angle < _id_80D2F6C978D12FE7 )
                    enable_outline = 1;
            }

            if ( enable_outline )
            {
                _id_83A92033FBDB65F8 hudoutlineenableforclient( self, getdvar( "dvar_B2A3957007BD8417" ) );
                continue;
            }

            _id_83A92033FBDB65F8 hudoutlinedisableforclient( self );
        }
...
_id_A9D32C0B976412DA( _id_68A6CD6D3CB58E79 )
{
    ...
    _id_68A6CD6D3CB58E79._id_779315766223441A = getdvarfloat( "dvar_82BF79A286A47409", 1.5 );
    _id_68A6CD6D3CB58E79.maxdist = scripts\engine\utility::ter_op( isads, getdvarfloat( "dvar_CDF293EFF59607EC", 1500.0 ), getdvarfloat( "dvar_8A73E104AE342A12", 1000.0 ) );
    _id_68A6CD6D3CB58E79._id_07F870143D9150C8 = scripts\engine\utility::ter_op( isads, getdvarfloat( "dvar_B04F78259746BFAA", 4.0 ), getdvarfloat( "dvar_7544939A6EC22268", 7.0 ) );

    if ( isads && weaponclass( self getcurrentweapon() ) == "sniper" )
    {
        _id_68A6CD6D3CB58E79.maxdist = getdvarfloat( "dvar_19E56997809E86B9", 2000.0 );
        _id_68A6CD6D3CB58E79._id_07F870143D9150C8 = getdvarfloat( "dvar_EF8FA48B3241B8F3", 3.0 );
    }
}
...
onspawnspectator( origin, angles, gameended )
{
    ...
    if ( scripts\mp\utility\game::isgroundwarcoremode() && !self.hasspawned )
        self _meth_8B338BF92F461E6C( spawnpoint.origin );
...
setuipregamefadeup( waitmsg )
{
    self endon( "disconnect" );
    scripts\cp_mp\utility\game_utility::fadetoblackforplayer( self, 1, 0 );
    starttime = gettime();

    if ( isdefined( self ) && isdefined( waitmsg ) )
        self waittill( waitmsg );

    if ( waitmsg == "spawn_cam_pos_predicted" )
    {
        while ( istrue( self.prestreaminglocation ) && !istrue( self._id_F9BCDD83C6C6899F ) )
            waitframe();
    }
    else if ( istrue( level._id_91CF3CF3867DBB33 ) )
        _id_21004EE0DC2CE538();
    else
        _id_2CEDCC356F1B9FC8::playerwaittillstreamhintcomplete();

    if ( isdefined( level._id_79B01F674B33AA92 ) || getdvarint( "dvar_D91436487F90E63C", 0 ) != 0 )
    {
        mintime = level._id_79B01F674B33AA92;
...
_id_21004EE0DC2CE538()
{
    ...
    while ( !self ispredictedstreamposready() )
    {
        ...
        if ( isdefined( level._id_C11DE8F1ACCEEE82 ) && gettime() - starttime > level._id_C11DE8F1ACCEEE82 * 1000 )
            return;
    }
}
...
callback_playerconnect()
{
    ...
    if ( scripts\mp\utility\game::getgametype() == "arm" && !scripts\mp\flags::gameflag( "infil_will_run" ) )
    {
        if ( self.sessionteam == "allies" )
        {
            _id_BEC37858599761DA = level.spawnselectionlocations["gw_fob_alliesHQ"]["allies"].anchorentity.origin;
            _id_D596480119E7A95B = level.spawncameras["gw_fob_alliesHQ"]["allies"];
            thread _id_2724805D492E905D( _id_D596480119E7A95B.origin, _id_D596480119E7A95B.angles );
            _id_2CEDCC356F1B9FC8::playerstreamhintlocation( _id_BEC37858599761DA, 0, undefined, undefined, -1 );
        }
        else if ( self.sessionteam == "axis" )
        {
            _id_BEC37858599761DA = level.spawnselectionlocations["gw_fob_axisHQ"]["axis"].anchorentity.origin;
            _id_D596480119E7A95B = level.spawncameras["gw_fob_axisHQ"]["axis"];
            thread _id_2724805D492E905D( _id_D596480119E7A95B.origin, _id_D596480119E7A95B.angles );
            _id_2CEDCC356F1B9FC8::playerstreamhintlocation( _id_BEC37858599761DA, 0, undefined, undefined, -1 );
        }

        _id_CE91130A97F4C478 = 0;
    }
    else if ( scripts\mp\flags::gameflag( "prematch_done" ) && istrue( level.usespawnselection ) && isdefined( self.sessionteam ) )
    {
        if ( scripts\mp\utility\game::getgametype() == "risk" )
        ...
        {
            _id_D596480119E7A95B = level.spawncameras["gw_fob_alliesHQ"]["allies"];
            self _meth_BA3E37BE1AC8B95C( _id_D596480119E7A95B.origin, 1 );
        }
        else if ( self.sessionteam == "axis" )
        {
            _id_D596480119E7A95B = level.spawncameras["gw_fob_axisHQ"]["axis"];
    ...
    if ( !isdefined( self.pers["streamSyncComplete"] ) && shouldshowwidemapshot( _id_C924752368853F99 ) )
    {
        ...
        waitmsg = "player_active";
    else if ( !scripts\mp\flags::gameflag( "infil_will_run" ) && getdvarint( "dvar_7958073A99A0D67E", 1 ) == 1 && scripts\mp\utility\game::getgametype() == "gwtdm" )
        waitmsg = "spawn_cam_pos_predicted";

    thread setuipregamefadeup( waitmsg );
    ...
    if ( self _meth_8420670EAFC8D391() && getdvarint( "dvar_A7EE88BD60E8075F" ) == 1 )
    {
...
totaldisttracking( spawnorigin )
{
    ...
    for (;;)
    {
        ...
        {
            _id_48C5A587383A90E7 = scripts\mp\utility\game::getsubgametype() != "br";
