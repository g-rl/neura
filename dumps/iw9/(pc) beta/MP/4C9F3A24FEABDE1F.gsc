...
preplayerdamaged( einflictor, eattacker, victim, idamage, idflags, smeansofdeath, objweapon, vpoint, vdir, shitloc, psoffsettime, modelindex, partname )
{
    ...
    _id_5F94B1C8535FE10B = !_id_2669878CF5A1B6BC::iskillstreakweapon( objweapon.basename ) && ( weapontype( objweapon ) == "bullet" || weapontype( objweapon ) == "projectile" );

    if ( scripts\mp\utility\game::_id_BA5574C7F287C587() && getdvarint( "dvar_432DF71393030BE5", 0 ) == 1 && isdefined( objweapon ) && _id_5F94B1C8535FE10B )
...
iskilltradedamage( eattacker, victim, idamage, smeansofdeath, objweapon, _id_B0FC59FF15058522 )
{
    ...
    {
        if ( _id_B0FC59FF15058522 && weaponclass( objweapon ) == "spread" )
            return 1;
        else if ( idamage >= victim.health )
        {
            if ( smeansofdeath == "MOD_MELEE" )
                return 1;
            else if ( _id_B0FC59FF15058522 )
                return 1;
        }
    }
...
_id_053B212D296370F7( victim, idamage )
{
    _id_702BFC08FABD86CB = idamage;
    vehicle = victim scripts\cp_mp\utility\player_utility::getvehicle();

    if ( isdefined( vehicle ) )
        _id_702BFC08FABD86CB = scripts\cp_mp\vehicles\vehicle_damage::_id_9CAC3AF67EE6B40A( vehicle, victim, idamage );
...
modifydamagegeneral( einflictor, eattacker, victim, idamage, idflags, smeansofdeath, objweapon, vpoint, vdir, shitloc, psoffsettime, _id_BE4285B26ED99AB1 )
{
    ...
    idamage = _id_053B212D296370F7( victim, idamage );
...
logattacker( victim, eattacker, einflictor, objweapon, idamage, vpoint, vdir, shitloc, psoffsettime, smeansofdeath )
{
    ...
    if ( isdefined( eattacker ) && isplayer( eattacker ) && isdefined( objweapon ) )
        eattacker thread scripts\mp\weapons::checkhit( objweapon, victim );
...
callback_playerlaststand( einflictor, attacker, idamage, smeansofdeath, objweapon, vdir, shitloc, psoffsettime, deathanimduration )
{
    ...
    if ( !istrue( _id_6D8F177C77496430 ) )
    {
        ...
        if ( !scripts\cp_mp\utility\game_utility::_id_0BEFF479639E6508() && !istrue( level._id_0D88255CD662EB5C ) && !istrue( self.killstreaklaststand ) && ( smeansofdeath == "MOD_MELEE" && objweapon.basename != "iw9_me_fists_mp" && objweapon.basename != "iw9_me_fists_mp_ls" ) )
            return 0;
        ...
        if ( !scripts\cp_mp\utility\game_utility::_id_0BEFF479639E6508() && isexplosivedamagemod( smeansofdeath ) && !istrue( level._id_0D88255CD662EB5C ) )
            return 0;
        ...
        if ( attacker == self && !scripts\cp_mp\utility\game_utility::_id_0BEFF479639E6508() && !istrue( level._id_0D88255CD662EB5C ) )
            return 0;
...
_id_49D8CC008E5F6722( eattacker, victim, idflags )
{
    ...
    victim hudoutlineenableforclient( eattacker, _id_6ACB3CD9C5BB74E9 );
    wait 0.2;
    victim hudoutlinedisableforclient( eattacker );
}
...
gamemodemodifyplayerdamage( einflictor, victim, eattacker, idamage, smeansofdeath, objweapon, vpoint, vdir, shitloc, idflags, _id_BE4285B26ED99AB1 )
{
    if ( isdefined( eattacker ) && isplayer( eattacker ) && isalive( eattacker ) )
    {
        ...
        if ( scripts\cp_mp\utility\game_utility::_id_0B2C4B42F9236924() && !isspreadweapon( objweapon ) )
        {
            switch ( shitloc )
            {
                case "neck":
                case "helmet":
                case "head":
                    idamage = victim.maxhealth + victim._id_8790C077C95DB752;
                    break;
                default:
                    break;
            }
